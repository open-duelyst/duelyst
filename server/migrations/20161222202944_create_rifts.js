exports.up = function(knex, Promise) {
	return Promise.all([
		knex.schema.createTable('user_rift_runs', function (table) {
		...
		knex.schema.createTable('user_rift_tickets', function (table) {
		...
		knex.schema.createTable('user_rift_tickets_used', function (table) {
			table.string('id',36).notNullable().primary()
			table.string('user_id',36).notNullable().index()
			table.string('transaction_type',36)
			table.string('transaction_id',36)
