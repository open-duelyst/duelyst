...
UsableDecks.splitUpDeck = function (deck, numCardsToRemove){
	...
	const lowCostMinions = _.filter(deck.slice(1), function (cardData) {
	...
	const spells = _.filter(deck.slice(1), function (cardData) {
		const card = SDK.GameSession.getCardCaches().getCardById(cardData.id);
	...
	const artifactsOtherMinions = _.filter(deck.slice(1), function (cardData) {
		const card = SDK.GameSession.getCardCaches().getCardById(cardData.id);
	...
	const highCostMinions = _.filter(deck.slice(1), function (cardData) {
		const card = SDK.GameSession.getCardCaches().getCardById(cardData.id);
		return card.getManaCost() > 5 && SDK.CardType.getIsEntityCardType(card.getType()) && !card.getIsGeneral();
	});
...
UsableDecks.randomizeDeck = function (deck, numRandomCards, difficulty) {
	...
	if (numRandomCards != null && numRandomCards > 0) {
		...
		let sizeBeforeFilterForUsable = randomizedDeck.length;
		randomizedDeck = _.filter(randomizedDeck, function (cardData) {
			let cardId = SDK.Cards.getBaseCardId(cardData.id);
		...
		// get usable cards
		let factionId = SDK.FactionFactory.factionIdForGeneralId(randomizedDeck[0].id);
		let usableFactionCardIds = UsableCards.getUsableCardsForFactionId(factionId).slice(0);
		let usableNeutralCardIds = UsableCards.getUsableCardsForFactionId(Factions.Neutral).slice(0);
		...
		if (usableFactionCardIds.length + usableNeutralCardIds.length > 0) {
			...
			let minNumRandomCards = Math.min(usableFactionCardIds.length + usableNeutralCardIds.length, numRandomCards);
			let numCardsToRemove = Math.min(randomizedDeck.length - 1, Math.max(0, minNumRandomCards - numMissingCards));
			...
			while (randomizedDeck.length < CONFIG.MAX_DECK_SIZE && usableFactionCardIds.length + usableNeutralCardIds.length > 0) {
				...
				let randomIndex = Math.floor(Math.random() * cardIdsToPickFrom.length);
				let cardIdToAdd = cardIdsToPickFrom[randomIndex];
