...
var ModifierDyingWishReSpawnEntityAnywhere = require("app/sdk/modifiers/modifierDyingWishReSpawnEntityAnywhere");
var ModifierDyingWishSpawnEntityInCorner = require("app/sdk/modifiers/modifierDyingWishSpawnEntityInCorner");
 ...
 * @constructor
 */
var StarterAI = function (gameSession, playerId, difficulty) {
...
StarterAI.prototype = {
	...
	_resetLastInvalidData: function () {
	...
	nextAction: function () {
		...
		} else if (this.getGameSession().isActive() && this.getGameSession().getCurrentPlayerId() === this.getMyPlayerId()) {
	...
	nextActionForNewGame: function () {
		if (this.getGameSession().isNew()) {
			...
			if (!myPlayer.getHasStartingHand()) {
				...
				var mulliganIndices = [];
				var cardsInHand = myPlayer.getDeck().getCardsInHand();
	 ...
	 * @see nextAction
	 */
	nextActionForActiveGame: function () {
		var nextAction;
		...
		if (this.getGameSession().isActive() && this.getGameSession().getCurrentPlayerId() === this.getMyPlayerId()) {
			...
			if (nextAction != null
				...
				var emitEventWhenInvalid = false;
	...
	_findNextActions: function () {
		if (this.getGameSession().getIsFollowupActive()) {
			...
			var cardWaitingForFollowups = this.getGameSession().getValidatorFollowup().getCardWaitingForFollowups();
			...
			return;
		} else if (this._wantsToKillEggAtPosition != null) {
			// try to find and kill egg at position if we just killed a rebirth unit
			var attackObjectivePosition = this._wantsToKillEggAtPosition;
			this._wantsToKillEggAtPosition = null;
			var eggToAttack = this.getGameSession().getBoard().getUnitAtPosition(attackObjectivePosition);
		...
		var opponentGeneral = this.getOpponentGeneral();
		var opponentUnits = this.getGameSession().getBoard().getFriendlyEntitiesForEntity(opponentGeneral);
		var numOpponentUnits = opponentUnits.length;
		...
		var myUsableUnits = this._getMyUsableUnits();

		// determine what units will not attack
		var numUnitsThatCanMissAttacks = myUsableUnits.length - DIFFICULTY_MIN_UNITS_THAT_ATTACK;
		if (this._difficulty < 1.0 && numUnitsThatCanMissAttacks > 0) {
			var unitsThatCanMissAttacks = _.difference(myUsableUnits, this._unitsMissingAttacks);
			var numMissedAttacks = Math.floor(DIFFICULTY_MAX_MISSED_ATTACKS_PCT * numUnitsThatCanMissAttacks * (1.0 - this._difficulty));
			for (var i = unitsThatCanMissAttacks.length - 1; i >= 0; i--) {
				if (this._unitsMissingAttacks.length >= numMissedAttacks) {
					break;
				} else {
					var unitMissingAttack = unitsThatCanMissAttacks.splice(Math.floor(Math.random() * unitsThatCanMissAttacks.length), 1)[0];
		...
		var maxUnitsRemovedPerTurn = Math.max(1, Math.floor(this._difficulty * DIFFICULTY_MAX_UNITS_REMOVED_PER_TURN));

		// start by allowing only lethal
		var allowOnlyLethalAttacks = true;

		//4, move and attack with units that have lethal on an objective
		//Logger.module("AI").debug("[G:" + this.getGameSession().gameId + "] moving");
		_.each(myUsableUnits, function (unit) {
	...
	_getMyUsableUnits: function () {
		// assemble my units
		// sort units by bounty score from highest to lowest
		// the idea here is that my units with higher bounty (i.e. more atk/hp/abilities)
		// will have a better chance of making favorable trades than my lower bounty units
		var myGeneral = this.getMyGeneral();
		var friendlyUnits = this.getGameSession().getBoard().getFriendlyEntitiesForEntity(myGeneral, SDK.CardType.Unit);
		friendlyUnits.push(myGeneral);
		var myUsableUnits = [];
		_.each(friendlyUnits, function (unit) {
	...
	_replaceCard: function () {
		...
		if (myPlayer.getDeck().getCanReplaceCardThisTurn()) {
			...
			var cardsInHand = myPlayer.getDeck().getCardsInHand();
			cardsInHand = _.reject(cardsInHand, function (card) { return card == null; }.bind(this));
			//first, find first card that costs at least 2 more than our max mana
			var cardToReplace = _.find(cardsInHand, function (card) {
				return (myPlayer.getDeck().getNumCardsReplacedThisTurn() == 0 && card.getManaCost() >= (myPlayer.getMaximumMana() + 2));
			}.bind(this));
			...
			var cardToReplace = _.find(cardsInHand, function (card) {
				return (myPlayer.getDeck().getNumCardsReplacedThisTurn() == 0 && _.where(cardsInHand, { id: card.getBaseCardId() }).length > 1);
			}.bind(this));
			if (cardToReplace != null) {
			...
			var spells = _.filter(cardsInHand, function (card) {
				return card instanceof SDK.Spell;
			}.bind(this));
			if (spells.length > 2) {
			...
			// this is intended to encourage play of units
			var artifacts = _.filter(cardsInHand, function (card) {
				return card instanceof SDK.Artifact;
			}.bind(this));
			...
			// this is intended to encourage play of units
			var unit = _.find(cardsInHand, function (card) {
				return card instanceof SDK.Unit;
			}.bind(this));
			if (unit == null) {
	...
	_generalLethalCheck: function () {
		if (!this._checkedForLethalOnEnemyGeneral) {
			...
			if (this._difficulty < Math.max(DIFFICULTY_WHEN_GENERAL_CAN_ATTACK_ENEMY_GENERAL, DIFFICULTY_WHEN_UNITS_CAN_ATTACK_ENEMY_GENERAL)) {
				...
				var myGeneral = this.getMyGeneral();
				var opponentGeneral = this.getOpponentGeneral();
				var opponentGeneralPosition = opponentGeneral.getPosition();
				var opponentGeneralMaxHP = opponentGeneral.getMaxHP();
				var opponentGeneralDamage = opponentGeneral.getDamage();
				...
				var myUsableUnits = this._getMyUsableUnits();
				var totalDamage = opponentGeneralDamage;
				var returnBuffedAtkValueOnly_noCast = true;
				_.each(myUsableUnits, function (unit) {
					...
					if (unit.getCanAttack() && !this._isUnitPreventedFromAttacking(unit)
						...
						// add buffed atk
						var buffedAtk = this._findAtkBuffAndBuffedAtkValue(unit, [opponentGeneral], returnBuffedAtkValueOnly_noCast);
				...
				var currentMana = myPlayer.getRemainingMana();
				_.each(this._getCardsInHandAndSignatureSpell(), function (card) {
					if (this._getCanPlayCard(card, currentMana) && this._getCardIsBurn(card, opponentGeneral)) {
						_.each(CardIntent.getIntentsByIntentTypeWithPartialTargetType(card.getBaseCardId(), CardIntentType.Burn, CardTargetType.General, true), function (intentObj) {
							// add spell damage
							var damage = intentObj.amount || 0;
							totalDamage += damage;
	...
	_getBountyForDistanceFromOpponentGeneral: function () {
	...
	_findPlayCardActionsByType: function (cardType) {
		//Logger.module("AI").debug("[G:" + this.getGameSession().gameId + "] _findPlayCardActionsByType() -> cardType", cardType);
		var myPlayer = this.getMyPlayer();
		var cardsInHand = this._getCardsInHandAndSignatureSpell();

		// find all playable cards
		var numBuffingArtifacts = 0;
		var playableCards = _.filter(cardsInHand, function (card) {
		...
		if (playableCards.length > 0) {
			// sort cards by score
			playableCards = _.sortBy(playableCards, function (card) { return ScoreForCard(card); }.bind(this)).reverse();

			// try to play each card until one is played
			var hasFullHand = myPlayer.getDeck().getNumCardsInHand() >= CONFIG.MAX_HAND_SIZE;
			for (var i = 0, il = playableCards.length; i < il; i++) {
	...
	_getIsCardNotBuffOrAllowableBuff: function (card) {
		if (card != null) {
			var cardId = card.getBaseCardId();
	...
	_getCanUseSignatureCard: function () {
		return this._difficulty >= 0.5;
	},

	_getCardsInHandAndSignatureSpell: function () {
	...
	_getCanPlayCard: function (card, currentMana) {
		return card != null
			&& (
	...
	_findPlayCardActionsByIntent: function (oldIntentString) {
	...
	_findBestPlayCardAction: function (useThreshold) {
		...
		var playableCards = _.filter(cardsInHand, function (card) {
			if (this._getCanPlayCard(card) && !this._getCannotPlayCardDueToDifficulty(card)) {
				if (card instanceof SDK.Unit) {
					...
					var intents = CardIntent.getIntentsByCardId(card.getBaseCardId());
		...
		if (playableCards.length > 0) {
			...
			var cardWithBestScore;
			var bestTargetPositionAndScore;
			var bestFollowupPositions;
			for (var i = 0, il = playableCards.length; i < il; i++) {
				var card = playableCards[i];
				var validTargetPositions = card.getValidTargetPositions();
				var filteredValidTargetPositionsScoresAndFollowups = this._findFilteredTargetPositionsScoresAndFollowupsForCardByIntent(card, validTargetPositions, useThreshold);
				if (filteredValidTargetPositionsScoresAndFollowups.positionsAndScores.length > 0) {
					var nextTargetPositionAndScore = filteredValidTargetPositionsScoresAndFollowups.positionsAndScores[0];
			...
			if (cardWithBestScore != null) {
				var player = cardWithBestScore.getOwner();
				var position = bestTargetPositionAndScore.position;
				...
				var cardIsFollowup = cardWithBestScore.getIsFollowup();
				var cardIsSignatureSpell = cardWithBestScore.isSignatureCard();
				if (cardIsFollowup) {
	...
	_findFilteredTargetPositionsScoresAndFollowupsForCardByIntent: function (card, validTargetPositions, useThreshold, withoutCache){
		var gameSession = this.getGameSession();
		var board = gameSession.getBoard();
		var cardId = card.getBaseCardId();
		var cacheForCard = this._cacheByCard[cardId];
		...
		var filteredPositionsScoresAndFollowups = cacheForCard.filteredPositionsScoresAndFollowups[useThreshold];
		if (filteredPositionsScoresAndFollowups == null) {
			...
			if (cacheForCard.anywherePosition != null) {
				...
				var validTargetPositionsCopy = validTargetPositions.slice(0);
				while(validTargetPositionsCopy.length > 0) {
					var validPosIndex = Math.floor(Math.random() * validTargetPositionsCopy.length);
					var validPos = validTargetPositionsCopy[validPosIndex]
				...
				validTargetPositions = _.reject(validTargetPositions, function (validPos) {
					// reject positions that:
					// - contain immune units
					// - do not ensure area of effect is fully on board
					return this._isTargetImmuneToSource(board.getUnitAtPosition(validPos), card) || !card.isAreaOfEffectOnBoard(validPos);
				}.bind(this));
			}
			...
			if (validTargetPositions.length > 0) {
				...
				var cardScore = cacheForCard.scoreForCard;
				if (cardScore == null) {
					cardScore = cacheForCard.scoreForCard = ScoreForCard(card);
				}
				var scoreAndFollowupPositionsCache = cacheForCard.scoreAndFollowupPositions;
				var columnCount = board.getColumnCount();
				_.each(validTargetPositions, function (validTargetPosition) {
					var scoreIndex = UtilsPosition.getMapIndexFromPosition(columnCount, validTargetPosition.x, validTargetPosition.y);
					var scoreAndFollowupPositions = scoreAndFollowupPositionsCache[scoreIndex];
					var score;
					var followupPositions;
					...
					if (scoreAndFollowupPositions != null) {
						...
						// score followups
						var followupScoreAndFollowupPositions = ScoreForIntentFollowup(card, validTargetPosition);
						...
						scoreAndFollowupPositionsCache[scoreIndex] = {
							score: score,
							followupPositions: followupPositions
						};
					}

					if (score >= useThreshold) {
						// sorted insert of target position based on score
						var positionAndScore = {position: validTargetPosition, score: score};
						var insertIndex = UtilsJavascript.arraySortedInsertByProperty(filteredPositionsScoresAndFollowups.positionsAndScores, positionAndScore, "score");
	...
	_getCannotPlayCardDueToDifficulty: function (card) {
		if (this._difficulty < 1.0) {
			var player = card.getOwner();
			var cardIsSignatureSpell = card.isSignatureCard();
			...
			var hasFullHand = player.getDeck().getNumCardsInHand() >= CONFIG.MAX_HAND_SIZE;
			if (!hasFullHand) {
				...
				if (!card.getIsFollowup() && this._getCardIsSpawn(card)) {
					if (this._numSpawnedUnitsThisTurn >= Math.max(1, Math.floor(this._difficulty * DIFFICULTY_MAX_SPAWNS_PER_TURN))) {
						...
						var numFriendlyUnits = this.getGameSession().getBoard().getFriendlyEntitiesForEntity(this.getMyGeneral()).length;
	...
	_getCardIsBurn: function (card, target) {
		var cardId = card.getBaseCardId();
	...
	_getCardIsAtkBuffFor: function (card, unit) {
		var cardId = card.getBaseCardId();
		var isBuff = false;
		var intents;
		...
		for (var i = 0, il = intents.length; i < il; i++) {
			var intentObj = intents[i];
	...
	_getCardIsAtkDebuffFor: function (card, unit) {
		var cardId = card.getBaseCardId();
		var isDebuff = false;
		var intents;
		if (unit.getIsGeneral()) {
	...
	_getCardIsRemoval: function (card) {
		var cardId = card.getBaseCardId();
	...
	_findPlayCardActionsForCard: function (card, targetPosition) {
		if (this._getCanPlayCard(card) && !this._getCannotPlayCardDueToDifficulty(card)) {
			var cardId = card.getBaseCardId();
			var cardIsFollowup = card.getIsFollowup();
			var cardIsSignatureSpell = card.isSignatureCard();
			var player = card.getOwner();
			//Logger.module("AI").debug("[G:" + this.getGameSession().gameId + "] _findPlayCardActionsForCard ->" + card.getLogName());
			var validTargetPositions;
			if (targetPosition != null) {
				...
				if (validTargetPositions.length > 0) {
					var needsOldBehavior = true;
					var intents = CardIntent.getIntentsByCardId(cardId);
					var hasIntents = intents != null && intents.length > 0;
					if(hasIntents || card instanceof SDK.Unit) {
						...
						var filteredValidTargetPositionsScoresAndFollowups = this._findFilteredTargetPositionsScoresAndFollowupsForCardByIntent(card, validTargetPositions, useThreshold);
						if(filteredValidTargetPositionsScoresAndFollowups.positionsAndScores.length > 0){
							var bestPositionsAndScore = filteredValidTargetPositionsScoresAndFollowups.positionsAndScores[0];
							var followupPositions = filteredValidTargetPositionsScoresAndFollowups.followupPositions[0];
					...
					if (needsOldBehavior) {
						var filteredValidTargetPositions = this._findFilteredTargetPositionsForCardByOldIntent(card, validTargetPositions);
			...
			if (validTargetPositions.length > 0) {
				var validTargetPosition = validTargetPositions[0];
				if (validTargetPosition != null) {
					...
					if (cardIsFollowup) {
						...
						playCardAction = player.actionPlayCardFromHand(this.getGameSession().getCurrentPlayer().getDeck().getCardsInHand().indexOf(card), validTargetPosition.x, validTargetPosition.y);
	...
	_findFilteredTargetPositionsForCardByOldIntent: function (card, validTargetPositions) {
		var cardId = card.getBaseCardId();
		...
		if (CARD_INTENT[cardId] != null) {
			...
			if (card instanceof SDK.Spell) {
				validTargetPositions = _.reject(validTargetPositions, function (validPos) {
					//reject immune units
					return this._isTargetImmuneToSource(this.getGameSession().getBoard().getUnitAtPosition(validPos), card);
				}.bind(this));
			}
			...
			switch (CARD_INTENT[cardId]) {
				...
				case "hand_improve_minion_mass_bbs":
					...
					var minionsInHand = _.filter(cardsInHand, function (card) {
						return card instanceof SDK.Unit;
					}.bind(this));
				...
				case "stun":
					...
					validTargetPositions = _.reject(validTargetPositions, function (validPos) {
						...
						var unitAtPosition = this.getGameSession().getBoard().getUnitAtPosition(validPos);
					...
					validTargetPositions = _.filter(validTargetPositions, function (validPos) {
						var unitAtPos = this.getGameSession().getBoard().getUnitAtPosition(validPos);
					...
					if (validTargetPositions.length > 0) {
						validTargetPositions = _.sortBy(validTargetPositions, function (validPos) {
							return ScoreForUnit(this.getGameSession().getBoard().getUnitAtPosition(validPos));
						}.bind(this)).reverse();
					}
					break;
				...
				case "burn_dispel":
					...
					if (validTargetPositions.length > 0) {
						validTargetPositions = _.sortBy(validTargetPositions, function (validPos) {
							return ScoreForUnitDamage(this.getGameSession().getBoard().getUnitAtPosition(validPos), card.damageAmount);
						}.bind(this)).reverse();
					}
					break;
				case "burn_move_enemy_minion":
					...
					if (!this._hasLethalOnEnemyGeneral) {
						...
						validTargetPositions = _.filter(validTargetPositions, function (validPos) {
							var potentialTarget = this.getGameSession().getBoard().getUnitAtPosition(validPos);
					...
					if (validTargetPositions.length > 0) {
						...
						var generalLethal = _.find(validTargetPositions, function (validPos) {
							var unitAtPos = this.getGameSession().getBoard().getUnitAtPosition(validPos);
						...
						validTargetPositions = _.reject(validTargetPositions_copy, function (validPos) {
							var enemy = this.getGameSession().getBoard().getUnitAtPosition(validPos);
				...
				case "removal_buff_minion":
					...
					//reject friendly units
					validTargetPositions = _.reject(validTargetPositions, function (validPos) {
						return this.getGameSession().getBoard().getUnitAtPosition(validPos).getIsSameTeamAs(myGeneral);
					}.bind(this));
					validTargetPositions = _.sortBy(validTargetPositions, function (validPos) {
						return ScoreForUnit(this.getGameSession().getBoard().getUnitAtPosition(validPos));
					}.bind(this)).reverse();
					...
					if (cardId == SDK.Cards.Spell.AspectOfTheWolf) //aspect of the fox replace with 3/3
						validTargetPositions = _.reject(validTargetPositions, function (validPos) {
							return ScoreForUnit(this.getGameSession().getBoard().getUnitAtPosition(validPos)) < 14;
						}.bind(this));
					else if (cardId == SDK.Cards.Spell.Martyrdom) //heal enemy general by unit's hp
						validTargetPositions = _.reject(validTargetPositions, function (validPos) {
							return ScoreForUnit(this.getGameSession().getBoard().getUnitAtPosition(validPos)) < 12;
						}.bind(this));
					else
						validTargetPositions = _.reject(validTargetPositions, function (validPos) { return (ScoreForUnit(this.getGameSession().getBoard().getUnitAtPosition(validPos)) < 10); }.bind(this));
					break;
				...
				case "shadownova":
					...
					var shadowTiles = _.filter(this.getGameSession().getBoard().getTiles(true), function (tile) { return tile.hasModifierClass(SDK.ModifierStackingShadows); }.bind(this));
					...
					enemies = _.reject(enemies, function (enemy) { return _.intersection([enemy.getPosition()], shadowTiles) > 0; }.bind(this));
					//reject immune units
					enemies = _.reject(enemies, function (enemy) {
						return this._isTargetImmuneToSource(enemy, card);
					}.bind(this));
					...
					if (_.some(enemies, function (enemy) { return enemy.getHP() <= shadownovaDamage; }.bind(this))) {
						...
						var enemiesWithinLethalRange = _.filter(enemies, function (enemy) { return enemy.getHP() <= shadownovaDamage; }.bind(this));
						var primaryTarget = _.max(enemiesWithinLethalRange, function (enemy) { return ScoreForUnitDamage(enemy, shadownovaDamage); }.bind(this));
						...
						var secondaryTargetsArray = this.getGameSession().getBoard().getFriendlyEntitiesAroundEntity(primaryTarget, SDK.CardType.Unit);
						//reject enemies already standing on shadow creep...?
						secondaryTargetsArray = _.reject(secondaryTargetsArray, function (enemy) { return _.intersection([enemy.getPosition()], shadowTiles) > 0; }.bind(this));
						if (secondaryTargetsArray.length == 0 && ScoreForUnitDamage(primaryTarget, shadownovaDamage) < 99) {
							...
							if (ScoreForUnitDamage(primaryTarget, shadownovaDamage) >= 99) {
								...
								secondaryTarget = _.max(secondaryTargetsArray, function (secondaryTarget) { return ScoreForUnitDamage(secondaryTarget, shadownovaDamage); }.bind(this));
							}
							...
							var unitsAroundPrimaryTarget = this.getGameSession().getBoard().getFriendlyEntitiesAroundEntity(primaryTarget, SDK.CardType.Unit);
							//exclude secondary
							unitsAroundPrimaryTarget = _.reject(unitsAroundPrimaryTarget, function(target) { return target == secondaryTarget});
							var unitsAroundSecondaryTarget = this.getGameSession().getBoard().getFriendlyEntitiesAroundEntity(secondaryTarget, SDK.CardType.Unit);
							//exclude primary
							unitsAroundSecondaryTarget = _.reject(unitsAroundSecondaryTarget, function(target) { return target == primaryTarget});
							//find new units who are adjacent to both our primary and secondary targets by intersecting the adjacent arrays
							var auxiliaryTargetsArray = _.intersection(unitsAroundPrimaryTarget, unitsAroundSecondaryTarget);
							//reject enemies already standing on shadow creep...?
							auxiliaryTargetsArray = _.reject(auxiliaryTargetsArray, function (enemy) { return _.intersection([enemy.getPosition()], shadowTiles) > 0; }.bind(this));
							if (auxiliaryTargetsArray.length > 0) {
								var auxiliaryTarget = _.max(auxiliaryTargetsArray, function (auxTarget) { return ScoreForUnitDamage(auxTarget, shadownovaDamage); }.bind(this));
							...
							var affectPattern = card.getAffectPattern();
							var positionsToMatch = [primaryTarget.getPosition()];
							...
							var positionThatAffectsAllTargets = _.find(validTargetPositions, function (pos) {
								//find position that contains all targets
								var bx = pos.x;
								var by = pos.y;
								var numMatches = 0;
								for (var a = 0, al = affectPattern.length; a < al; a++) {
									var offset = affectPattern[a];
				...
				case "burn_mass_enemy":
					...
					var damageAmount = 0;
					if (CARD_INTENT[cardId].indexOf("burn") > -1)
					...
					var myGeneral = this.getMyGeneral();
					_.each(this.getGameSession().getBoard().getUnits(), function (unit) {
				...
				case "burn_mass_column":
					...
					validTargetPositions = [];
					var damageAmount = card.damageAmount;
					var enemiesKilled = 0;
					var score = 0;
					var myGeneral = this.getMyGeneral();
					...
					for (i = 0; i < columnCount; i++) {
						...
						enemiesKilled = 0;
						score = 0;
						var unitsInColumn = this.getGameSession().getBoard().getEntitiesInColumn(i, SDK.CardType.Unit);
						...
						if (unitsInColumn.length > 0) {
							_.each(unitsInColumn, function (unit) {
								if (this._isTargetImmuneToSource(unit, card)) return; //don't count immune units
								//if (CARD_INTENT[cardId].indexOf("minion") > -1 && unit.getIsGeneral()) return; //don't count generals if spell effects only minions (i.e. burn_mass_minion)
								if (unit.getIsSameTeamAs(myGeneral) && CARD_INTENT[cardId].indexOf("enemy") == -1) {
				...
				case "summon_move_enemy_minion":
					...
					validTargetPositions = [];
					_.each(original_validTargetPositions, function (pos) {
						unitsAroundPos = this.getGameSession().getBoard().getCardsAroundPosition(pos, SDK.CardType.Unit);
						if (unitsAroundPos.length > 0) {
							var myGeneral = this.getMyGeneral();
							enemiesAroundPos = _.reject(unitsAroundPos, function (unit) { return unit.getIsSameTeamAs(myGeneral); }.bind(this));
					...
					//can't move generals
					potentialEnemiesToMove = _.reject(potentialEnemiesToMove, function (enemy) { return enemy.getIsGeneral() }.bind(this));
					//reject immune units
					potentialEnemiesToMove = _.reject(potentialEnemiesToMove, function (enemy) {
						return this._isTargetImmuneToSource(enemy, card);
					}.bind(this));
					if (potentialEnemiesToMove.length > 0) {
						//then reject if atk < THRESHOLD.HIGH_ATK && not provoke (this means atk > 4 and provokers pass)
						potentialEnemiesToMove = _.reject(potentialEnemiesToMove, function (enemy) { return (enemy.getATK() < THRESHOLD.HIGH_ATK && !enemy.hasModifierClass(SDK.ModifierProvoke)); }.bind(this));
						if (potentialEnemiesToMove.length > 0) {
							//sort by bounty
							potentialEnemiesToMove = _.sortBy(potentialEnemiesToMove, function (enemy) { return ScoreForUnit(enemy); }.bind(this)).reverse();
							//take top enemy on list
							var targetEnemyToMove = potentialEnemiesToMove[0];
							//find available spawn location adjacent to it
							var validTargetPositionsAdjacentToTargetEnemyToMove = _.filter(original_validTargetPositions, function (pos) { return arePositionsEqualOrAdjacent(pos, targetEnemyToMove.position); }.bind(this));
				...
				case "summon_ranged_bbs":
					...
					break;
				case "refresh":
					validTargetPositions = _.filter(validTargetPositions, function (validPos) { return this.getGameSession().getBoard().getUnitAtPosition(validPos).attacksMade > 0; }.bind(this));
					break;
				...
				case "heal_followup":
					...
					var mostDamagedUnit;
					_.each(validTargetPositions, function (pos) {
						var unit = board.getUnitAtPosition(pos);
	...
	_findAtkBuffAndBuffedAtkValue: function (unit, targets, returnBuffedAtkValueOnly_noCast) {
		//function returns atk buff amount for unit if a buff is loaded into actions
		var baseAtk = unit.getATK();
		var buffedAtk = baseAtk;
		var myPlayer = this.getMyPlayer();
		...
		if (this._nextActions.length === 0 && targets.length > 0) {
			...
			var cardsInHand = this._getCardsInHandAndSignatureSpell();
			var playableBuffs = _.filter(cardsInHand, function (card) {
				return this._getCanPlayCard(card) && this._getCardIsAtkBuffFor(card, unit)
			}.bind(this));

			// if we have playable buffs, check for any enemy targets with more hp than unit atk
			if (playableBuffs.length > 0 && _.some(targets, function (enemy) { return enemy.getHP() > baseAtk; }.bind(this))) {
				...
				if (playableBuffs.length > 0 && unit.hasModifierClass(SDK.ModifierEphemeral)) {
					playableBuffs = _.filter(playableBuffs, function (buffCard) {
						var buffCardId = buffCard.getBaseCardId();
				...
				_.find(playableBuffs, function (buffCard) {
					var buffCardId = buffCard.getBaseCardId();
					var atkBuff = 0;
					...
					var hasIntentTypeModifyAtk = CardIntent.getHasIntentType(buffCardId, CardIntentType.ModifyATK, true);
					if (hasIntentTypeModifyAtk) {
						_.each(CardIntent.getIntentsByIntentType(buffCardId, CardIntentType.ModifyATK, true), function (intentObj) {
						...
						var modifiersContextObjects = _.filter(buffCard.targetModifiersContextObjects, function (modifierContextObject) {
							return modifierContextObject.attributeBuffs != null && modifierContextObject.attributeBuffs.atk != null;
						}.bind(this));
						if (modifiersContextObjects.length > 0) {
							atkBuff = _.max(modifiersContextObjects, function (modifierContextObject) {
								return modifierContextObject.attributeBuffs.atk;
							}.bind(this)).attributeBuffs.atk;
						}
					}

					// check if atk buff will result in lethal on a target that we didn't have lethal on
					var hasTargetLethalWithBuff = false;
					if (buffCardId == SDK.Cards.Spell.PsionicStrike
						...
						hasTargetLethalWithBuff = _.some(targets, function (enemy) {
						...
						if (returnBuffedAtkValueOnly_noCast) {
							...
							if (hasIntentTypeModifyAtk !== CardIntent.getHasIntentType(buffCardId, CardIntentType.ModifyATK)) {
								...
								if (buffCard instanceof SDK.Unit) {
									...
									var validSpawnLocations = this.getGameSession().getBoard().getUnobstructedPositionsForEntityAroundEntity(buffCard, unit);
									...
									if (validSpawnLocations.length > 0) {
										var initialPosition = this._highestOrLowestPositionObjectiveAndScoreForUnitFromPositions(buffCard, validSpawnLocations).targetPosition;
	...
	_findAtkDebuffForUnitAndCastOnTarget: function (unit, target) {
		var myPlayer = this.getMyPlayer();
		//if there's an action already loaded, assume it's a buff or debuff and don't bother.
		if (this._nextActions.length > 0) return;

		var unitHP = unit.getHP();
		var isBlast = unit.hasModifierClass(SDK.ModifierBlastAttack);
		...
		if (isBlast && distanceBetweenBoardPositions(unit.getPosition(), target.getPosition()) > 1) {
			var enemiesInAttack = this.getGameSession().getBoard().getEnemyEntitiesOnCardinalAxisFromEntityToPosition(unit, target.getPosition(), SDK.CardType.Unit, false, false);
			var swapEnemy = _.find(enemiesInAttack, function (enemy) {
				return enemy.getATK() >= unitHP && (distanceBetweenBoardPositions(unit.getPosition(), enemy.getPosition()) <= 1 || enemy.hasModifierClass(SDK.ModifierRanged));
			}.bind(this));
			if (swapEnemy != null) {
				target = swapEnemy;
			}
		...
		var cardsInHand = this._getCardsInHandAndSignatureSpell();
		var debuffsInHand = _.filter(cardsInHand, function (card) {
			return this._getCanPlayCard(card) && this._getCardIsAtkDebuffFor(card, unit)
		}.bind(this));
		if (typeof debuffsInHand != 'undefined' && debuffsInHand.length > 0) {
			...
			_.each(debuffsInHand, function (debuffCard) {
				if (this._nextActions.length > 0 || debuffCard == null) return;

				var debuffCardId = debuffCard.getBaseCardId();
				...
				var hasIntentTypeModifyAtk = CardIntent.getHasIntentType(debuffCardId, CardIntentType.ModifyATK, true);
				if (hasIntentTypeModifyAtk !== CardIntent.getHasIntentType(debuffCardId, CardIntentType.ModifyATK)) {
					...
					if (debuffCard instanceof SDK.Unit) {
						// determine if adjacent space available and set followup target & target
						var validSpawnLocations = this.getGameSession().getBoard().getUnobstructedPositionsForEntityAroundEntity(debuffCard, target);
						var validSummonLocations = this.getGameSession().getBoard().getValidSpawnPositions(debuffCard);
						var actualSummonLocations = [];

						// need to make sure the valid spawn location is actually a location that we can summon
						for(var i = 0; i < validSpawnLocations.length; i++){
							for(var j = 0; j < validSummonLocations.length; j++){
						...
						if (actualSummonLocations.length > 0) {
							var initialPosition = this._highestOrLowestPositionObjectiveAndScoreForUnitFromPositions(debuffCard, actualSummonLocations).targetPosition;
	...
	_isUnitPreventedFromAttacking: function (unit) {
		return unit.hasModifierClass(SDK.ModifierStunned);
	},

	_isUnitPreventedFromAttackingTarget: function (unit, target) {
		return _.find(this._invalidAttackActions, function (invalidAttackAction) {
				return invalidAttackAction.getSource() === unit
					&& invalidAttackAction.getTarget() === target;
			}) != null;
	},

	_isTargetImmuneToSource: function (target, source) {
		if (target instanceof SDK.Card && source instanceof SDK.Card) {
			var immuneModifiers = target.getActiveModifiersByClass(SDK.ModifierImmune);
			if (immuneModifiers.length > 0) {
				var immuneModifier = _.find(immuneModifiers, function (modifier) {
					// immune to attack
	...
	_isUnitIgnorableForAttacks: function (unit) {
	...
	_findAttackActionsForUnit: function (unit, allowOnlyLethalAttacks) {
		if (unit.getCanAttack() && !_.contains(this._unitsMissingAttacks, unit) && !this._isUnitPreventedFromAttacking(unit)) {
		  var unitPosition = unit.getPosition();
			var returnBuffedAtkValueOnly_noCast = false; //allow buff casts for attack actions
			var attackObjectivesAndScores = this._findSortedFilteredAttackObjectivesAndScoresForUnit(unit, unitPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast);
			...
			if (attackObjectivesAndScores.length > 0) {
				var attackTarget = true;
				var attackObjective = attackObjectivesAndScores[0].objective;
				var attackObjectiveIndex = 1;
				var unitInRangeToKillEgg = false;
				var attackObjectivePosition;
				...
				if (attackTarget && attackObjective.hasActiveModifierClass(SDK.ModifierForcefieldAbsorb)) {
					var successfullyPoppedFF = this._forceAttackOnTargetIfPossible(attackObjective, unit);
				...
				if(attackTarget){
					...
					var attackAction = unit.actionAttack(attackObjective);
	...
	_isUnitNearbyThatCanAttackTarget: function (attackTarget, ignoreUnit) {
		...
		var myUsableUnits = this._getMyUsableUnits().reverse();
		var bestMovePosition = null;

		var attackerUnit = _.find(myUsableUnits, function (unit) {
			if(unit !== ignoreUnit
				...
				var nearTarget = distanceBetweenBoardPositions(attackTarget.getPosition(), unit.getPosition()) <= 1;
	...
	_forceAttackOnTargetIfPossible: function (forcedTarget, ignoreUnit) {
		...
		var attackerUnit = _.find(myUsableUnits, function (unit) {
			if(unit !== ignoreUnit
				...
				// check to see if the unit is already next to the unit its trying to attack
				var nearTarget = distanceBetweenBoardPositions(forcedTarget.getPosition(), unit.getPosition()) <= 1;
				...
				if (!nearTarget && unit.getCanMove() && !this._isUnitPreventedFromMoving(unit)) {
					...
					if (bestMovePosition != null) {
						...
						var moveAction = unit.actionMove(bestMovePosition);
		...
		if(attackerUnit != null){
			var attackAction = attackerUnit.actionAttack(forcedTarget);
			if (bestMovePosition != null) {
	...
	_bestAttackPositionForUnitWithObjective: function (unitToSend, objective) {
		// find the highest scoring position for the a unit wanting to attack another unit
		var bestPosition = null;
		var board = this.getGameSession().getBoard();
		var potentialMoves = this._findPotentialMovePositionsForUnit(unitToSend); //get potential move spaces
		if (potentialMoves.length > 0) {
			var locationsNearTarget = board.getUnobstructedPositionsForEntityAroundEntity(unitToSend, objective); //get spaces near the objective
			if (locationsNearTarget.length > 0) {
				...
				if (!objective.hasActiveModifierClass(SDK.ModifierProvoke)) {
					var objectiveFriends = board.getFriendlyEntitiesForEntity(objective, SDK.CardType.Unit);
					for (var i = 0, il = objectiveFriends.length; i < il; i++) {
						var objectiveFriend = objectiveFriends[i];
						var objectiveFriendPos = objectiveFriend.getPosition();
						if (objectiveFriend.hasActiveModifierClass(SDK.ModifierProvoke)) {
							for (var j = locationsNearTarget.length - 1; j >= 0; j--) {
				...
				for (var i = 0, il = potentialMoves.length; i < il; i++) {
					var potentialMovePos = potentialMoves[i];
					var potentialX = potentialMovePos.x;
					var potentialY = potentialMovePos.y;
					for (var k = 0, kl = locationsNearTarget.length; k < kl; k++) {
						...
						if (potentialX === locationNearTarget.x && potentialY === locationNearTarget.y) {
							...
							if (bestPosition == null) {
								...
								var scoreForNewPosition = ScoreForCardAtTargetPosition(unitToSend, potentialMovePos, objective);
								var scoreForCurrentBestPosition = ScoreForCardAtTargetPosition(unitToSend, bestPosition, objective);
	...
	_findSortedFilteredAttackObjectivesAndScoresForUnit: function (unit, targetPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast) {
		var sourceUnit = unit.getIsPlayed() ? unit : this.getGameSession().getGeneralForPlayerId(unit.getOwnerId());
		// get potential attack targets at target position
		var potentialAttackTargets = sourceUnit.getAttackRange().getValidTargets(this.getGameSession().getBoard(), sourceUnit, targetPosition);
		var filteredPotentialAttackTargets = this._filterOutBadTargets(potentialAttackTargets, sourceUnit);
	...
	_scoreAndSortAttackTargetsForUnit: function (unit, attackTargets, targetPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast) {
		//sort and filter targets
		var filteredAttackTargets = this._findFilteredAttackTargetsForUnit(unit, attackTargets);
		...
		if (allowOnlyLethalAttacks && !this._hasLethalOnEnemyGeneral) {
			...
			filteredAttackTargets = _.reject(filteredAttackTargets, function (enemy) {
				return enemy.getHP() > unitATK;
			});
		...
		var filteredAttackTargetsAndScores = [];
		_.each(filteredAttackTargets, function (enemy) {
	...
	_scoreAttackTargetForUnit: function (unit, unitATK, enemy, targetPosition) { //todo: remove. redudnant with scoreforunitdamage
		var score = 0;
		...
		if (unit.getIsPlayed()) {
			...
			if (unit.hasModifierClass(SDK.ModifierBlastAttack)) {
				score += _.reject(this.getGameSession().getBoard().getEntitiesInRow(enemy.getPosition().y, SDK.CardType.Unit), function (entity) { return entity.getIsSameTeamAs(unit); }.bind(this)).length * BOUNTY.TARGET_ENEMIES_IN_SAME_ROW;
			}
		}
	...
	_findFilteredAttackTargetsForUnit: function (unit, potentialTargets) {
	  ...
	  potentialTargets = this._findFilteredAttackTargetsForRangedProvokedUnit(unit, potentialTargets);
		potentialTargets = _.reject(potentialTargets, function (target) { return this._isTargetImmuneToSource(target, unit); }.bind(this));
		return potentialTargets;
	},

	_findFilteredAttackTargetsForProvokedUnit: function(unit, potentialTargets) {
		if (unit.getIsProvoked()) { //may only attack provoker(s)
			potentialTargets = _.filter(potentialTargets, function (target) {
	...
	_findFilteredAttackTargetsForRangedProvokedUnit: function (unit, potentialTargets) {
	...
	_findFilteredAttackTargetsForGeneralUnit: function (unit, potentialTargets) {
		if (unit.getIsGeneral()) {
			potentialTargets = _.filter(potentialTargets, function (enemy) {
				return this._isUnitSafeAttackTargetForGeneralUnit(unit, enemy);
			}.bind(this));
		}
		return potentialTargets;
	},

	_isUnitSafeAttackTargetForGeneralUnit: function (unit, enemy) {
		var safe = true;
	...
	_isUnitPreventedFromMoving: function (unit) {
		return unit.hasModifierClass(SDK.ModifierStunned) || unit.getIsProvoked();
	},

	_isUnitPreventedFromMovingTo: function (unit, targetPosition) {
		return _.find(this._invalidMoveActions, function (invalidMoveAction) {
			return invalidMoveAction.getSource() === unit
				&& invalidMoveAction.getTargetPosition().x === targetPosition.x
				&& invalidMoveAction.getTargetPosition().y === targetPosition.y;
		}) != null;
	},

	_findMoveActionsForUnit: function (unit, allowOnlyLethalAttacks) {
		if (unit.getCanMove() && !this._isUnitPreventedFromMoving(unit)) {
			//get potential move spaces
			var potentialMoves = this._findPotentialMovePositionsForUnit(unit);
			if (potentialMoves.length > 0) {
				var unitPosition = unit.getPosition();
				var returnBuffedAtkValueOnly_noCast = true; //do not cast buffs during movement+attack lethal calculations. Buffs will cast during attack actions.
				var currentScoreAndObjective = this._positionObjectiveAndScoreForUnitFromSourceToTargetPosition(unit, unitPosition, unitPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast);
				var bestMoveScoreAndObjective = this._highestOrLowestPositionObjectiveAndScoreForUnitFromPositions(unit, potentialMoves, allowOnlyLethalAttacks);
				...
				if (currentScoreAndObjective.score < bestMoveScoreAndObjective.score
					...
					var moveAction = unit.actionMove(bestMoveScoreAndObjective.targetPosition);
					this._nextActions.push(moveAction);
				}
			}
		}
	...
	_findPotentialMovePositionsForUnit: function (unit) {
		var potentialMoves = [];
		var traversalPath = unit.getMovementRange().getValidPositions(this.getGameSession().getBoard(), unit);
		_.each(traversalPath, function (validMoveNodes) {
			var movePosition = _.last(validMoveNodes);
	...
	_isOnMySideOfBoard: function (unitOrSpell, position) {
		var mySideStartX = 0;
		var mySideEndX = CONFIG.BOARDCOL;
	...
	_highestOrLowestPositionObjectiveAndScoreForUnitFromPositions: function (sourceUnit, positions, allowOnlyLethalAttacks) {
		var sourcePosition = sourceUnit.getIsPlayed() ? sourceUnit.getPosition() : this.getGameSession().getGeneralForPlayerId(sourceUnit.getOwnerId()).getPosition();
		var myGeneral = this.getMyGeneral();
		var bestScoreAndObjective;

		// get scores and best objectives for each position
		var returnBuffedAtkValueOnly_noCast = true; //don't cast buffs. they will cast during attacks
		var scoresAndObjectives = _.map(positions, function (currentPotentialLocation) {
			return this._positionObjectiveAndScoreForUnitFromSourceToTargetPosition(sourceUnit, sourcePosition, currentPotentialLocation, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast);
		}.bind(this));
		...
		if (sourceUnit.getIsSameTeamAs(myGeneral)) {
			bestScoreAndObjective = _.max(scoresAndObjectives, function (scoreAndObjective) {
				return scoreAndObjective.score;
			}.bind(this));
		} else {
			bestScoreAndObjective = _.min(scoresAndObjectives, function (scoreAndObjective) {
				return scoreAndObjective.score;
			}.bind(this));
		}
	...
	_lowestPositionObjectiveAndScoreFromPositions: function (card, positions) {
		// get scores and best objectives for each position
		var allowOnlyLethalAttacks = false;
		var returnBuffedAtkValueOnly_noCast = true;
		var scoresAndObjectives = _.map(positions, function (position) {
			var unitAtPosition = card instanceof SDK.Entity ? card : this.getGameSession().getBoard().getUnitAtPosition(position);
		...
		return _.min(scoresAndObjectives, function (scoreAndObjective) {
			return scoreAndObjective.score;
		}.bind(this));
	},

	_highestPositionObjectiveAndScoreFromPositions: function (card, positions) {
		// get scores and best objectives for each position
		var allowOnlyLethalAttacks = false;
		var returnBuffedAtkValueOnly_noCast = true;
		var scoresAndObjectives = _.map(positions, function (position) {
		...
		return _.max(scoresAndObjectives, function (scoreAndObjective) {
			return scoreAndObjective.score;
		}.bind(this));
	},
	...
	_getIsScoredModifier: function (modifier) {
		return !(
	...
	_filterOutBadTargets: function (targets, unit) {
		var board = this.getGameSession().getBoard();
		var ignoreWraithlings = false;
		var filteredTargets = _.reject(targets, function (target) {
			...
			if (target.getBaseCardId() === SDK.Cards.Faction4.Wraithling) {
				if(ignoreWraithlings == false){
					var enemyUnits = board.getFriendlyEntitiesForEntity(target);
			...
			var isObjectiveIgnorable = this._isUnitIgnorableForAttacks(target);
	...
	_positionObjectiveAndScoreForUnitFromSourceToTargetPosition: function (card, sourcePosition, targetPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast) {
		//--PARAMETERS--
		...
		var attackObjectivesAndScores = this._findSortedFilteredAttackObjectivesAndScoresForUnit(unitToSend, targetPosition, allowOnlyLethalAttacks, returnBuffedAtkValueOnly_noCast);

		var objective;
		var score = 0;
		if (attackObjectivesAndScores.length === 0) {
			//use nearest if nothing in range
			var enemies = this.getGameSession().getBoard().getEnemyEntitiesForEntity(unitToSend, SDK.CardType.Unit);
			var filteredEnemies = this._filterOutBadTargets(enemies, unitToSend);
			...
			var objectiveAndScore = attackObjectivesAndScores[0];
...
var THRESHOLD_HIGH_VALUE_UNIT = 14;
