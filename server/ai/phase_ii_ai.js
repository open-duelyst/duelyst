...
var StarterAI = require('./starter_ai');
var generatePushId = require('app/common/generate_push_id');
var Promise = require("bluebird");
var _ = require("underscore");
var cluster = require('cluster');
var CLUSTER_NUMBEROFWORKERS = require('os').cpus().length;
...
var UsableDecks = require('./decks/usable_decks');
...
if (cluster.isMaster) {
	 ...
	 */
	app.post('/v1_find_next_actions', function (req,res,next) {
		try {
			var difficulty = req.body.difficulty != null ? req.body.difficulty : 1.0;

			// deserialize
			var gameSessionData = JSON.parse(req.body.game_session_data);
			...
			var playerId = req.body.player_id || gameSession.getCurrentPlayerId();
			Logger.module("AI").debug("AI v1 finding next actions for " + playerId);

			var ai = new StarterAI(gameSession, playerId, difficulty);
		  ...
		  // respond with action
			var actionsJSON = gameSession.serializeToJSON(actions);
	 ...
	 * @see ai_findActionSequence
	 */
	app.post('/v2_find_action_sequence', function (req,res,next) {
		try {
			// deserialize
			var gameSessionData = JSON.parse(req.body.game_session_data);
			// force game session game type to single player
			...
			var playerId = req.body.player_id != null ? req.body.player_id : gameSession.getCurrentPlayerId();
			var depthLimit = req.body.depth_limit != null ? req.body.depth_limit : 9999;
			var msTimeLimit = req.body.ms_time_limit != null ? req.body.ms_time_limit : 0;
			...
			ai_findActionSequence(gameSession, playerId, depthLimit, msTimeLimit)
				.then(function (results) {
					...
					var sequenceActionsJSON = gameSession.serializeToJSON(sequence.actions);
	...
	app.post('/start_game', function (req,res,next) {
		try {
			var ai1GeneralId = req.body.ai_1_general_id;
			var ai2GeneralId = req.body.ai_2_general_id;
			var ai1Version = req.body.ai_1_version != null ? req.body.ai_1_version : 2;
			var ai2Version = req.body.ai_2_version != null ? req.body.ai_2_version : 2;
			var depthLimit = req.body.depth_limit != null ? req.body.depth_limit : 9999;
			var msTimeLimit = req.body.ms_time_limit != null ? req.body.ms_time_limit : 0;
			var ai1NumRandomCards = req.body.ai_1_num_random_cards != null ? parseInt(req.body.ai_1_num_random_cards) : 0;
			var ai2NumRandomCards = req.body.ai_2_num_random_cards != null ? parseInt(req.body.ai_2_num_random_cards) : 0;
			var ai1Deck = UsableDecks.getAutomaticUsableDeck(ai1GeneralId, 1.0, ai1NumRandomCards);
			var ai2Deck = UsableDecks.getAutomaticUsableDeck(ai2GeneralId, 1.0, ai2NumRandomCards);
			...
			// setup player data
			var ai1PlayerId = "AI1_v" + ai1Version;
			var ai2PlayerId = "AI2_v" + ai2Version;
			var player1Data = {
				userId: ai1PlayerId,
				name: ai1PlayerId,
				deck: ai1Deck
			...
			var ai2IsPlayer1 = false;
			if (Math.random() > 0.5) {
				ai2IsPlayer1 = true;
				var tempData = player1Data;
			...
			runningGameData = {
				session: gameSession,
				ai2IsPlayer1: ai2IsPlayer1,
				ai1: null,
				ai1PlayerId: ai1PlayerId,
				ai1Version: ai1Version,
				ai2: null,
				ai2PlayerId: ai2PlayerId,
				ai2Version: ai2Version,
				depthLimit: depthLimit,
				msTimeLimit: msTimeLimit
			};
	 ...
	 * @returns {GameSession}
	 */
	app.post('/start_game_from_data', function (req,res,next) {
		try {
			var ai1Version = req.body.ai_1_version != null ? req.body.ai_1_version : 2;
			var ai2Version = req.body.ai_2_version != null ? req.body.ai_2_version : 2;
			var depthLimit = req.body.depth_limit != null ? req.body.depth_limit : 9999;
			...
			// setup game data
			var ai1PlayerId = gameSession.getPlayer1Id();
			var ai2PlayerId = gameSession.getPlayer2Id();
			runningGameData = {
				session: gameSession,
				ai2IsPlayer1: false,
				ai1: null,
	 ...
	 */
	app.post('/stop_game', function (req,res,next) {
		try {
	 ...
	 */
	app.post('/step_game', function (req,res,next) {
		try {
			if (runningGameData != null) {
				var gameSession = runningGameData.session;
				var aiPromises = [];
				if (!gameSession.isOver()) {
					// determine what players need to execute actions
					var playerIdsToExecuteActions;
					...
					for (var i = 0, il = playerIdsToExecuteActions.length; i < il; i++) {
						var currentPlayerId = playerIdsToExecuteActions[i];
						var ai;
						...
						if (ai instanceof StarterAI) {
							aiPromises.push(new Promise(function (resolve, reject) {
								var stepsExecuted = [];
								var aiAction = ai.nextAction();
								if (aiAction != null) {
									gameSession.executeAction(aiAction);
									if (aiAction.getIsValid()) {
										var lastStep = gameSession.getLastStep();
										while (lastStep != null) {
				...
				Promise.all(aiPromises).then(function (results) {
						var stepsExecuted = [];
						for (var i = 0, il = results.length; i < il; i++) {
							stepsExecuted = stepsExecuted.concat(results[i]);
						}
						var stepsJSON = gameSession.serializeToJSON(stepsExecuted);
	 ...
	 * @returns {GameSession}
	 */
	app.post('/run_headless_games', function (req,res,next) {
		try {
			var ai1GeneralId = req.body.ai_1_general_id;
			var ai2GeneralId = req.body.ai_2_general_id;
			var ai1Version = req.body.ai_1_version != null ? req.body.ai_1_version : 2;
			var ai2Version = req.body.ai_2_version != null ? req.body.ai_2_version : 2;
			var numGames = req.body.num_games != null ? req.body.num_games : 10;
			var depthLimit = req.body.depth_limit != null ? req.body.depth_limit : 9999;
			var msTimeLimit = req.body.ms_time_limit != null ? req.body.ms_time_limit : 0;
			var ai1NumRandomCards = req.body.ai_1_num_random_cards != null ? parseInt(req.body.ai_1_num_random_cards) : 0;
			var ai2NumRandomCards = req.body.ai_2_num_random_cards != null ? parseInt(req.body.ai_2_num_random_cards) : 0;
			...
			var handleErrorsAndRoping = function (error) {
				var errorGameState = "";
				if (gameSession != null) {
					var currentPlayerId = gameSession.getCurrentPlayerId();
					...
					// action
					var lastAction = gameSession.getLastStep() && gameSession.getLastStep().getAction();
					...
					// bbs
					var signatureCard = gameSession.getCurrentPlayer().getCurrentSignatureCard();
					errorGameState += "\nbbs: " + ((signatureCard && signatureCard.getName()) || "none");

					if (gameSession.getIsFollowupActive()) {
						var followupParentCard = gameSession.getValidatorFollowup().getCardWaitingForFollowups();
						var followupRootCard = followupParentCard.getRootCard();
					...
					// get hand
					errorGameState += "\n\nhand:";
					var cardsInHand = gameSession.getCurrentPlayer().getDeck().getCardsInHand();
					_.each(cardsInHand, function (cardInHand, i) {
						if (cardInHand == null) {
							errorGameState += "\n[" + i + "]: empty";
						} else {
					...
					// get board
					errorGameState += "\n\nboard:";
					var cardsOnBoard = gameSession.getBoard().getEntities(true);
					_.each(cardsOnBoard, function (cardOnBoard, i) {
						if (cardOnBoard != null) {
			...
			var gameSession;
			var games = _.range(numGames);
			Promise.each(games, function (i) {
				return new Promise(function (gameResolve, gameReject) {
					// timestamp start of game
					var startTime = Date.now();

					// pick random general if none provided
					var ai1GeneralIdForGame;
					...
					var ai1Deck = UsableDecks.getAutomaticUsableDeck(ai1GeneralIdForGame, 1.0);
					var ai2Deck = UsableDecks.getAutomaticUsableDeck(ai2GeneralIdForGame, 1.0);
					...
					var executeMulligan = function () {
						var aiPromises = [];

						if (ai1 instanceof StarterAI) {
							aiPromises.push(new Promise(function (resolve, reject) {
								var stepsExecuted = [];
								var aiAction = ai1.nextAction();
								if (aiAction != null) {
									gameSession.executeAction(aiAction);
									if (aiAction.getIsValid()) {
										var lastStep = gameSession.getLastStep();
					...
					var executeTurnsUntilGameOver = function () {
						...
						if (gameSession.isOver()) {
							...
							// recurse
							return aiPromise.then(function (stepsExecuted) {
					...
					var afterGameOver = function () {
						...
						if (gameStats.winnerId == null) {
							...
							var winnerSetupData = gameSession.getPlayerSetupDataForPlayerId(gameStats.winnerId);
							gameStats.winnerFactionId = winnerSetupData.factionId;
							gameStats.winnerGeneralId = winnerSetupData.generalId;
							var loserSetupData = gameSession.getPlayerSetupDataForPlayerId(gameStats.loserId);
			...
			.then(function () {
				...
				var winsByVersion = {};
				var winsByFactionId = {};
				var numGamesByFactionId = {};
				var winsByGeneralId = {};
				var numGamesByGeneralId = {};
				var winsByVersionAndFactionId = {};
				var winsByVersionAndGeneralId = {};
				var draws = 0;
				var totalDuration = 0;
				var totalNumberOfTurns = 0;
				for (var i = 0, il = gamesStats.length; i < il; i++) {
					var gameStats = gamesStats[i];
					totalDuration += gameStats.duration;
					totalNumberOfTurns += gameStats.numberOfTurns;
					if (gameStats.isDraw) {
						draws++;
					} else {
						var aiNumber = gameStats.winnerId === ai2PlayerId ? 2 : 1;
				...
				// create stats string
				var statsString = "AI Simulator Server run games stats: \n";
				...
				var versions = Object.keys(winsByVersion);
				...
				var factionIds = Object.keys(winsByFactionId);
				...
				var generalIds = Object.keys(winsByGeneralId);
				...
				var versionAndFactionIds = Object.keys(winsByVersionAndFactionId);
				...
				var versionAndGeneralIds = Object.keys(winsByVersionAndGeneralId);
	...
	var ai_cluster_ready = false;
	var ai_cluster_workers_ready = 0;
	...
	var ai_cluster_master_receiveMessageFromWorker = function (msg) {
		var worker = this;
		...
		if (msg.ready) {
			...
			for (var i = 0; i < ai_cluster_workerPool_busy.length; i++) {
				var obj = ai_cluster_workerPool_busy[i];
				if (obj === worker) {
	...
	var ai_cluster_worker_evaluateGameSessionAndSendToMaster = function (gameSession, bestScoresByDepth, depthLimit, msTimeLimit, includedRandomness) {
		...
		var rootNode = generateRootNode(gameSession, depthLimit, msTimeLimit);
    ...
    //also truncates any nodes that ocurred after an action with RNG
		var nodeSequence = buildNodeSequenceFromTreeRoot(rootNode);
		if (nodeSequence.length > 0) {
		  var psuedoActionSequence = _.map(nodeSequence, function (node) {
		    ...
		    return node.parentPsuedoAction;
		  });
		}
    ...
    //it's possible (in a 1 action only scenario) that the action sequence only consists of 1 action (from master).
	  ...
	  //send message to master: ai_cluster_master_receiveMessageFromWorker( )
		process.send({ actions: psuedoActionSequence, score: rootNode.bestLeaf.boardScore, branchesPruned: rootNode.branchesPruned, numSequencesEvaluated: rootNode.numSequencesEvaluated, bestScoresByDepth: rootNode.bestScoresByDepth, gameId: getGameId(rootNode) });
	};
	...
	process.on('message', function (msg) {
		...
		var gameSessionCopy = SDK.GameSession.create();
		...
		.then(function (results) {
			...
			for (var i = 0, il = actions.length; i < il; i++) {
			  ...
			  Logger.module("AI").debug("[G:" + gameSession.gameId + "] ai_findAndExecuteActionSequence( ) -> building actions from psuedoactions: on psuedoAction# " + i + ". Object:  ", pseudoAction);
				var action = generateActionFromPseudoAction(gameSession, pseudoAction);
...
var ai_findActionSequence = function (gameSession, playerId, depthLimit, msTimeLimit) {
	var gameId = getGameId(gameSession);
	return new Promise(function (resolve, reject) {
		...
		if (gameSession.isNew()) {
			...
			var drawStartingHandPseudoAction = ai_getDrawStartingHandPseudoAction(gameSession, playerId);
		  ...
		  //+++CURRENTLY only allows card replace at beginning of turn! Because RNG detection is needed
			var replaceCardFromHandPseudoAction = ai_getReplaceCardFromHandPseudoAction(gameSession, playerId);
 ...
 */
var ai_foundActionSequence = function (rootNode) {
	var gameId = rootNode.gameId;
  ...
  // pick highest scoring sequence
	...
	var sequence = _.max(rootNode.evaluatedSequences, function (sequence) { return sequence.score; });
	...
	if (ai_cluster_evaluationQueue.length > 0) {
		ai_cluster_evaluationQueue = _.reject(ai_cluster_evaluationQueue, function (gameSession) { return getGameId(gameSession) == gameId; });
	}
	...
	var resolve = ai_cluster_findActionSequenceResolveByGameId[gameId];
...
var ai_cluster_master_evaluateQueuedGameSessions = function () {
	...
	while (ai_cluster_evaluationQueue.length > 0 && ai_cluster_workerPool_idle.length > 0) {
		...
		var worker = ai_cluster_workerPool_idle.shift();
		ai_cluster_workerPool_busy.push(worker);
		var gameSession = ai_cluster_evaluationQueue.shift();
		var serializedGameSession = gameSession.generateGameSessionSnapshot();
		//get depth scores for pruning if available
		var bestScoresByDepth = ai_cluster_rootNodesByGameId[getGameId(gameSession)].bestScoresByDepth;
		var depthLimit = ai_cluster_rootNodesByGameId[getGameId(gameSession)].depthLimit;
		var msTimeLimit = ai_cluster_rootNodesByGameId[getGameId(gameSession)].msTimeLimit;
		var lastStep = gameSession.getLastStep();
		var cardWaitingForFollowupsIndex = null;
		if (gameSession.getIsFollowupActive()) {
    //transmit the followupcard index with the active followup so that we can inject it after deserializing in the worker process
		...
		var includedRandomness = lastStep.getIncludedRandomness();
		...
		worker.send({ gameSession: serializedGameSession, bestScoresByDepth: bestScoresByDepth, depthLimit: depthLimit, msTimeLimit: msTimeLimit, includedRandomness: includedRandomness, cardWaitingForFollowupsIndex: cardWaitingForFollowupsIndex });
	}
};
...
var ai_cluster_master_addReturnedSequenceToRootNode = function (msgFromWorker) {
	...
	var gameId = msgFromWorker.gameId;
	var rootNode = ai_cluster_rootNodesByGameId[gameId];
	if (rootNode == null) {
...
// TODO: remove me
var _checkedForLethalOnEnemyGeneral = false;
var _nextActions = [];
var _followupTargets = [];
var _debugMode = true;
...
var ai_preprocessProtoActions = function (protoActions, gameSession) {
	//var protoAction = {
	//    card: card,
	...
	_.each(protoActions, function (protoAction) {
...
var ai_preProc_filterProtoActionTargets = function (protoAction) {
	...
	var filteredProtoActionTargetsAndScores = protoAction.targetsAndScores;
  ...
  //rejects any PlayCardFromHandAction action that scores below qualitythreshold
	if (protoAction.actionType == "PlayCardFromHandAction") {
		filteredProtoActionTargetsAndScores = _.filter(filteredProtoActionTargetsAndScores, function (targetAndScore) {
		  return targetAndScore.score > preProc_qualityThreshold_playCard; //-2 TODO: unexpected behavior with summon spells - consider excluding summons or diff. threshold for summons?
		});
	}


	//relative filtering next
	var targetsAndScoresToReturn = [];
  ...
  //optimize - to remove/replace with faster sorting (during scoring)
	...
	filteredProtoActionTargetsAndScores = _.sortBy(filteredProtoActionTargetsAndScores, function (targetAndScore) {
		return targetAndScore.score;
	}).reverse();
	...
	if (filteredProtoActionTargetsAndScores.length > 0) {
		var numberToTake = Math.floor(filteredProtoActionTargetsAndScores.length * preProc_percentOfActionsToEvaluateAfterMinimum);
...
var ai_preProc_playCard_module_draw = function (card, target, gameSession) {
	...
	if (card.drawCardsPostPlay !== null) {
		var numberOfCardsInHand = gameSession.getCurrentPlayer().getDeck().getNumCardsInHand();
		var maxHandSize = CONFIG.MAX_HAND_SIZE;
		var remainingManaAfterCast = gameSession.getCurrentPlayer().getRemainingMana() - card.getManaCost();
...
var ai_preProc_playCard_module_removal = function (card, targetPosition, gameSession) {
	...
	if (CARD_INTENT[cardId] != null && CARD_INTENT[cardId].indexOf("removal") !== -1) {
		var affectedUnits; //array of units that will be affected by spell.
		var myPlayer = card.getOwner();
		var opponentGeneral = gameSession.getGeneralForOpponentPlayer(myPlayer);
		if (CARD_INTENT[cardId].indexOf("mass")) {
			//find targets here
		  affectedUnits = gameSession.getBoard().getUnits();
			if (CARD_INTENT[cardId].indexOf("enemy") !== -1) {
			  affectedUnits = _.reject(affectedUnits, function (unit) { return unit.getOwner() == myPlayer; });
			}
			if (CARD_INTENT[cardId].indexOf("minion") !== -1) {
				affectedUnits = _.reject(affectedUnits, function (unit) { return unit.getIsGeneral(); });
			}
			if (cardId == SDK.Cards.Spell.PlasmaStorm) { //TODO+++++ REMOVE CARD SPECIFIC CODE?
				affectedUnits = _.reject(affectedUnits, function (unit) { return unit.getATK() > 3; });
			}
		} else {
			//use target position only
			affectedUnits = [gameSession.getBoard().getUnitAtPosition(targetPosition)];
		}
		_.each(affectedUnits, function (affectedUnit) {
			//score for each removed unit
			var scoreOfAffectedUnit = ScoreForUnit(affectedUnit);
...
var ai_preProc_playCard_module_dealDamage = function (card, targetPosition, gameSession) {
	...
	if (card.damageAmount !== null) {
		...
		if (CARD_INTENT[cardId] != null && CARD_INTENT[cardId].indexOf("burn") !== -1) {
			...
			_.each(affectedUnits, function (affectedUnit) {
				//score for each removed unit
				var scoreOfAffectedUnit = ScoreForUnitDamage(gameSession, affectedUnit, card.damageAmount);
				//***+++TODO - HAND SCORING MODULE (FOR UNPLAYED CARDS) NEEDS TO INCLUDE THRESHOLD SCORING FOR REMOVAL CARDS!
				//subtract opportunity cost of removal to create a base threshold for removal (so we don't waste removal on 1/1's)
				//removal spells range from 2 (martyrdor) to 5 (abyssian hard removal). Most are 3-4.
...
var ai_getDrawStartingHandPseudoAction = function (gameSession, playerId) {
	if (gameSession.isNew()) {
		...
		var myPlayer = gameSession.getPlayerById(playerId);
...
var ai_getReplaceCardFromHandPseudoAction = function (gameSession, playerId) {
	...
	if (myPlayer.getDeck().getCanReplaceCardThisTurn()) {
		...
		var cardToReplace;
		var cardsInHand = myPlayer.getDeck().getCardsInHandExcludingMissing();

		// find first card that costs at least 2 more than our max mana
		cardToReplace = _.find(cardsInHand, function (card) {
		...
		if (cardToReplace == null) {
			cardToReplace = _.sample(_.filter(cardsInHand, function (card) { return card instanceof SDK.Spell; }));
		}

		// if we have more than 1 artifact, force an artifact to be replaced at random
		// this is intended to encourage play of units
		if (cardToReplace == null) {
			cardToReplace = _.sample(_.filter(cardsInHand, function (card) { return card instanceof SDK.Artifact; }));
		}

		// if we have no units in hand, force a card to be replaced at random
		// this is intended to encourage play of units
		var unitInHand = _.find(cardsInHand, function (card) { return card instanceof SDK.Unit; });
		if (unitInHand == null) {
...
var getIsDepthLimitReached = function(node) {
...
var buildTree = function (parentNode, gameSession) {
  ...
  //if the best sequence overall includes randomness, only a partial sequence will be returned to be stepped-through.
	...
	var protoActions = _findAllProtoActions(gameSession);
	var actions = [];
	...
	if (!depthLimitReached && !timeLimitReached && protoActions.length > 0) {
		...
		_.each(protoActions, function (protoAction) {
			if (protoAction.targetsAndScores.length > 0) {
				var actionsToPush = _buildActionsAndBranchFromProtoAction(gameSession, protoAction, snapshotOfCurrentGameSession);
				actions = actions.concat(actionsToPush);
			}
		});
		//Logger.module("AI").debug("[G:" + gameSession.gameId + "] buildTree( ) -> branched actions = ", actions);
		_.each(actions, function (action) {
			var child = generateNode(action, parentNode, parentNode.root, getGameId(gameSession));
			var gameSessionBranched = action.getGameSession(); //branching takes place inside findAllActions()
	 ...
	 Logger.module("AI").debug("[G:" + gameSession.gameId + "] buildTree( ) -> terminating leaf. # of actions = " + protoActions.length + ". depthLimitReached = " + depthLimitReached + ". timeLimitReached = " + timeLimitReached);
	  ...
	  if (cluster.isMaster) {
	    ...
	    var endTurnPsuedoAction = { actionType: SDK.EndTurnAction.type };
	    parentNode.root.numberOfChildren = 1;
	    var spoofedWorkerMsg = ai_getSpoofedWorkerMsg(gameSession, endTurnPsuedoAction);
	    ai_cluster_master_addReturnedSequenceToRootNode(spoofedWorkerMsg);
	  }
		//get board score, apply to leaf, check root for best leaf (overwriting if applicable), then return
		var myPlayerId = gameSession.getCurrentPlayerId();
...
var buildNodeSequenceFromTreeRoot = function (root) {
	...
	var bestLeaf = root.bestLeaf;
	var currentNodeInTraversal = bestLeaf;
	var nodeSequence = [];
  //if bestLeaf == rootNode, then we do not want to take any actions from the sequences.
	var isBestLeafRootNode = bestLeaf == root;
...
var generateNode = function (action, parent, root, gameId) {
  var child = { parent: parent, root: root, children: [], parentPsuedoAction: generatePseudoActionFromAction(action), boardScore: null, depth: ++parent.depth, numberOfChildren: 0, gameId: gameId, noMoreActions: false, includedRandomness: false };
...
var generateRootNode = function (gameSession, depthLimit, msTimeLimit) {
  ...
  gameSessionCopy.deserializeSessionFromFirebase(JSON.parse(snapshotOfCurrentGameSession));
  gameSessionCopy.executeAction(gameSessionCopy.actionEndTurn());
  var baseBoardScore = ScoreForBoard(gameSessionCopy, myPlayerId); //sometimes NOT acting is better. sequences will have to beat this baseline to be selected
	////Logger.module("AI").debug("[G:" + gameSession.gameId + "] generateRootNode( ) -> baseline board score = " + baseBoardScore);
	//create root node
	var rootNode = { parent: null, root: null, children: [], parentPsuedoAction: null, boardScore: baseBoardScore, bestLeaf: null, numSequencesEvaluated: 0, branchesPruned: 0, depth: 0, bestScoresByDepth: [], numberOfChildren: 0, startTime: Date.now(), gameId: getGameId(gameSession), depthLimit: depthLimit, msTimeLimit: msTimeLimit, evaluatedSequences: [], includedRandomness: false, truncated: false };
	rootNode.root = rootNode;
	rootNode.bestLeaf = rootNode;
	return rootNode;
};

var PseudoAction = function (action) {
...
var DrawStartingHandPseudoAction = function (action) {
...
var ReplaceCardFromHandPseudoAction = function (action) {
	PseudoAction.call(this, action);
...
var PlayCardFromHandPseudoAction = function (action) {
	PseudoAction.call(this, action);
	this.handIndex = action.getIndexOfCardInHand();
...
var PlaySignatureCardPseudoAction = function (action) {
	PseudoAction.call(this, action);
	this.targetPosition = action.getTargetPosition();
};
...
var PlayFollowupPseudoAction = function (action) {
	PseudoAction.call(this, action);
	this.targetPosition = action.getTargetPosition();
};
...
var MovePseudoAction = function (action) {
	PseudoAction.call(this, action);
...
var AttackPseudoAction = function (action) {
	PseudoAction.call(this, action);
	this.sourcePosition = action.getSourcePosition();
	this.targetPosition = action.getTargetPosition();
};
...
var generateActionFromPseudoAction = function (gameSession, pseudoAction) {
	switch (pseudoAction.actionType) {
		case SDK.MoveAction.type:
			...
			var targetPosition = pseudoAction.targetPosition;
...
var ai_getCardsInHandAndSignatureSpell = function (gameSession) {
  var myPlayer = gameSession.getCurrentPlayer();
...
var _findPlayCardProtoActions = function (gameSession) {
	...
	if (gameSession.getIsFollowupActive()) {
	  ...
	  var cardWaitingForFollowups = gameSession.getValidatorFollowup().getCardWaitingForFollowups();
		...
		//no followup active
		var cardsInHand = ai_getCardsInHandAndSignatureSpell(gameSession);
		// find all playable cards in hand
		playableCards = _.filter(cardsInHand, function (card) {
		...
		if (cluster.isMaster) {
			...
			playableCards = _.uniq(playableCards, function (card) { return card.getBaseCardId(); });
		}
	}
	if (playableCards.length > 0) {
		...
		for (var i = 0, il = playableCards.length; i < il; i++) {
			...
			var targetPositions = card.getValidTargetPositions();
			if (targetPositions.length > 0) {
				...
				var targetsAndScores = _.map(targetPositions, function (target) { return { target:target, score:null } });
				var protoAction = {
					card: card,
					targetsAndScores: targetsAndScores
				};
...
var _findAttackProtoActions = function (gameSession) {
	...
	// assemble my units
	var myGeneral = gameSession.getGeneralForPlayer(gameSession.getCurrentPlayer());

	var myUnits = gameSession.getBoard().getFriendlyEntitiesForEntity(myGeneral);
	...
	_.each(myUnits, function (unit) {
		if (!_isUnitPreventedFromAttacking(unit)) {
			var potentialAttackTargets = unit.getAttackRange().getValidTargets(gameSession.getBoard(), unit, unit.getPosition());
			//filter out bad targets i.e. suicide targets for generals or illegal targets while provoked or immune units
			var filteredPotentialAttackTargets = filterAttackTargetsForUnit(unit, potentialAttackTargets);
			var potentialAttackPositions = _.map(filteredPotentialAttackTargets, function (target) { return target.getPosition() });
			if (potentialAttackPositions.length > 0) {
				var targetsAndScores = _.map(potentialAttackPositions, function (target) { return { target: target, score: null } });
				var protoAction = {
					card: unit,
					targetsAndScores: targetsAndScores,
...
var _findMoveProtoActions = function (gameSession) {
	...
	_.each(myUnits, function (unit) {
		if (!_isUnitPreventedFromMoving(unit)) {
			var potentialMoves = _findPotentialMovePositionsForUnit(gameSession, unit);
			if (potentialMoves.length > 0) {
				var targetsAndScores = _.map(potentialMoves, function (target) { return { target: target, score: null } });
				var protoAction = {
					card: unit,
					targetsAndScores: targetsAndScores,
...
var _branch = function (gameSession, snapshotOfCurrentGameSession) {
...
var injectFollowupIntoGameSession = function (gameSession, followupParentCardIndex) {
  var parentCard = gameSession.getCardByIndex(followupParentCardIndex);
  //inject copied followups onto branched parent card since they were cleared during branching deserialization
  var referenceCardForFollowup = gameSession.getCardCaches().getCardById(parentCard.getId())
  ...
  var cardInBranchedSession = parentCard.getCurrentFollowupCard();
...
var _buildActionsAndBranchFromProtoAction = function (gameSession, protoAction, snapshotOfCurrentGameSession) {
	...
	var targetsAndScores = protoAction.targetsAndScores;
	var card = protoAction.card;
	...
	_.each(targetsAndScores, function (targetAndScore) {
		var position = targetAndScore.target;
		var gameSessionBranched = _branch(gameSession, snapshotOfCurrentGameSession);
		if (card.getIsFollowup()) {
		  //reconstruct followup data in cloned gamesession
		  var followupParentCardIndex = card.getParentCardIndex();
...
var _findPotentialMovePositionsForUnit = function (gameSession, unit) {
	var potentialMoves = [];
	var traversalPath = unit.getMovementRange().getValidPositions(gameSession.getBoard(), unit);
...
var _isUnitAdjacentToOrOnManaTile = function (unit) {
	var gameSession = unit.getGameSession();
	var manaTiles = _.filter(gameSession.getBoard().getTiles(true), function (tile) { return tile.getBaseCardId() === SDK.Cards.Tile.BonusMana; });
	return _.some(manaTiles, function (manaTile) { return _arePositionsEqualOrAdjacent(unit.position, manaTile.position) })
};
...
var scoreForUnit_module_generalRetreat = function (gameSession, unit, position) {
	var score = 0;
	var myGeneral = gameSession.getGeneralForPlayerId(unit.getOwnerId());
	...
	if (!_hasLethalOnEnemyGeneral && myGeneral.getHP() < THRESHOLD_HP_GENERAL_RETREAT) {
		...
		if (unit.getIsGeneral())
		{
			...
			var allEnemyUnits = gameSession.getBoard().getEnemyEntitiesForEntity(unit, SDK.CardType.Unit);
			_.each(allEnemyUnits, function (enemyUnit) {
				var distanceFromEnemyUnit = _distanceBetweenPositions(position, enemyUnit.getPosition());
...
var _getIsScoredModifier = function (modifier) {
	return !(
...
var _getBountyForDistanceFromMyGeneral = function (gameSession, playerId) {
...
var _getBountyForDistanceFromOpponentGeneral = function () {
...
var findNearestObjective = function (position, objectives) {
	var nearestObjective = objectives[0];
	var closestDistance = 999;
	//this should be _.min, lol
	_.each(objectives, function (objective) {
		var distance = distanceBetweenBoardPositions(position, objective.getPosition());
...
var ai_getScoreForAttackAction = function (attackSource, attackTargetPosition) {
	var score = 0;
	var gameSession = attackSource.getGameSession();
	var targetedEnemyUnit = gameSession.getBoard().getUnitAtPosition(attackTargetPosition);
...
var PRUNING_THRESHOLD = .90; //if a branch falls below XX% of best depth score, it is pruned
var preProc_percentOfActionsToEvaluateAfterMinimum = .2;
