 ...
 * @public
 */
let ScoreForPhaseDeath = function (card, targetPosition, cardIntents) {
	let score = 0;
	...
	_.each(validIntents, function (intent) {
		...
		else if(intent.type === CardIntentType.ModifyATK){bonus += ScoreForIntentModifyATK(card, targetPosition);}
		else if(intent.type === CardIntentType.ModifyHP){bonus += ScoreForIntentModifyHP(card, targetPosition);}
