...
const findAttackObjectivesAndScoresForUnit = function (gameSession, unit, targetPosition) {
  ...
  // get scores for all filtered attack targets
	const filteredAttackTargetsAndScores = _.map(filteredAttackTargets, function (enemy) {
    let sortBounty = 0;
    ...
    if (unit.getIsPlayed()) {
      if (unit.hasModifierClass(SDK.ModifierRanged) || unit.hasModifierClass(SDK.ModifierBlastAttack))
        sortBounty += distanceBetweenBoardPositions(unit.position, enemy.position) <= 1 ? 0 : BOUNTY.TARGET_AT_RANGE; //ranged prefer to attack units at range
      if (unit.hasModifierClass(SDK.ModifierBackstab))
        sortBounty += gameSession.getBoard().getIsPositionBehindEntity(enemy, unit.position, 1, 0) ? BOUNTY.TARGET_BACKSTAB_PROC : 0; //backstab prefer to attack units from behind
      if (unit.hasModifierClass(SDK.ModifierBlastAttack)) {
        sortBounty += _.reject(gameSession.getBoard().getEntitiesInRow(enemy.getPosition().y, SDK.CardType.Unit), function (entity) { return entity.getIsSameTeamAs(unit); }).length * BOUNTY.TARGET_ENEMIES_IN_SAME_ROW;
      }
    }
    else { //unit not played, but we still want ranged/blastAttackers to spawn at distance.
      if (unit.hasModifierClass(SDK.ModifierRanged) || unit.hasModifierClass(SDK.ModifierBlastAttack))
        sortBounty += distanceBetweenBoardPositions(sourceUnit.position, enemy.position) <= 1 ? 0 : BOUNTY.TARGET_AT_RANGE; //ranged prefer to attack units at range
    }
  ...
  //sort high to low (descending)
	const sortedFilteredAttackTargetsAndScores = _.sortBy(filteredAttackTargetsAndScores, function (objectiveAndScore) {
    return objectiveAndScore.score;
  }).reverse();
