...
const filterAttackTargetsForUnit = function (unit, potentialTargets) {
  ...
  potentialTargets = _.reject(potentialTargets, function (target) { return _isTargetImmuneToSource(target, unit); });
...
const _filterAttackTargetsForProvokedUnit = function (unit, potentialTargets) {
  if (unit.getIsProvoked()) { //may only attack provoker
    potentialTargets = _.filter(potentialTargets, function (target) { return target.getIsProvoker(); });
    potentialTargets = _.filter(potentialTargets, function (target) { return _.contains(target.getEntitiesProvoked(), unit); });
  }
  return potentialTargets;
};
...
const _filterAttackTargetsForGeneralUnit = function (unit, potentialTargets) {
  if (unit.getIsGeneral()) {
    ...
    potentialTargets = _.reject(potentialTargets, function (enemy) {
      return enemy.getATK() > unit.getHP() * BOUNTY.THRESHOLD_COUNTERATTACK_HP_PCT_GENERAL;
    });
    potentialTargets = _.reject(potentialTargets, function (enemy) {
      //avoid suicide
      return unit.getHP() <= enemy.getATK();
    });
    //TODO ***DRAW GAMES***
...
const _isTargetImmuneToSource = function (target, source) {
  if (target instanceof SDK.Card && source instanceof SDK.Card) {
    ...
    if (immuneModifiers.length > 0) {
			const immuneModifier = _.find(immuneModifiers, function (modifier) {
        // immune to attack
