...
createSinglePlayerGame = require 'server/lib/create_single_player_game'
...
getRequeueParams = () ->
  ...
  Consul.kv.get("environments/#{process.env.NODE_ENV}/matchmaking-rift-params.json")
  .then (v)->
    params = JSON.parse(v)
...
requeueJob = (job, done) ->
  ...
  .then (params) ->
    ...
    return new Promise (resolve, reject) ->
      Redis.Jobs.create("matchmaking-search-rift", job.data)
...
logMatchMade = (token1, token2) ->
  ...
  riftQueue.matchMade("rift", waitTime2)
...
findOpponent = (userId, lastOpponentId, rank, radius) ->
  ...
  .then (players) ->
    ...
    if !this.allowMatchWithLastOpponent and lastOpponentId
      Logger.module("MATCHMAKING-RIFT-JOB").debug("excluding last opponent #{lastOpponentId?.blue}")
...
module.exports = (job, done) ->
  ...
  gameType = job.data.gameType || GameType.Rift
  ...
  isQueued = riftQueue.isPlayerQueued(userId)
          ...
          if !opponent
            ...
            if moment.duration(moment.utc().valueOf() - firstAttemptAt.valueOf()).asSeconds() > 25
              return Promise.all([
                ...
                return knex("users").where('is_bot',true).offset(knex.raw('floor(random()*110)')).first('id','username')
              .then (randomBotRow) ->

                @.randomBotRow = randomBotRow
                ...
                aiGeneralId = _.sample(FactionFactory.generalIdsForFaction(aiFactionId))
                aiDifficulty = 1.0

                # bots should use around ~12 random cards
                aiNumRandomCards = Math.floor(CONFIG.MAX_DECK_SIZE * 0.3)

                Logger.module("MATCHMAKING-RIFT-JOB").log("[J:#{job.id}] #{gameType.yellow} - #{userId} has waited too long, matching with bot #{@.randomBotRow.username}")
              ...
              .then ()->
                # We're done
                return done(null, {opponentName: @.randomBotRow.name})
            else
            ...
            .then (opponentToken) -> # TODO: We should validate results
              ...
              return Promise.all([
                ...
                riftQueue.remove([@token1.userId,@token2.userId])
              ])
            .then (results) -> # TODO: We should validate results
              ...
              # log it
              Logger.module("MATCHMAKING-RIFT-JOB").log("[J:#{job.id}] #{gameType.yellow} - Search for Game (#{userId}) done(), matched versus #{@token2.userId}")
