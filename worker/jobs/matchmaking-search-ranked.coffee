...
requeueJob = (job, done) ->
  ...
  .then (params) ->
    ...
    return new Promise (resolve, reject) ->
      Redis.Jobs.create("matchmaking-search-ranked", job.data)
...
###
findOpponent = (userId, lastOpponentId, rank, deckValue, attempt, firstAttemptAt) ->
  ...
  .then (params) ->
    ...
    deckValueRadius = (attempt % 6) + Math.floor(attempt/3) + Math.floor(attempt * params.rankedDeckRadiusIncrease)
    ...
    taperEnd = 15
    if rank < taperStart
      delta = Math.max(0,rank - taperEnd)
      taperVal = 10 - 10 * (delta / (taperStart - taperEnd))
  ...
  .spread (playersWithinRank,playersWithinDeckValue) ->

    # exclude the user that's looking
    rankPlayers = _.filter(playersWithinRank, (id) -> return id != userId)
    deckPlayers = _.filter(playersWithinDeckValue, (id) -> return id != userId)

    # exclude last opponent
    if !this.allowMatchWithLastOpponent and lastOpponentId
      Logger.module("MATCHMAKING-JOB").debug("excluding last opponent #{lastOpponentId?.blue}")
      rankPlayers = _.filter(playersWithinRank, (id) -> return id != lastOpponentId)
      deckPlayers = _.filter(playersWithinDeckValue, (id) -> return id != lastOpponentId)

    # players within both RANK and DECK VALUE
    players = _.intersection(rankPlayers,deckPlayers)
...
module.exports = (job, done) ->
  ...
  isQueued = rankedQueue.isPlayerQueued(userId)
          ...
          if !opponent
            ...
            .then (opponentToken) -> # TODO: We should validate results
              ...
              return Promise.all([
                ...
                rankedQueue.remove([@token1.userId,@token2.userId])
                rankedDeckValueQueue.remove([@token1.userId,@token2.userId])
              ])
            .then (results) -> # TODO: We should validate results
              ...
              # log it
              Logger.module("MATCHMAKING-JOB").debug("[J:#{job.id}] #{gameType.yellow} - Search for Game (#{userId}) done(), matched versus #{@token2.userId}")
