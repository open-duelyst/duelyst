(function () {
	...
	helpers.getIsFileReadable = function (file) {
		var ext = helpers.getContentAfterLastDot(file).toLowerCase();
	...
	helpers.stripComments = function (content) {
		return content.replace(/\/\/[\s\S]*?(\r|\n)/g, "$1").replace(/\/\*[\s\S]*?\*\//g, "").replace(/<!--[\s\S]*?-->/g, "").replace(/###[\s\S]*?###/g, "").replace(/[^#]{1}#[^#][\s\S]*?(\r|\n)/g, "$1");
	};
					...
					list.forEach(function (file, index) {
						...
						fs.stat(file, function (err, stat) {
							if (stat && stat.isDirectory()) {
								var subDirectoryPromise = helpers.recursivelyGetFilesStartingFrom(file);
								subDirectoryPromise.then(function (filesInSubDirectory) {
							...
							if (index === list.length - 1) {
								Promise.all(subDirectoryPromises).then(function () {
									resolve(files);
								});
							}
						});
