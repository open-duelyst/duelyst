'use strict'
const { app, ipcMain, Menu, BrowserWindow, globalShortcut, dialog } = require('electron')
...
function setupWin32Shortcuts() {
	...
	localShortcut.register('Ctrl+R', function () {
		const focusedWindow = BrowserWindow.getFocusedWindow()
		if (focusedWindow) focusedWindow.webContents.reloadIgnoringCache()
	})
	localShortcut.register('Ctrl+Shift+I', function () {
		const focusedWindow = BrowserWindow.getFocusedWindow()
		if (focusedWindow) focusedWindow.toggleDevTools()
	})
	if (!argv.steam) {
		localShortcut.register('Ctrl+Shift+F', function () {
			const focusedWindow = BrowserWindow.getFocusedWindow()
			if (focusedWindow) focusedWindow.setFullScreen(!focusedWindow.isFullScreen())
		})
	}
	if (argv.steam && !argv.overlay) {
		localShortcut.register('Shift+Tab', function () {
			const data = {
	...
	let template = [
		...
		},
		{
			...
			submenu: [
				{
					label: 'Support',
					click: function () { require('shell').openExternal('https://support.duelyst.com') }
				}
			]
		}
	];
...
ipcMain.on('create-window', function (event, options) {
	const windowOptions = {
	...
	let win = window.createWindow(windowOptions)
	win.showURL(options.url, {}, () => { })
})

ipcMain.on('steam-error', function (event, options) {
	dialog.showMessageBox({
		type: 'error',
		title: 'ERROR',
