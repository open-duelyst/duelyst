...
describe('api', function() {
	...
	before( function(){
		this.timeout(5000);
		escapedEmail = fbUtil.escapeEmail('unittestdummy@gmail.com');
	...
	describe('register', function() {
		...
		it('expect a new user id when registering with valid credentials', function(done) {
			...
			request
				.post('/session/register')
				.set('Client-Version', version)
				.send({ email: "unittestdummy@gmail.com", username: 'unittestdummy', password: "password", keycode: "kumite14"})
				.expect(200, done);
		});
	});
	...
	describe('/api/me/ladder/ranking', function() {
		it('returns 200 OK', function(done) {
			this.timeout(2500);
			request
				.post('/api/me/rank')
	...
	describe('/api/me/quests/daily/generate', function() {
	...
	describe('/api/me/quests/daily/mulligan', function() {
	...
	describe('/api/me/booster_packs/buy', function() {
		...
		before( function(){
			...
			.then(function(userId){
				return UsersModule.updateWalletWithFunction(userId, function(data) {
					if (data)
						data.gold_amount = 100;
					return data;
				});
			})
	...
	describe('/api/me/booster_packs/unlock', function() {
		...
		it('expect 200 and unlocked booster data when providing valid booster id', function(done) {
			...
			request
				...
				.end(function(err,res){
					...
					_.each(res.body.cards, function(card){
	...
	describe('/api/me/shop/customer', function() {
	...
	describe('/api/me/craft/disenchant', function() {
	...
	describe('/api/me/craft/card', function() {
		...
		before( function(){
			...
			.then(function(userId){
				return UsersModule.updateWalletWithFunction(userId, function(walletData){
