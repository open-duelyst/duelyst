...
UtilsSDK.executeTutorialUntilOTK = function () {
	...
	while (!gameSession.isOver()) {
		...
		if (gameSession.isNew() && !challenge.getSkipMulligan()) {
			// mulligan as needed
			action = myPlayer.actionDrawStartingHand(challenge.requiredMulliganHandIndices);
			...
			if (gameSession.isMyTurn()) {
				var instruction = challenge.getCurrentInstruction();
				if (instruction != null) {
					var action = null;
					if (instruction.expectedActionType === SDK.EndTurnAction.type) {
						...
						var movingUnit = gameSession.getBoard().getUnitAtPosition(instruction.sourcePosition);
						action = movingUnit.actionMove(instruction.targetPosition);
					} else if (instruction.expectedActionType === SDK.AttackAction.type) {
						var attackingUnit = gameSession.getBoard().getUnitAtPosition(instruction.sourcePosition);
						...
						var targetPosition = instruction.targetPosition;
						if (targetPosition == null) {
							var card = myPlayer.getDeck().getCardInHandAtIndex(instruction.handIndex);
...
UtilsSDK.executeTutorialOpponentTurn = function () {
	var gameSession = SDK.GameSession.getInstance();
	var challenge = gameSession.getChallenge();
	var opponentAgent = challenge.getOpponentAgent();
	var opponentAgentActions;
	...
	while (true) {
		...
		if (finalAgentAction == null || finalAgentAction.isSoft) {
			...
			var opponentAction = AgentActions.createSDKActionFromAgentAction(opponentAgent, finalAgentAction);
...
UtilsSDK.removeCardFromBoard = function (boardX, boardY) {
	var card = SDK.GameSession.getInstance().getBoard().getEntityAtPosition({x: boardX, y: boardY}, true);
...
UtilsSDK.executeActionWithoutValidation = function (action) {
	var validators = SDK.GameSession.getInstance().getValidators();
...
UtilsSDK.modifyUnitStats = function (position, atk, maxHP, dmg) {
	...
	var card = board.getUnitAtPosition(position);
  if (card != null) {
  	if (atk != null) {
    	card.atk = atk;
    }
...
UtilsSDK.getEntityOnBoardById = function (cardId) {
	...
	var entityArray = board.getEntities(true);
