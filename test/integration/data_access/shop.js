    ...
    describe("purchaseProduct()", function() {
      ...
      it('expect buying an EMOTE Cosmetic to update inventory', function() {
        return stripe.tokens.createAsync({
          ...
          const productData = SDK.CosmeticsFactory.cosmeticProductAttrsForIdentifier(SDK.CosmeticsLookup.Emote.HealingMysticHappy)
      ...
      it('expect buying a Cosmetics Bundle to unlock all cosmetics in the bundle', function() {
        ...
        .then(function(token){
          ...
          return Promise.all([
            ...
            const cosmeticIdStrings = _.pluck(cosmeticsRows,"cosmetic_id");
            const bundleCosmeticIds = ShopData["bundles"]["Bundle_Argeon_Emotes"]["bundle_cosmetic_ids"];
            const numMatching = 0;
            for (var i = 0, il = cosmeticIdStrings.length; i < il; i++) {
              const cosmeticId = parseInt(cosmeticIdStrings[i]);
