...
describe("gauntlet module", function() {
	...
	var numDeckRarityTests = 3000;

	var fillOutArenaDeck = function(userId) {
	...
	describe("buyArenaTicketWithGold()", function() {
		...
		it('expect to be able to buy a ticket for 150 gold', function() {
			return knex("users").where('id',userId).update({'wallet_gold':150}).then(function(numUpdates){
				return GauntletModule.buyArenaTicketWithGold(userId)
	...
	describe("startRun()", function() {
		...
		// before cleanup to check if user already exists and delete
		before(function(){
			return knex("user_gauntlet_tickets").where('id',otherUserTicketId).delete()
			.then(function(){
				return knex("user_gauntlet_tickets").insert({
		...
		it('expect to NOT be able to start a run with an invalid ticket', function() {
			return GauntletModule.startRun(userId,"doesnt-exist")
			.then(function(result){
				expect(result).to.not.exist;
			}).catch(function(error){
				expect(error).to.exist;
			...
			}).spread(function(gauntletRunRow,fbRun){
		...
		it('expect to NOT be able to start a run with another user\'s ticket', function() {
			return GauntletModule.startRun(userId,otherUserTicketId)
			.then(function(result){
				expect(result).to.not.exist;
			}).catch(function(error){
				expect(error).to.exist;
		...
		it('expect to be able to start a run with a valid ticket', function() {
			return knex("user_gauntlet_tickets").where('user_id',userId).first()
			.bind({})
			...
			}).spread(function(ticketRows,gauntletRunRow,fbRun){
				expect(ticketRows.length).to.equal(0);
		...
		it('expect to ERROR out attempting starting a run in the middle of another one and to NOT use up an arena ticket', function() {
			return knex("users").where('id',userId).update({'wallet_gold':150}).then(function(numUpdates){
				return GauntletModule.buyArenaTicketWithGold(userId)
	...
	//		}).then(function(arenaData){
    //
	//			var invalidChoices = _.difference([
	...
	//			var generalSDKCard = gameSession.getCardCaches().getCardById(gauntletRow.general_id);
	...
	//		this.timeout(10000);
    //
	//		var emphasizedSet = SDK.CardSet.Unity;
	//		var numRounds = 10000;
	//		var totalChoices = 0;
	//		var totalSetChoices = 0;
	//		var all = [];
	//		var allPlayableFactions = SDK.FactionFactory.getAllPlayableFactions();
	//		for (var i = 0; i < numRounds; i++) {
	//			var factionId = _.sample(allPlayableFactions).id;
	//			var cardChoicesPromise = GauntletModule._generateCardChoices(Promise.resolve(), knex, userId, factionId, i % 30)
	//			.then(function (cardChoices) {
	//				totalChoices += cardChoices.length;
	//				for (var j = 0, jl = cardChoices.length; j < jl; j++) {
	//					var cardId = cardChoices[j];
	//					var sdkCard = _.find(SDK.GameSession.getCardCaches().getCardSet(emphasizedSet).getCards(), function (card) { return card.getId() === cardId; });
	//					if (sdkCard != null) {
	//						totalSetChoices++;
	//					}
	//				}
	...
	describe("getArenaDeck()", function() {

		it('expect to be able to retrive a user\'s active arena deck', function() {
			...
			.then(function () {
				...
				return GauntletModule.chooseCard(userId,arenaData.general_choices[0])
			}).then(function () {
				return fillOutArenaDeck(userId)
			}).then(function (){
		...
		it('expect the first card in a user\'s active arena deck to be the correct GENERAL', function() {
			var generalId;
			return GauntletModule.getArenaDeck(userId).then(function(deck){
				expect(deck).to.exist;
	...
	describe("getRunMatchmakingMetric()",function(){
		...
		it('expect to be able to retrive a user\'s active arena matchmaking metric', function() {
			return GauntletModule.getRunMatchmakingMetric(userId).then(function(metric){
				expect(metric).to.exist;
				expect(metric).to.within(0,12);
			});
		});

		it('expect metric to equal MAX WINS - win count', function() {
			return GauntletModule.updateArenaRunWithGameOutcome(userId,true,"metric_game_1")
			.then(function(){
				return GauntletModule.getRunMatchmakingMetric(userId)
		...
		it('TODO: expect metric request to fail on run with incomplete deck', function() {
			expect(true).to.exist;
		});

		it('TODO: expect metric request to fail on run that is finished', function() {
			expect(true).to.exist;
		});

		it('TODO: expect metric request to fail when there\'s no run', function() {
			expect(true).to.exist;
		});

	});

	describe("resignRun()", function() {

		var lastResignedAt = null;
		...
		it('expect to ERROR out an attempt to resign with no run', function() {
			return GauntletModule.resignRun(userId)
			.then(function(data){
		...
		it('expect to be able to resign an active arena run', function() {
			return knex("users").where('id',userId).update({'wallet_gold':150})
			.bind({})
			.then(function(){
				return GauntletModule.buyArenaTicketWithGold(userId);
		...
		it('expect an attempt to resign an ended run to ERROR out and leave data untouched', function() {
			return GauntletModule.resignRun(userId)
			.then(function(data){
				expect(data).to.not.exist;
			}).catch(function(error){
		...
		it('expect to ERROR out attempts to start a run before claiming rewards on a resigned run', function() {
			return knex("users").where('id',userId).update({'wallet_gold':150})
			.bind({})
			.then(function(){
				return GauntletModule.buyArenaTicketWithGold(userId);
	...
	describe("updateArenaRunWithGameOutcome()", function() {
		...
		before(function(){
			...
			return knex("users").where('id',userId).update({'wallet_gold':2500}).then(function(numUpdates){
				return knex("user_gauntlet_run").where({user_id:userId}).delete()
			}).then(function(){
				return Promise.all([
		...
		it('expect to FAIL to update arena run with a game if no arena run is active', function() {
			return GauntletModule.updateArenaRunWithGameOutcome(userId,"game 1",true)
			.then(function(arenaData){
				expect(arenaData).to.not.exist;
			}).catch(function(error){
				expect(error).to.exist;
		...
		it('expect to FAIL to update arena run with a game if an arena run is over', function() {
		...
		it('expect a won game to update the arena win counter', function() {
			...
			.then(function(){
				...
				return GauntletModule.updateArenaRunWithGameOutcome(userId,true,gameId)
			}).then(function(arenaData){
				expect(arenaData).to.exist;
			...
			}).spread(function(gauntletRow,fbRun,firebaseGameJobStatusSnapshot){
		...
		it('expect a lost game to update the arena loss counter', function() {
			return GauntletModule.updateArenaRunWithGameOutcome(userId,false,"game 2")
			.then(function(arenaData){
				expect(arenaData).to.exist;
				expect(arenaData.win_count).to.equal(1);
		...
		it('expect a draw to update the arena draw counter and not win/loss', function() {
			return GauntletModule.updateArenaRunWithGameOutcome(userId,false,"game 2",true)
			.then(function(arenaData){
				expect(arenaData).to.exist;
				expect(arenaData.win_count).to.equal(1);
				expect(arenaData.loss_count).to.equal(1);
		...
		it('expect 3 losses to end the run', function() {
			return Promise.all([
			...
			]).spread(function(arenaDataNoFinal,arenaData){
		...
		it('expect to be able to claim rewards for a complete run', function() {
			...
			}).spread(function(gauntletRow,rewardRows,fbRun){

				var arenaRewards = _.filter(rewardRows,function(row){ return row.source_id == gauntletRow.ticket_id })
		...
		it('expect NOT to be able to claim rewards TWICE for a complete run', function() {
			return knex.select().from("user_rewards").where({'user_id':userId})
			.bind({})
	...
	describe("claimRewards()", function() {
		...
		it('expect inventory and wallet to update after claiming rewards', function() {
			...
			return Promise.all([
				...
				var newCollectionData = collectionSnapshot.val();
				var totalGoldEarned = 0;
				var totalSpiritEarned = 0;

				_.each(rewardRows,function(reward){
					if (reward["cards"]) {

						var cardId = reward["cards"][0];
						// Logger.module("UNITTEST").log("checking card "+cardId);
						var newCount = newCollectionData[cardId].count;
						var oldCount = 0;
						if (this.collectionRow && this.collectionRow.cards[cardId])
							oldCount = this.collectionRow.cards[cardId].count;

						expect(oldCount+1).to.equal(newCount);
						expect(oldCount+1).to.equal(collectionRow.cards[cardId].count);

					} else if (reward["gold"]) {
						totalGoldEarned += parseInt(reward["gold"])
					} else if (reward["spirit"]) {
						totalSpiritEarned += parseInt(reward["spirit"])
					}
				}.bind(this));
				...
				// check gold
				var oldGold = this.userRow.wallet_gold || 0;
				...
				// check spirit
				var oldSpirit = this.userRow.wallet_spirit || 0;
		...
		it('expect not to be able to claim rewards twice', function() {
			return GauntletModule.claimRewards(userId)
			.then(function(response){
				expect(response).to.not.exist;
			}).catch(function(error){
	...
	////
	describe("generate card rarity output", function() {
		...
		it('iterate over card choice rarities', function() {
			...
			var numEpic = 0;
			var numLegendary = 0;
			var minRaritySum = Number.MAX_SAFE_INTEGER;
			var maxRaritySum = 0;

			var numFactionCards = 0;
			var numNeutralCards = 0;

			var generateDeck = function () {
				var deck = [];
				var factionId = _.random(1,6);
			...
			var gatherDeckResults = function (iteration) {
				return generateDeck()
				.then(function (deck) {
					var deckRaritySum = 0;
					_.each(deck, function (cardId) {
						var sdkCard = SDK.GameSession.getCardCaches().getCardById(cardId);
						var cardRarity = sdkCard.getRarityId();
						var cardFactionId = sdkCard.getFactionId();
			...
			var deckPromises = [];
			for (var i=0; i<numDeckRarityTests; i++) {
				deckPromises.push(gatherDeckResults(i))
			}

			return Promise.all(deckPromises)
			.then(function () {
				var sumCards = numBasics + numCommon + numRare + numEpic + numLegendary;
				console.log("============");
