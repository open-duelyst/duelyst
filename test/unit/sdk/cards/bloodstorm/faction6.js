...
describe("bloodstorm", function() {
	describe("faction6", function(){
		...
		it('expect frigid corona to stun an enemy and draw a card', function() {
			...
			var myriad = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction6.Myriad}, 4, 3, gameSession.getPlayer2Id());
		...
		it('expect sleet dasher to reactivate whenever it destroys a minion', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();

			var sleetDasher = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction6.SleetDasher}, 0, 1, gameSession.getPlayer1Id());
			var valeHunter = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.ValeHunter}, 1, 1, gameSession.getPlayer2Id());
			var valeHunter2 = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.ValeHunter}, 0, 0, gameSession.getPlayer2Id());
			var valeHunter3 = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.ValeHunter}, 1, 2, gameSession.getPlayer2Id());
			var windstopper = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.WindStopper}, 1, 0, gameSession.getPlayer2Id());
		...
		it('expect concealing shroud to make your general immune to damage until your next turn', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
		...
		it('expect enfeeble to turn all minions into 1/1', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
			...
			var adept = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction1.WindbladeAdept}, 1, 2, gameSession.getPlayer1Id());
