...
describe("unity", function() {
	describe("faction2", function(){
		...
		it('expect sparrowhawk to give mist dragon seal if you have another arcanyst', function() {
		...
		it('expect joseki to give both players a card from their opponents deck', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
		...
		it('expect joseki to do nothing if there are no cards in deck', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			var player2 = gameSession.getPlayer2();
		...
		it('expect bangle of blinding strike to give the General Celerity', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
		...
		it('expect kindling to give +1 attack to only arcanysts when spells are cast', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;

			var kindling = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction2.Kindling}, 2, 2, gameSession.getPlayer1Id());
		...
		it('expect calligrapher to put 3 random songhai spells in action bar', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			//make calligrapher, attack
			var calligrapher1 = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction2.Calligrapher}, 1, 1, gameSession.getPlayer1Id());
			var brightmossGolem1 = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.BrightmossGolem}, 0, 1, gameSession.getPlayer2Id());
			var action = calligrapher1.actionAttack(brightmossGolem1);
			gameSession.executeAction(action);
