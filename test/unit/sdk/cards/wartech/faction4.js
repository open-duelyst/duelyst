...
describe("wartech", function() {
	describe("faction4", function(){
		...
		it('expect cacophynos to destroy a random nearby enemy when dying', function() {
		...
		it('expect void talon to successfully build', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
		...
		*/
		...
		it('expect vellumscry to destroy a friendly minion to draw 3 cards', function() {
			...
			var kaidoAssassin = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction2.KaidoAssassin}, 1, 2, gameSession.getPlayer1Id());
		...
		it('expect nightmare operant to put a MECHAZ0R in your deck', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;

			var nightmareOperant = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction4.NightmareOperant}, 1, 3, gameSession.getPlayer1Id());
		...
		it('expect deathmark to deal 1 damage to a minion, and for the minion to be destroyed when attacked', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
		...
		it('expect furor chakram to give friendly minions +2 Attack and Frenzy', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
		...
		it('expect moonrider to summon a Fiend nearby when you use your BBS', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
			...
			var moonrider = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction4.Moonrider}, 4, 3, gameSession.getPlayer1Id());

			var action = player1.actionPlaySignatureCard(4, 3);
			gameSession.executeAction(action);

			var fiend = board.getEntitiesAroundEntity(moonrider);
			expect(fiend.length).to.equal(1);
			expect(fiend[0].getId()).to.equal(SDK.Cards.Faction4.Fiend);

		});
		it('expect betrayal to cause enemy minions near the enemy general to attack them', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			var player2 = gameSession.getPlayer2();
			...
			var cacophynos = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction4.Cacophynos}, 8, 1, gameSession.getPlayer2Id());
		...
		it('expect abhorrent unbirth to destroy all your minions and  create a 1/1 abomination that gains all their Attack, Health, and keywords', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
			...
			var abomination = board.getUnitAtPosition({x:1,y:2});
			...
			var action = abomination.actionMove({ x: 1, y: 4});
			gameSession.executeAction(action);
		...
		*/
		...
		it('expect stygian observer to give friendly minions in your action bar +2/+2 whenever a minion dies', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;

			var stygianObserver = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction4.StygianObserver}, 3, 1, gameSession.getPlayer1Id());
			...
			var valeHunter = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.ValeHunter}, 2, 2, gameSession.getPlayer1Id());
			var silverguardSquire = UtilsSDK.applyCardToBoard({id: SDK.Cards.Faction1.SilverguardSquire}, 2, 3, gameSession.getPlayer1Id());
		...
		it('expect infest to deal 2 damage to the enemy General when the targeted minion is destroyed, and spread to nearby enemies', function() {
			var gameSession = SDK.GameSession.getInstance();
			var board = gameSession.getBoard();
			var player1 = gameSession.getPlayer1();
			player1.remainingMana = 9;
			...
			var valeHunter2 = UtilsSDK.applyCardToBoard({id: SDK.Cards.Neutral.ValeHunter}, 2, 3, gameSession.getPlayer2Id());
