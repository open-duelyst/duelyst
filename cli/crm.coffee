...
program
	...
	.option('-t, --hoursAgo <hours>', 'Number of hours ago to start report')
	.option('-m, --maxResults <number>', 'Maximum number of results')
	.description 'get steam charge report'
	.action (args, callback)->
		opts = {
			sandbox: args.options.sandbox
			maxResults: args.options.maxResults
		}
		if args.options.hoursAgo
			opts.time = moment().utc().subtract(args.options.hoursAgo, 'hours').format("YYYY-MM-DDTHH:mm:ss") + 'Z'
		Steam.getReport(opts).then (data)->
			if not data?
				throw new Error("Could not find data")
			console.log "STEAM DATA:".green
			console.log prettyjson.render(data)
		.catch (e)->
			console.log e
			console.log prettyError.render(e)
...
program
	...
	.option('-s, --sandbox', 'Enable sandbox mode')
	.description 'get steam charge details with transaction id (found in users purchase history)'
	.action (args, callback)->
		txId = args.txId.replace("t","")
		console.log("Steam transaction id: #{txId}")
		Steam.queryTxn({sandbox: args.options.sandbox, transactionId: txId}).then (data)->
			...
			return knex("user_charges").where('charge_id',data["orderid"]).first()

		.then (chargeData)->

			console.log "CHARGE DATA:".cyan
			console.log prettyjson.render(chargeData)
...
program
	...
	.option('-s, --sandbox', 'Enable sandbox mode')
	.description 'refund steam charge, get the order id by looking up transaction id'
	.action (args,callback)->
		orderId = args.orderId.replace("o","")
		console.log("Steam order id: #{orderId}")

		if not SyncModule?
			SyncModule = require 'server/lib/data_access/sync'

		knex("user_charges").where('charge_id',orderId).first()
		.bind {}
		.then (row)->
			if not row?
				throw new Error("Could not find row")
			...
			return confirmAsync("Refunding #{orderId} for #{@charge.amount} ... items will NOT be deleted")
		.then ()->
			return knex.transaction (tx)->
				knex("user_charges").where("charge_id",orderId).update({
						refunded_at: moment().utc().toDate()
						updated_at: moment().utc().toDate()
						memo: "refunded with no items removed"
					}).transacting(tx)
				.then (count)->
		...
		.then ()->
			console.log "refunding steam transaction"
			return Steam.refundTxn({sandbox: args.options.sandbox, orderId: orderId})
		.then (data) ->
			console.log "steam refund data:".green
			console.log prettyjson.render(data)
			return Steam.queryTxn({sandbox: args.options.sandbox, transactionId: data.transid })
		.then (data) ->
			console.log "steam transaction data:".green
			console.log prettyjson.render(data)
		.catch DidNotConfirmError, (e)->
			callback(e)
		.catch (e)->
...
program
	.command 'bnea:unlink <username_or_email>'
	.description 'unlink bnea id from a user\'s account'
	.action (args, callback)->
		console.log("loading modules...")
		UsersModule ?= require 'server/lib/data_access/users'
		console.log("loading modules... "+"DONE".green)
		...
		.then (id)->
			...
			return UsersModule.userDataForId(id)
		.then (data)->
			console.log "current bnea id: #{data.bnea_id}"
			return confirmAsync("unlink user #{username_or_email} from bnea... confirm?")
		.then () ->
			return UsersModule.disassociateBneaId(@id)
		.then () ->
			console.log 'unlinking complete, user must re-link to bnea account'
			callback()
		.catch DidNotConfirmError, (e)->
			console.log(e.message)
			callback()
...
program
	.command 'bnea:relink <username_or_email>'
	.description 'link bnea id to a user\'s account'
	.action (args, callback)->
		console.log("loading modules...")
		UsersModule ?= require 'server/lib/data_access/users'
		console.log("loading modules... "+"DONE".green)
		...
		.then () ->
			...
			return inquirerPromisifed.prompt([{
				name:'bneaId'
				message:"Enter new BNEA ID for linking:"
				default: null
			}])
		.then (input) ->
			console.log "linking to new bnea id: #{input.bneaId}"
			return UsersModule.associateBneaId({userId: @id, bneaId: input.bneaId})
		.then () ->
