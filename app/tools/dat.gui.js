...
dat.GUI = dat.gui.GUI = (function (css, saveDialogueContents, styleSheet, controllerFactory, Controller, BooleanController, FunctionController, NumberControllerBox, NumberControllerSlider, OptionController, ColorController, requestAnimationFrame, CenteredDiv, dom, common) {
  ...
  var GUI = function (params) {
    ...
    let use_local_storage = SUPPORTS_LOCAL_STORAGE
  ...
  common.extend(
    ...
    /** @lends dat.gui.GUI */
    {
      ...
      /**
         ...
         */
      remove(controller) {
        // check if this controller is actually a folder
        if (this.__folders && this.__folders[controller.name] === controller) {
          controller.close();
          const li = this.__folders[controller.name].domElement.parentNode || this.__folders[controller.name].domElement.parentElement;
          this.__ul.removeChild(li);
      ...
      /**
         * @param name
         * @param [alphabetical=false] whether to sort folder alphabetically
         * @returns {dat.gui.GUI} The new folder.
         * @throws {Error} if this GUI already has a folder by the specified
         * name
         * @instance
         */
      addFolder(name, alphabetical) {
        ...
        gui.__alphabetical = alphabetical || false;
      ...
      openFolders(recursive) {
        for (const name in this.__folders) {
          const folder = this.__folders[name];
          folder.open();
          if (recursive) {
            folder.openFolders(recursive);
          }
        }
      },

      closeFolders(recursive) {
        for (const name in this.__folders) {
          const folder = this.__folders[name];
          folder.close();
          if (recursive) {
            folder.closeFolders(recursive);
          }
        }
      },
      ...
      updateDisplay() {
        for (const i in this.__controllers) {
  ...
  /**
   * Adds a row to the DOM.
   *
   * @param gui
   * @param dom element to be added
   */
