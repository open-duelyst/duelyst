...
var PERF = {
	 ...
	 */
	get_stats_data: function (noise_threshold, below_threshold, above_threshold, fixed, unconverted) {
		var data = {};
	 ...
	 * @returns {Object}
	 */
	get_stats_data_by_id: function (id, noise_threshold, below_threshold, above_threshold, fixed, unconverted) {
		var stats_data = {};
		var stats = PERF.get_stats_by_id(id);
		if (stats != null) {
			...
			var stats_noise_threshold = noise_threshold != null ? noise_threshold : stats.get_noise_threshold();
			if (stats_noise_threshold != null) {
				stats_data["noise(" + stats_noise_threshold + ")"] = stats.noise(stats_noise_threshold, fixed, unconverted);
			}

			var stats_below_threshold = below_threshold != null ? below_threshold : stats.get_below_threshold();
			if (stats_below_threshold != null) {
				stats_data["below(" + stats_below_threshold + ")"] = stats.below(stats_below_threshold, fixed, unconverted);
			}
			var stats_above_threshold = above_threshold != null ? above_threshold : stats.get_above_threshold();
	 ...
	 * @see log_stats_by_id
	 */
	log: function (id, noise_threshold, below_threshold, above_threshold, fixed, unconverted) {
		if (id != null) {
	 ...
	 * @param {Boolean} [unconverted=false] whether to get the raw recorded values
	 */
	log_stats_by_id: function (id, noise_threshold, below_threshold, above_threshold, fixed, unconverted) {
		var stats = PERF.get_stats_by_id(id);
		if (stats != null) {
	...
	_start_tracking_draw_scene: function () {
		// track engine scene draw loop
		var drawScene_stats = PERF.stats_by_id["FPS"] = new Performance_Stats(
			// 10 minute sample
			60 * 60 * 10,
			// convert recorded delta time values to fps
			function (val) {
				return val > 0 ? 1000.0 / val : 0.0;
			},
		...
		var drawScene_method = PERF._drawScene_method = cc.Director.prototype.drawScene;
		var drawScene_timestamp;
		var drawScene_wrapper = function () {
	...
	_stop_tracking_draw_scene: function () {
	...
	_start_tracking_loads: function () {
		// setup stats
		var load_major_nongame_stats = PERF.stats_by_id["Load out of Game (s)"] = new Performance_Stats(20, function (val) { return val / 1000.0; });
		var load_major_game_stats = PERF.stats_by_id["Load into Game (s)"] = new Performance_Stats(20, function (val) { return val / 1000.0; });
		var load_minor_game_stats = PERF.stats_by_id["Load during Game (ms)"] = new Performance_Stats(1000);

		// reference original methods
		var method_loadMajorPackage = PERF._method_loadMajorPackage = PackageManager.getInstance().loadMajorPackage;
		var method_activateLoadingMajorPackage = PERF._method_activateLoadingMajorPackage = PackageManager.getInstance().activateLoadingMajorPackage;
		var method_unloadUnusedMajorMinorPackages = PERF._method_unloadUnusedMajorMinorPackages = PackageManager.getInstance().unloadUnusedMajorMinorPackages;
		var method_loadMinorPackage = PERF._method_loadMinorPackage = PackageManager.getInstance().loadMinorPackage;

		// replace original methods
		var load_major_id;
		var load_major_timestamp;
		var load_major_promise;
		var activate_major_promise;
		var unload_major_promise;
		var on_major_promises_present = function () {
			if (load_major_id != null && load_major_promise != null && activate_major_promise != null && unload_major_promise != null) {
				Promise.all([
					...
					var time_delta = performance.now() - load_major_timestamp;
		...
		PackageManager.getInstance().loadMinorPackage = function (id, resources, majorId) {
			...
			var game_layer = scene && scene.getGameLayer();
			if (game_layer != null && game_layer.getStatus() !== GameLayer.STATUS.DISABLED) {
				var load_minor_timestamp = performance.now();
				return method_loadMinorPackage.call(PackageManager.getInstance(), id, resources, majorId).then(function () {
	...
	_start_tracking_network: function () {
		// setup stats
		var action_response_stats = PERF.stats_by_id["Server Action Response (ms)"] = new Performance_Stats(1000);

		// reference original methods
		var method_broadcastGameEvent = PERF._method_broadcastGameEvent = SDK.NetworkManager.getInstance().broadcastGameEvent;
		...
		SDK.NetworkManager.getInstance().broadcastGameEvent = function (eventData) {
			...
			// call original method
			var validForBroadcast = method_broadcastGameEvent.call(SDK.NetworkManager.getInstance(), eventData);
	...
	_on_game_event: function (eventData) {
	...
	_stop_tracking_network: function () {
...
Performance_Stats.prototype = {
	...
	_converter: function (val) { return val; },
	 ...
	 */
	set_converter: function (converter) {
	 ...
	 * @param {Number} val
	 */
	set_noise_threshold: function (val) {
		if (_.isNumber(val)) {
			this._noise_threshold = val;
		}
	},
	get_noise_threshold: function () {
	 ...
	 * @param {Number} val
	 */
	set_below_threshold: function (val) {
		if (_.isNumber(val)) {
			this._below_threshold = val;
		}
	},
	get_below_threshold: function () {
	 ...
	 * @param {Number} val
	 */
	set_above_threshold: function (val) {
		if (_.isNumber(val)) {
			this._above_threshold = val;
		}
	},
	get_above_threshold: function () {
	 ...
	 * @returns {Array}
	 */
	get_recorded_values: function (unconverted) {
		var result = [];
		var num_records = this._num_records;

		if (num_records > 0) {
			var buffer_interface = this._buffer_interface;
			if (unconverted) {
				// raw
	 ...
	 * @returns {Number}
	 */
	average: function (fixed, unconverted) {
		var recorded_values = this.get_recorded_values(unconverted);
		var result = 0;

		if (recorded_values.length > 0) {
			for (var i = 0, il = recorded_values.length; i < il; i++) {
	 ...
	 * @returns {Number}
	 */
	deviation: function (fixed, unconverted) {
		var recorded_values = this.get_recorded_values(unconverted);
		var result = 0;

		if (recorded_values.length > 0) {
			var average = this.average(unconverted);
			var squared_average = 0;
			for (var i = 0, il = recorded_values.length; i < il; i++) {
				squared_average += Math.pow(recorded_values[i] - average, 2);
			}
	 ...
	 * @returns {String}
	 */
	noise: function (threshold, fixed, unconverted) {
		var recorded_values = this.get_recorded_values(unconverted);
		var result = 0;
	 ...
	 * @returns {String}
	 */
	below: function (threshold, fixed, unconverted) {
		var recorded_values = this.get_recorded_values(unconverted);
		var result = 0;
	 ...
	 * @returns {String}
	 */
	above: function (threshold, fixed, unconverted) {
		var recorded_values = this.get_recorded_values(unconverted);
		var result = 0;
