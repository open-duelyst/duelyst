...
uniform vec3 u_colorBlackPoint;
uniform vec3 u_colorMidPoint;
...
void main() {
	...
	// overlay edges over existing texture
	vec4 finalColor = textureColor + edgeColor;

	// temp
	vec3 blackPoint = u_colorBlackPoint; //
	vec3 midPoint = u_colorMidPoint; //
	vec3 whitePoint = vec3(1.0);

	// colorize
	float desaturated = (finalColor.r + finalColor.g + finalColor.b) / 3.0;
	vec3 colorized = mix(blackPoint, midPoint, smoothstep(0.0, 0.7, desaturated));
	colorized = mix(colorized, whitePoint, smoothstep(0.7, 1.0, desaturated));

	// output final color
	gl_FragColor = vec4(colorized, finalColor.a);
}
