...
// this shader is very similar to RarityFlare just lower intensity. could be combined if params made into uniforms
...
void main() {
  float t = u_time;
  float phase = 0.75*u_phase;

  vec2 aspect = vec2(1.0); // vec2(u_texResolution.x / u_texResolution.y, 1.0);
  vec2 uv = v_texCoord;
  ...
  vec3 turbColor = 1.0 * pow(turb,3.0) * discColor * vec3(smoothstep(0.0,1.0,falloffVal-0.01));

  // add horizontal flaring
  vec2 horizontal = vec2(0.0, uv.y) * COMPLEXITY * 2.0;
  float turbH = getNoiseAbsFBMOctaves3(vec3(horizontal, t*0.1));
  turbColor += 0.5*pow(turbH,3.0)*discColor;
