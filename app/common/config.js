...
// background color
CONFIG.BACKGROUND_COLOR = {r: 0, g: 0, b: 0, a: 255};
...
CONFIG.CARD_METADATA_BARS_COLOR = {r: 20, g: 20, b: 20, a: 255};
...
CONFIG.PLAYER_CARD_GLOW_RAMP_FROM_COLOR = {r: 255, g: 255, b: 255, a: 255};
...
CONFIG.OPPONENT_CARD_GLOW_RAMP_FROM_COLOR = {r: 255, g: 175, b: 175, a: 255};
...
CONFIG.INSTRUCTIONAL_CARD_GLOW_RAMP_FROM_COLOR = {r: 255, g: 255, b: 255, a: 255};
...
CONFIG.INSTRUCTION_NODE_BACKGROUND_COLOR = {r: 0, g: 0, b: 0, a:255};
CONFIG.INSTRUCTION_NODE_OUTLINE_COLOR = {r: 255, g: 255, b: 255, a:255};
CONFIG.INSTRUCTION_NODE_CARROT_BACKGROUND_COLOR = {r: 255, g: 69, b: 0, a:255};
CONFIG.INSTRUCTION_NODE_CARROT_OUTLINE_COLOR = {r: 0, g: 0, b: 0, a:200};
...
CONFIG.CONTINUE_BG_COLOR = {r:19, g:19, b:49, a:128};
CONFIG.BUY_TEXT_COLOR = {r: 141, g: 253, b: 255};
CONFIG.BUY_BUTTON_TEXT_COLOR = {r: 255, g: 255, b: 255};
CONFIG.BUY_BG_COLOR = {r:19, g:19, b:49, a:128};
CONFIG.SEASON_BG_COLOR = {r:19, g:19, b:49, a:192};
...
CONFIG.ALL_BOARD_POSITIONS = (function() {
	var pattern = [];
	for (var x = 0; x < CONFIG.BOARDCOL; x++) {
		for (var y = 0; y < CONFIG.BOARDROW; y++) {
...
CONFIG.PATTERN_WHOLE_BOARD = (function() {
	var pattern = [];
	for (var x = -CONFIG.BOARDCOL; x < CONFIG.BOARDCOL; x++) {
		for (var y = -CONFIG.BOARDROW; y < CONFIG.BOARDROW; y++) {
			pattern.push({x: x, y: y});
		}
	}
	return pattern;
...
CONFIG.PATTERN_HALF_BOARD = (function() {
	var pattern = [];
	for (var x = 0; x < Math.floor(CONFIG.BOARDCOL/2); x++) {
		for (var y = 0; y < CONFIG.BOARDROW; y++) {
			pattern.push({x: x, y: y});
		}
	}
...
CONFIG.PATTERN_WHOLE_ROW = (function() {
	var pattern = [];
	for (var x = -Math.floor(CONFIG.BOARDCOL/2); x < Math.ceil(CONFIG.BOARDCOL/2); x++) {
		pattern.push({
			x: x,
			y: 0
		});
...
CONFIG.PATTERN_WHOLE_COLUMN = (function() {
  var pattern = [];
  for (var y = -Math.floor(CONFIG.BOARDROW/2); y < Math.ceil(CONFIG.BOARDROW/2); y++) {
    pattern.push({
      x: 0,
      y: y
    });
...
CONFIG.PATTERN_BLAST = (function() {
	...
	for (var i = 0, il = pattern.length; i < il; i++) {
		...
		var unique = true;
		for (var j = 0, jl = finalPattern.length; j < jl; j++) {
...
CONFIG.getGlobalScaleForResolution = function (resolution, width, height) {
	...
	if (resolution !== CONFIG.RESOLUTION_PIXEL_PERFECT) {
		...
		globalScale = globalScale - globalScaleMult;
