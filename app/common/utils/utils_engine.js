...
UtilsEngine.transformBoardToTileMap = function(boardPoint) {
	var screenPoint = UtilsEngine.transformBoardToScreen(boardPoint);
...
UtilsEngine.projectPoint = function (point, projectionMatrix, zMatrix) {
	var mat = projectionMatrix.mat;
	var winCenterPosition = UtilsEngine.getGSIWinCenterPosition();

	// FIXME: this is still pretty hacky
	var x = point.x - winCenterPosition.x;
	var y = point.y - winCenterPosition.y;
	var d = 1.0 / ( mat[3] * x + mat[7] * y + mat[15] );
	var xp;
	var yp;
	if (zMatrix) {
		var zMat = zMatrix.mat;
		var z = zMat[2] * x + zMat[6] * y + zMat[14] - zMat[15];
	...
	// scale
	var xs = xp * winCenterPosition.x + winCenterPosition.x;
	var ys = yp * winCenterPosition.y + winCenterPosition.y;
	return cc.p(xs, ys);
};
...
UtilsEngine.getSpriteFrameFromIdentifier = function (identifier) {
	...
	if(identifier) {
		var animation = cc.animationCache.getAnimation(identifier);
	...
	if (spriteIdentifier != null) {
		...
		if(animation instanceof cc.Animation) {
			...
			if (spriteFrame instanceof cc.SpriteFrame) {
				animateAction = cc.callFunc(function (target) {
...
UtilsEngine.getNodeUnderMouse = function (node, screenX, screenY) {
	if (node.getNodeUnderMouse != null) {
		...
		var renderCmd = node._renderCmd;
		var stackMat = renderCmd._stackMatrix.mat;
		var x = stackMat[12] + node.centerOffset.x * 0.5;
		var y = stackMat[13] + node.centerOffset.y * 0.5;
		if (renderCmd.getNeedsPerspectiveProjection()) {
			x += cc.winSize.width * 0.5;
			y += cc.winSize.height * 0.5;
		}
		var width = node._contentSize.width * stackMat[0];
		var height = node._contentSize.height * stackMat[5];
...
UtilsEngine.getNodeCenterScreenPosition = function (node) {
	var worldPosition = UtilsEngine.getNodeScreenPosition(node);
	var stackMat = node._renderCmd._stackMatrix.mat;
...
UtilsEngine.getGameSpeed = function () {
	var gameSpeed = parseFloat(CONFIG.gameSpeed);
	if (isNaN(gameSpeed)) { gameSpeed = 1.0; }
...
UtilsEngine.rebuild = function() {
	...
	var eye = cc.kmVec3Fill(null, cc.winSize.width / 2, cc.winSize.height / 2, zeye);
	var center = cc.kmVec3Fill(null, cc.winSize.width / 2, cc.winSize.height / 2, 0.0);
	...
	// hand
	var cardsExpandX = Math.min(100.0, (UtilsEngine.getGSIWinWidth() - CONFIG.REF_WINDOW_SIZE.width) * 0.125);
	var cardsExpandY = Math.min(100.0, (UtilsEngine.getGSIWinHeight() - CONFIG.REF_WINDOW_SIZE.height) * 0.35);
	...
	// player frames
	var playerFramesExpandX = Math.min(100.0, (UtilsEngine.getGSIWinWidth() - CONFIG.REF_WINDOW_SIZE.width) * 0.25);
	var playerFramesExpandY = Math.min(100.0, (UtilsEngine.getGSIWinHeight() - CONFIG.REF_WINDOW_SIZE.height) * 0.25);
	...
	// player 1 artifacts
	var artifactLayoutSize = CONFIG.ARTIFACT_SIZE * 0.875;
	UtilsEngine._player1ArtifactsPositions = [];
	var player1ArtifactsPosition = cc.p(
		UtilsEngine._player1FramePosition.x + 38.0,
		UtilsEngine._player1FramePosition.y - 195.0
	);
	var x = player1ArtifactsPosition.x;
	var y = player1ArtifactsPosition.y;
	for (var i = 0, il = CONFIG.MAX_ARTIFACTS; i < il; i++) {
	...
	var player2ArtifactsPosition = cc.p(
		cc.winSize.width - player1ArtifactsPosition.x,
		player1ArtifactsPosition.y
	);
	var x = player2ArtifactsPosition.x;
	var y = player2ArtifactsPosition.y;
	for (var i = 0, il = CONFIG.MAX_ARTIFACTS; i < il; i++) {
	...
	for (var i = 0, il = UtilsEngine._player1ArtifactsPositions.length; i < il; i++) {
	...
	for (var i = 0, il = UtilsEngine._player2ArtifactsPositions.length; i < il; i++) {
	...
	if (CONFIG.XYZ_ROTATION.x !== 0.0 || CONFIG.XYZ_ROTATION.y !== 0.0 || CONFIG.XYZ_ROTATION.z !== 0.0) {
		...
		var xyzRotationMatrix = cc.kmMat4RotationPitchYawRoll(
			new cc.kmMat4(),
		...
		var xyzRotationMatrixInv = cc.kmMat4Inverse(new cc.kmMat4(), xyzRotationMatrix) || UtilsEngine.MAT4_IDENTITY;
		var transformMatrix = cc.kmMat4Multiply(new cc.kmMat4(), UtilsEngine.MAT4_PERSPECTIVE_STACK, xyzRotationMatrix);
		var transformMatrixInv = cc.kmMat4Multiply(new cc.kmMat4(), UtilsEngine.MAT4_PERSPECTIVE_STACK, xyzRotationMatrixInv);
