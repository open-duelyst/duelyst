...
cc.ActionInterval.prototype.setSpeedModifier = function (val) {
	...
	if (this._speedModifier !== val) {
		var lastSpeedModifier = this._speedModifier;
...
cc.ActionManager.prototype.setAllActionsSpeedModifiers = function (val) {
	...
	var actionTargets = this._arrayTargets;
	for (var i = 0; i < actionTargets.length; i++) {
		var actionTarget = actionTargets[i];
		var actions = actionTarget.actions;
...
cc.ControlButton.prototype.ctor = function(){
	...
	var cascadeOpacityEnabled = cc.ControlButton.prototype._cascadeOpacityEnabled;
...
cc.audioEngine.stopAllEffects = function(){
	...
	var poolKeys = Object.keys(ap);
	for (var i = 0, il = poolKeys.length; i < il; i++) {
