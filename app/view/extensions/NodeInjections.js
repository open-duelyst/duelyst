...
cc.Node.prototype.fadeTo = function (duration, opacity, callback) {
	...
	if(typeof duration === "number" && duration > 0.0) {
		var fadeAction = cc.fadeTo( duration, opacity );
...
cc.Node.prototype.fadeToInvisible = function (duration, callback) {
	if (this.isVisible()) {
		this.stopFadeTo();
		if(typeof duration === "number" && duration > 0.0) {
			var fadeAction = cc.fadeTo( duration, 0 );
			if (this._autoFadeEasing != null) {
				fadeAction = this._autoFadeEasing.create(fadeAction);
			}
			var sequenceActions = [
			...
			var makeInvisibleAction = cc.sequence(sequenceActions);
...
cc.Node.prototype.startPulsingOpacity = function (duration, opacityMin, opacityMax) {
	...
	var actionA = cc.fadeTo(duration, opacityMin).easing(cc.easeSineInOut());
	var actionB = cc.fadeTo(duration, opacityMax).easing(cc.easeSineInOut());
	if (this.getOpacity() != opacityMax) {
		var temp = actionA;
		actionA = actionB;
		actionB = temp;
	}
	var pulseAction = cc.sequence(actionA, actionB).repeatForever();
...
cc.Node.prototype.startPulsingScale = function (duration, scale) {
	...
	var scaleActionA = cc.EaseSineIn.create( cc.scaleBy( duration, scale ) );
	var scaleActionB = cc.EaseSineOut.create( cc.scaleTo( duration, 1.0 ) );
	if (this.getScale() != 1.0) {
		var temp = scaleActionA;
		scaleActionA = scaleActionB;
		scaleActionB = temp;
	}
	var pulseAction = cc.sequence( scaleActionA, scaleActionB).repeatForever();
	pulseAction.setTag(CONFIG.PULSE_TAG);
	this.runAction(pulseAction);
};
...
cc.Node.prototype.startRotating = function (duration, rotation, easing) {
	...
	var rotateAction = cc.rotateBy(duration, rotation);
	if (easing != null) {
...
cc.Node.prototype.destroy = function (duration) {
	if (this.isRunning()) {
		if (typeof duration === "number") {
			this.stopActionByTag(CONFIG.FADE_TAG);
			var fadeAction = cc.fadeTo(duration, 0.0);
			if (this._autoFadeEasing != null) {
				fadeAction = this._autoFadeEasing.create(fadeAction);
			}
			var destroyAction = cc.sequence(
				fadeAction,
...
cc.Node.prototype.updateAutoZOrder = function () {
	...
	var zOrder = this.getAutoZOrderValue();
...
cc.Node.prototype.ctor = function(){
	...
	var cascadeOpacityEnabled = cc.Node.prototype._cascadeOpacityEnabled;
	var cascadeColorEnabled = cc.Node.prototype._cascadeColorEnabled;
...
cc.Node.prototype.addGeneratedVisualStateTag = function(newVisualStateTag) {
	...
	for (i=0; i<this._generatedVisualStateTags.length; i++) {
		var currentTag = this._generatedVisualStateTags[i];
...
cc.Node.prototype.addInjectedVisualStateTagWithId = function(newVisualStateTag, id) {
	...
	for (var i = 0, il = this.injectedVisualStateTags.length; i < il; i++) {
		var currentTag = this.injectedVisualStateTags[i];
...
cc.Node.prototype._determineActiveTagsByResourcesNeeded = function() {
	...
	for (var i = this.injectedVisualStateTags.length - 1; i >= 0; i--) {
		var currentTag = this.injectedVisualStateTags[i];
		// A tag is active if all resources it needs are available
		var shouldBeActive = _.intersection(currentTag.neededResources,usedResources).length == 0;
...
cc.Node.prototype.setXYZRotation = function (rotation) {
	...
	var rotX = (rotation.x || 0.0) % 360.0;
	var rotY = (rotation.y || 0.0) % 360.0;
	var rotZ = (rotation.z || 0.0) % 360.0;
...
cc.Node.WebGLRenderCmd.prototype.getIsShowingBackside = function () {
	var xyzRotation = this._node.getCompositeXYZRotation();
	if (xyzRotation.x !== 0.0 || xyzRotation.y !== 0.0 || xyzRotation.z !== 0.0) {
		var xyzRotationMatrix = this._xyzRotationMatrix;
...
cc.Node.WebGLRenderCmd.prototype._rebuildXYZRotationMatrix = function () {
	...
	var node = this._node;
	var xyzRotation = node.getCompositeXYZRotation();
	var rotX = xyzRotation.x;
	var rotY = xyzRotation.y;
	var rotZ = xyzRotation.z;
...
cc.Node.WebGLRenderCmd.prototype.getNodeToParentTransform = function () {
	var node = this._node;
	var camera = node._camera;
	var cameraDirty = camera && camera._dirty;
	var transformDirty = (this._dirtyFlag & cc.Node._dirtyFlags.transformDirty) || cameraDirty;
	var t4x4 = this._transform4x4;
	...
	if (transformDirty) {
		...
		var needsCamera = camera && !(node.grid != null && node.grid.isActive());
		var needsRotation = this.getNeedsXYZRotation();

		// correct the rotation around the anchor point
		var translate = (needsCamera || needsRotation) && (apx !== 0.0 || apy !== 0.0);
		if (translate) {
...
cc.Node.prototype.setPositionAboveSprite = function (relativeSprite,offset,scaledOffset) {
	...
	this.setPosition(
		...
		relativeSprite.getPositionY() + relativeSprite.getBoundingBox().height * .5 + boundingBox.height * .5 + totalOffset.y
	)
};
...
cc.Node.prototype.setPositionLeftOfSprite = function (relativeSprite,offset,scaledOffset) {
	...
	this.setPosition(
		relativeSprite.getPositionX() - relativeSprite.getBoundingBox().width * .5 - boundingBox.height *.5 + totalOffset.x,
