...
cc.rendererWebGL.rendering = function (ctx) {
	...
	if (scene != null) {
		...
		for (var i = 0, len = renderCmds.length; i < len; i++) {
			var renderCmd = renderCmds[i];
...
cc.rendererWebGL._renderingToBuffer = function (renderTextureId) {
	...
	var renderCmds = this._cacheToBufferCmds[renderTextureId];
...
cc.Node.WebGLRenderCmd.prototype.updateMatricesForRender = function () {
	...
	if (!this.getNeedsSubPixelPosition()) {
		var stackMat4 = this._stackMatrix.mat;
...
cc.Sprite.WebGLRenderCmd.prototype.updateTransform = function () {
	...
	if (this._dirty) {
		var locQuad = this._quad, locParent = node._parent;
...
cc.Node.prototype.setActionsUsingSubPixelPosition = function (val) {
	if (this.actionsUsingSubPixelPosition != val) {
		var lastVal = this.actionsUsingSubPixelPosition;
...
cc.Node.WebGLRenderCmd.prototype.transform = function (parentCmd, recursive) {
	var transformDirty = this._dirtyFlag & cc.Node._dirtyFlags.transformDirty;
	...
	var renderingToBuffer = cc.rendererWebGL._isCacheToBufferOn;
	var parentNeedsPerspectiveProjection = parentCmd != null && _.isFunction(parentCmd.getNeedsPerspectiveProjection) ? parentCmd.getNeedsPerspectiveProjection() : (!renderingToBuffer && !CONFIG.DYNAMIC_PROJECTION);
	this._needsPerspectiveProjection = parentNeedsPerspectiveProjection || this.getNeedsPerspectiveProjectionForCache();

	var parentNeedsSubPixelPosition = parentCmd != null && _.isFunction(parentCmd.getNeedsSubPixelPosition) ? parentCmd.getNeedsSubPixelPosition() : cc.Node.prototype.usesSubPixelPosition;
...
cc.Sprite.WebGLRenderCmd.prototype._setTextureCoords = function () {
	var node = this._node;
	var texture = node._batchNode != null ? node.textureAtlas.texture : node._texture;
	if (texture == null) {
...
cc.Sprite.WebGLRenderCmd.prototype.flipTextureCoords = function () {
	...
	var blV = locQuad.bl.texCoords.v;
	var brV = locQuad.br.texCoords.v;
	var tlV = locQuad.tl.texCoords.v;
	var trV = locQuad.tr.texCoords.v;
...
cc.LabelTTF.WebGLRenderCmd.prototype._updateTexture = function () {
	...
	var textureWidth = Math.ceil(width * CONFIG.pixelScaleEngine);
	var textureHeight = Math.ceil(height * CONFIG.pixelScaleEngine);
