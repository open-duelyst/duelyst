...
var RenderPass = cc.Class.extend({
	...
	ctor: function (format, width, height, scale, antiAlias, depthStencilFormat) {
	 ...
	 */
	rebuild: function (format, width, height, scale, antiAlias, depthStencilFormat) {
		...
		var textureWidth = Math.ceil(this.width * devicePixelRatio);
		var textureHeight = Math.ceil(this.height * devicePixelRatio);
		this.antiAlias = antiAlias;

		var gl = cc._renderContext;
		var frameBufferLast = gl.getParameter(gl.FRAMEBUFFER_BINDING);
		...
		var quad = this._quad = new cc.V3F_C4B_T2F_Quad();
		...
		var zeyePass = (this.height / 2.0 / Math.tan(Math.PI / 6.0));
		...
		var eye = cc.kmVec3Fill(null, this.width / 2, this.height / 2, zeyePass);
		var center = cc.kmVec3Fill(null, this.width / 2, this.height / 2, 0.0);
	...
	getQuadDirty: function () {
		return this._quadDirty;
	},
	setQuadDirty: function () {
	...
	getColor: function () {
		...
		return {r: this._quad.tl.colors.r, g: this._quad.tl.colors.g, b: this._quad.tl.colors.b, a: this._quad.tl.colors.a};
	},
	 ...
	 */
	beginWithClear: function(r, g, b, a, stencilValue) {
		this.begin();
	 ...
	 * @param [stencilValue = 0x0]
	 */
	beginWithResetClear: function(stackId, r, g, b, a, stencilValue) {
	 ...
	 */
	beginWithReset: function(stackId) {
		this.begin();
		RenderPass.push_to_reset_stack(this, stackId);
	},
	/**
	 ...
	 * @param [stencil = 0x0]
	 */
	clear: function(r, g, b, a, stencil) {
		var gl = cc._renderContext;
		r = r ? r / 255.0 : 0.0;
		g = g ? g / 255.0 : 0.0;
		b = b ? b / 255.0 : 0.0;
	...
	visitNode: function(node) {
		...
		var ox = (position.x || 0);
		var oy = (position.y || 0);
		var ax = this.width * (anchorPoint.x || 0);
		var ay = this.height * (anchorPoint.y || 0);
		var stackMat = this._stackMatrix.mat;
		stackMat[5] = -1.0;
		stackMat[12] = ax - ox;
...
RenderPass.push_to_reset_stack = function (renderPass, stackId) {
	...
	var resetStack = RenderPass._reset_stacks_by_id[stackId];
	if (resetStack == null) {
...
RenderPass.pop_from_reset_stack = function (stackId) {
	...
	if (resetStack != null && resetStack.length > 0) {
		...
		if (RenderPass._reset_stacks.length > 0) {
			var nextResetStack = RenderPass._reset_stacks[RenderPass._reset_stacks.length - 1];
...
RenderPass.get_top_of_reset_stack = function (stackId) {
	...
	if (stackId != null) {
		...
		var reset_stacks = RenderPass._reset_stacks;
