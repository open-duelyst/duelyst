...
var BatchLights = require('./BatchLights');
var FXShockWaveSprite = require('app/view/nodes/fx/FXShockwaveSprite');
var TweenTypes = require('app/view/actions/TweenTypes');
var GradientColorMap = require('app/view/actions/GradientColorMap');
var BlurShaderGenerator = require('app/shaders/helpers/blurShaderGenerator');

/****************************************************************************
 FX
 ****************************************************************************/

var FX = cc.Class.extend({
	...
	_initShaderUniforms: function () {
		...
		var glowNoiseProgram = shaderCache.programForKey("GlowNoise");
		...
		var glowProgram = shaderCache.programForKey("Glow");
		glowProgram.use();
		...
		var highlightProgram = shaderCache.programForKey("Highlight");
		highlightProgram.use();
		highlightProgram.setUniformLocationWith4f(highlightProgram.loc_color, 1.0, 1.0, 1.0, 1.0);

		var lensFlareProgram = shaderCache.programForKey("LensFlare");
		...
		var wispFlareProgram = shaderCache.programForKey("WispLensFlare");
		...
		var lightingProgram = shaderCache.programForKey("Lighting");
		lightingProgram.use();
		...
		var shadowLowQualityProgram = shaderCache.programForKey("ShadowLowQuality");
		...
		var shadowHighQualityProgram = shaderCache.programForKey("ShadowHighQuality");
		...
		var energyBallProgram = shaderCache.programForKey("EnergyBall");
		...
		var fireRingProgram = shaderCache.programForKey("FireRing");
		fireRingProgram.use();
		...
		var whiteCloudVignetteProgram = shaderCache.programForKey("WhiteCloudVignette");
		...
		var toneCurveProgram = shaderCache.programForKey("ToneCurve");
		toneCurveProgram.use();
		toneCurveProgram.setUniformLocationWith1f(toneCurveProgram.loc_amount, this.getToneCurveAmount());

		var glowImageMapControlProgram = shaderCache.programForKey("GlowImageMapControl");
		...
		var glowImageMapRippleProgram = shaderCache.programForKey("GlowImageMapRipple");
		...
		var cardAngleGradientShineProgram = shaderCache.programForKey("CardAngledGradientShine");
	 ...
	 */
	updateShaderResolutionUniforms: function () {
		var shaderCache = cc.shaderCache;

		var depthProgram = shaderCache.programForKey("Depth");
		depthProgram.use();
		depthProgram.setUniformLocationWith2f(depthProgram.loc_resolution, cc.winSize.width, cc.winSize.height);

		var depthTestProgram = shaderCache.programForKey("DepthTest");
		...
		var fxaaProgram = shaderCache.programForKey("FXAA");
		fxaaProgram.use();
		...
		var chromaticProgram = shaderCache.programForKey("Chromatic");
		...
		var distortionProgram = shaderCache.programForKey("Distortion");
		...
		var waterProgram = shaderCache.programForKey("Water");
		waterProgram.use();
		waterProgram.setUniformLocationWith2f(waterProgram.loc_resolution, cc.winSize.width, cc.winSize.height);

		var shockwaveProgram = shaderCache.programForKey("Shockwave");
		...
		var vortexProgram = shaderCache.programForKey("Vortex");
		vortexProgram.use();
		vortexProgram.setUniformLocationWith2f(vortexProgram.loc_resolution, cc.winSize.width, cc.winSize.height);

		var fireWaveProgram = shaderCache.programForKey("FireLinearWave");
		fireWaveProgram.use();
	 ...
	 * @returns {RenderCmd}
	 */
	getBeginSurfaceCompositeRenderCmd: function () {
	 ...
	 * @returns {RenderCmd}
	 */
	getEndSurfaceCompositeRenderCmd: function () {
	 ...
	 * @returns {Number} global fx time
	 */
	getLoopingTimeForFrequency: function (frequency) {
	 ...
	 */
	getLoopingDirectionalTime: function () {
	 ...
	 */
	requestBlurScreen: function(id) {
		if (id != null) {
			if (!_.contains(this._blurScreenRequests, id)) {
				var numRequests = this._blurScreenRequests.length;
	 ...
	 * @param {String|Number|Integer} id
	 */
	requestUnblurScreen: function(id) {
		if (id != null) {
			var indexOf = _.lastIndexOf(this._blurScreenRequests, id);
			if (indexOf !== -1) {
				var numRequests = this._blurScreenRequests.length;
	 ...
	 * @param {String|Number|Integer} id
	 */
	requestBlurSurface: function (id) {
		if (id != null) {
			if (!_.contains(this._blurSurfaceRequests, id)) {
				var numRequests = this._blurSurfaceRequests.length;
	 ...
	 * @param {String|Number|Integer} id
	 */
	requestUnblurSurface: function (id) {
		if (id != null) {
			var indexOf = _.lastIndexOf(this._blurSurfaceRequests, id);
			if (indexOf !== -1) {
				var numRequests = this._blurSurfaceRequests.length;
	 ...
	 */
	requestStartScreenCache: function () {
	 ...
	 */
	requestStopScreenCache: function () {
		if (this._cacheScreenRequested) {
	 ...
	 */
	getScreenCacheRequested: function () {
	 ...
	 */
	getIsCachingScreen: function () {
		return this._cachingScreen;
	},

	_startCachingScreen: function () {
	 ...
	 */
	requestStartSurfaceCache: function () {
	 ...
	 */
	requestStopSurfaceCache: function () {
		if (this._cacheSurfaceRequested) {
	 ...
	 */
	getSurfaceCacheRequested: function () {
	 ...
	 */
	getIsCachingSurface: function () {
		return this._cachingSurface;
	},

	_startCachingSurface: function () {
	 ...
	 */
	addInstance: function (instancingId, instancedSprite) {
		var instances = this._instancesById[instancingId];
	 ...
	 * @param {cc.Sprite} instancedSprite
	 */
	removeInstance: function (instancingId, instancedSprite) {
		var instances = this._instancesById[instancingId];
		if (instances != null) {
			var index = _.indexOf(instances, instancedSprite);
			if (index != -1) {
	 ...
	 * @param {String} instancingId
	 */
	setInstancesDirty: function (instancingId) {
	 ...
	 * @returns {RenderPass}
	 */
	getInstancedRenderPass: function (instancingId) {
	 ...
	 * @returns {RenderPass}
	 */
	getInstancedTexture: function (instancingId) {
		var instancedRenderPass = this.getInstancedRenderPass(instancingId);
		return instancedRenderPass && instancedRenderPass.getTexture();
	},

	_renderForInstancing: function () {
		var instancingIds = Object.keys(this._instancesById);
		for (var i = 0, il = instancingIds.length; i < il; i++) {
			var instancingId = instancingIds[i];
			...
			var instances = this._instancesById[instancingId];
			var instancedRenderPass = this._instancedRenderPassesById[instancingId];
			if (instances != null && instancedRenderPass != null) {
				var instancedRenderMethod = instances[0].getRenderingForInstancing();
				instancedRenderMethod(this, instancingId, instancedRenderPass, instances)
			}
		}
	},
	_rebuildInstancedRenderPass: function (instancingId) {
	...
	_stopInstancing: function (instancingId) {
		var instances = this._instancesById[instancingId];
		if (instances != null) {
			for (var i = 0, il = instances.length; i < il; i++) {
	...
	_cleanupInstance: function (instancedSprite) {
	...
	getToneCurveAmount: function() {
		return this.toneCurveAmount;
	},

	setToneCurveAmount: function(val) {
	...
	setFalloffModifier: function(value) {
		this.falloffModifier = value;
	},
	setIntensityModifier: function(value) {
		this.intensityModifier = value;
	},
	setShadowIntensity: function (value) {
		this.shadowIntensity = value;

		var shadowLowQualityProgram = cc.shaderCache.programForKey("ShadowLowQuality");
		shadowLowQualityProgram.use();
		shadowLowQualityProgram.setUniformLocationWith1f(shadowLowQualityProgram.loc_intensity, this.shadowIntensity);

		var shadowHighQualityProgram = cc.shaderCache.programForKey("ShadowHighQuality");
		shadowHighQualityProgram.use();
		shadowHighQualityProgram.setUniformLocationWith1f(shadowHighQualityProgram.loc_intensity, this.shadowIntensity);
	},
	setShadowBlurShiftModifier: function (value) {
	...
	setShadowBlurIntensityModifier: function (value) {
	 ...
	 */
	setFromGradientColorMapWhiteColor: function(color) {
	 ...
	 * @param 		{cc.Color} 		color 	The color to set for this value.
	 */
	setFromGradientColorMapMidColor: function(color) {
	 ...
	 * @param 		{cc.Color} 		color 	The color to set for this value.
	 */
	setFromGradientColorMapBlackColor: function(color) {
	 ...
	 * @param 		{cc.Color} 		color 	The color to set for this value.
	 */
	setToGradientColorMapWhiteColor: function(color) {
	 ...
	 * @param 		{cc.Color} 		color 	The color to set for this value.
	 */
	setToGradientColorMapBlackColor: function(color) {
	 ...
	 * @param 		{cc.Color} 		color 	The color to set for this value.
	 */
	setToGradientColorMapMidColor: function(color) {
	 ...
	 */
	getToGradientColorMapWhiteColor: function() {
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getToGradientColorMapBlackColor: function() {
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getFromGradientColorMapWhiteColor: function() {
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getFromGradientColorMapBlackColor: function() {
	 ...
	 */
	setGradientMapTransitionPhase: function(phase) {
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getPhaseGradientColorMapWhiteColor: function() {
		var p = this._gradientMapTransitionPhase;
		var ps = p * p * (3.0 - 2.0 * p);
		var ips = 1.0 - ps;
		var fw = this._fromGradientColorMapWhiteColor;
		var tw = this._toGradientColorMapWhiteColor;
		return {
			r: fw.r * ips + tw.r * ps,
			g: fw.g * ips + tw.g * ps,
			b: fw.b * ips + tw.b * ps,
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getPhaseGradientColorMapMidColor: function() {
		var p = this._gradientMapTransitionPhase;
		var ps = p * p * (3.0 - 2.0 * p);
		var ips = 1.0 - ps;
		var fm = this._fromGradientColorMapMidColor;
		var tm = this._toGradientColorMapMidColor;
		return {
			r: fm.r * ips + tm.r * ps,
			g: fm.g * ips + tm.g * ps,
			b: fm.b * ips + tm.b * ps,
	 ...
	 * @return 		{cc.Color} 		color 	The color value.
	 */
	getPhaseGradientColorMapBlackColor: function() {
		var p = this._gradientMapTransitionPhase;
		var ps = p * p * (3.0 - 2.0 * p);
		var ips = 1.0 - ps;
		var fb = this._fromGradientColorMapBlackColor;
		var tb = this._toGradientColorMapBlackColor;
		return {
			r: fb.r * ips + tb.r * ps,
			g: fb.g * ips + tb.g * ps,
			b: fb.b * ips + tb.b * ps,
	 ...
	 */
	showGradientColorMap: function(id, duration, toColorWhite, toColorBlack, toColorMid) {
		if (duration == null) { duration = 0.0; }

		// stop current top of stack
		if (this._gradientMapStack.length > 0) {
			var currentGradientMapData = this._gradientMapStack[this._gradientMapStack.length - 1];
		...
		// get from colors
		var fromColorWhite = this.getPhaseGradientColorMapWhiteColor();
		var fromColorMid = this.getPhaseGradientColorMapMidColor();
		var fromColorBlack = this.getPhaseGradientColorMapBlackColor();

		// find gradient map data
		var gradientMapData;
		var gradientMapDataIndex = -1;
		for (var i = 0, il = this._gradientMapStack.length; i < il; i++) {
	 ...
	 * @param [duration=0.0]
   */
	clearGradientColorMap: function (id, duration) {
		if (this._gradientMapStack.length > 0) {
			...
			if (gradientMapDataIndex !== -1) {
				if (gradientMapDataIndex === this._gradientMapStack.length - 1) {
					...
					if (this._gradientMapStack.length > 0) {
						...
						var nextGradientMapData = this._gradientMapStack[this._gradientMapStack.length - 1];
	 ...
	 */
	addLight: function(light, suppress) {
	 ...
	 * @param {Boolean} suppress whether to suppress setting lights as dirty
	 */
	removeLight: function(light, suppress) {
	 ...
	 * @param {cc.Node} occluder occluder node
	 */
	removeOccluder: function(occluder) {
	 ...
	 */
	addShadowCaster: function(shadowCaster) {
	 ...
	 * @param {cc.Node} shadowCaster shadow casting node
	 */
	removeShadowCaster: function(shadowCaster) {
	...
	setLightsDirty: function() {
		...
		for (var i = 0, il = this.occluders.length; i < il; i++) {
	...
	setShadowCastingLightsDirty: function() {
		...
		for (var i = 0, il = this.shadowCasters.length; i < il; i++) {
			this.shadowCasters[i].setShadowCastingLightsDirty();
		}
	},
	getBatchLights: function () {
		return this.batchLights;
	},
	getBatchShadows: function () {
	 ...
	 */
	addDecal: function (decal) {
		var index = UtilsJavascript.arrayCautiousAdd(this.decals, decal);
		if(index === -1) {
	...
	removeDecal: function (decal) {
		var index = UtilsJavascript.arrayCautiousRemove(this.decals, decal);
		if(index !== -1) {
	 ...
	 */
	fadeNextDecal: function () {
		var decal = this.decals.shift();
		this.fadeDecal(decal);
	},
	 ...
	 */
	setMaxNumDecals: function (maxNumDecals) {
	 ...
	 */
	addDistortion: function (distortion) {
	 ...
	 * @param {cc.Node} distortion distortion node
	 */
	removeDistortion: function (distortion) {
	 ...
	 */
	setDistortionsDirty: function () {
		this._distortionsDirty = true;
	},

	_compareDistortions: function (a, b) {
	 ...
	 */
	beginForVisit: function () {
		// calculate whether gradient color map is needed this frame
		if (this._gradientMapDirty) {
			var pgw = this.getPhaseGradientColorMapWhiteColor();
			var pgb = this.getPhaseGradientColorMapBlackColor();
	 ...
	 */
	endWithComposite: function() {
		var gl = cc._renderContext;
		var shaderCache = cc.shaderCache;
		var needsCache = this.getScreenCacheRequested();
		...
		if (this._redirectingToScreenPass) {
			...
			if (this.getIsBlurringScreen()) {
				...
				var blurProgram = shaderCache.programForKey(this.screenBlurShaderProgramKey);
				blurProgram.use();
	 ...
	 */
	beginSurfaceWithClear: function() {
	...
	setRenderPassMatrices: function (shaderProgram) {
	 ...
	 */
	endSurfaceWithComposite: function() {
		var gl = cc._renderContext;
		var shaderCache = cc.shaderCache;
		var needsCache = this.getSurfaceCacheRequested();
		...
		if (this._redirectingToSurfacePass) {
			...
			var needsSurfaceBlur = this.getIsBlurringSurface();
			if (needsSurfaceBlur) {
			...
			var bloomTransition = this.getSurfaceCacheRequested() || this.getScreenCacheRequested() ? 1.0 : this.bloomTransition;
			...
			// setup for tone curve
			var toneCurveAmount = this.getToneCurveAmount();
			var needsToneCurve = toneCurveAmount > 0.0;
			if (needsToneCurve) {
				// redirect to tone curve pass
			...
			var needsRadialBlur = this.radialBlurStrength > 0.0 && this.radialBlurSpread > 0.0;
			if (needsRadialBlur) {
			...
			var surfaceProgram = shaderCache.programForKey("Surface");
			surfaceProgram.use();
			...
			if (CONFIG.DEBUG_DEPTH) {
				var depthDebugProgram = shaderCache.programForKey("DepthDebug");
			...
			if (distortions.length > 0) {
				...
				for (var i = 0, il = distortions.length; i < il; i++) {
					var distortion = distortions[i];
					var renderCmd = distortion._renderCmd;
			...
			if (needsRadialBlur) {
				...
				// radial blur
				var radialBlurProgram = shaderCache.programForKey("RadialBlur");
	...
	update: function (dt) {
		...
		var loopingTime = (this._loopingTimeOverflow || this.loopingTime) + dt;
		...
		var ldt = dt * this._loopingDirection;
		var loopingDirectionalTime;
		var loopingDirectionalTimeOverflow = this._loopingDirectionalTimeOverflow;
		this._loopingDirectionalTimeOverflow = 0.0;
...
FX.setup = function() {
	if(_needsSetup) {
		...
		var monochromeProgram = new cc.GLProgram();
		...
		var tintingProgram = new cc.GLProgram();
		...
		var depthProgram = new cc.GLProgram();
		...
		var depthTestProgram = new cc.GLProgram();
		...
		var maskProgram = new cc.GLProgram();
		...
		var fxaaProgram = new cc.GLProgram();
		...
		var depthDebugProgram = new cc.GLProgram();
		...
		var surfaceProgram = new cc.GLProgram();
		...
		var bloomProgram = new cc.GLProgram();
		...
		var highpassProgram = new cc.GLProgram();
		...
		var radialBlurProgram = new cc.GLProgram();
		...
		var glowNoiseProgram = new cc.GLProgram();
		...
		var glowProgram = new cc.GLProgram();
		...
		var highlightProgram = new cc.GLProgram();
		...
		var lensFlareProgram = new cc.GLProgram();
		...
		var wispLensFlareProgram = new cc.GLProgram();
		...
		var chromaticProgram = new cc.GLProgram();
		...
		*/
		...
		var chromaticFlareProgram = new cc.GLProgram();
		...
		var causticPrismaticGlowProgram = new cc.GLProgram();
		...
		var voronoiPrismaticProgram = new cc.GLProgram();
		...
		var lightingProgram = new cc.GLProgram();
		...
		var shadowLowQualityProgram = new cc.GLProgram();
		...
		var shadowHighQualityProgram = new cc.GLProgram();
		...
		var multipliedLightingProgram = new cc.GLProgram();
		...
		var distortionProgram = new cc.GLProgram();
		...
		var waterProgram = new cc.GLProgram();
		...
		var shockwaveProgram = new cc.GLProgram();
		...
		var vortexProgram = new cc.GLProgram();
		...
		var toneCurveProgram = new cc.GLProgram();
		...
		var gradientColorMapProgram = new cc.GLProgram();
		...
		var dissolveProgram = new cc.GLProgram();
		...
		var dissolveWithDiscFromCenterProgram = new cc.GLProgram();
		...
		var energyBallProgram = new cc.GLProgram();
		...
		var fireRingProgram = new cc.GLProgram();
		...
		var fireRingFlareWarpedProgram = new cc.GLProgram();
		...
		var lensNoiseProgram = new cc.GLProgram();
		...
		var whiteCloudVignetteProgram = new cc.GLProgram();
		...
		var fireLinearWaveShaderProgram = new cc.GLProgram();
		...
		var glowImageMapControlProgram = new cc.GLProgram();
		...
		var glowImageMapRippleProgram = new cc.GLProgram();
		...
		var fbmPolarFlareShaderProgram = new cc.GLProgram();
		...
		var fbmNoiseRaysShaderProgram = new cc.GLProgram();
		...
		var fbmNoiseGradientMaskProgram = new cc.GLProgram();
		...
		var levelsShaderProgram = new cc.GLProgram();
		...
		var cardAngleGradientShineProgram = new cc.GLProgram();
		...
		var colorizeProgram = new cc.GLProgram();
		...
		var rarityFlareProgram = new cc.GLProgram();
		...
		var horizontalGlowFlare = new cc.GLProgram();
		...
		var shadowBlobProgram = new cc.GLProgram();
		...
		var coreGemProgram = new cc.GLProgram();
		...
		var coreGemEdgesAndColorizeFragmentProgram = new cc.GLProgram();
		...
		var polarFlareWipeProgram = new cc.GLProgram();
		...
		var timerProgram = new cc.GLProgram();
		...
		var causticProgram = new cc.GLProgram();
		...
		var riftLineProgram = new cc.GLProgram();
		...
		var riftFireProgram = new cc.GLProgram();
...
SurfaceCompositeRenderCmd.prototype = {
	...
	getNeedsPerspectiveProjection: function () {
		return false;
	}
};
