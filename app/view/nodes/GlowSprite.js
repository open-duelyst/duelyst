...
var GlowSprite = BaseSprite.extend({
	...
	setIsLightSource: function (isLightSource, lightSourceOptions) {
	...
	setLightSourceOffset: function (lightSourceOffset) {
	...
	setGlowRampTransition: function (r, g, b, a) {
		if (arguments.length === 1) {
			var c = arguments[0];
			r = c.r;
			g = c.g;
	...
	setGlowColorTo: function (r, g, b, a) {
		if (arguments.length === 1) {
			var c = arguments[0];
			r = c.r;
			g = c.g;
	...
	setGlowNoiseExpandModifier: function (glowNoiseExpandModifier) {
		if(glowNoiseExpandModifier != null) {
			this.glowNoiseExpandModifier = glowNoiseExpandModifier;
		}
	},

	showGlowForAttackableTarget: function () {
	...
	showGlowForInstructional: function () {
	...
	setHighlightBlurStrength: function (highlightBlurStrength) {
	...
	fadeInGlow:function(duration, easing) {
		...
		if (needsFade) {
			...
			var tweenAction = cc.actionTween(duration, TweenTypes.GLOW_FADE, 0.0, 1.0);
			if (easing != null) {
	...
	fadeOutGlow:function(duration, easing) {
		var needsFade = this.glowing;
		if (this._glowFadeAction) {
			this.stopAction(this._glowFadeAction);
			this._glowFadeAction = null;
		...
		if (needsFade) {
			if (duration == null) { duration = CONFIG.FADE_FAST_DURATION; }
			var tweenAction = cc.actionTween(duration, TweenTypes.GLOW_FADE,1.0, 0.0);
			if (easing != null) {
				tweenAction = tweenAction.easing(easing);
			}
			this._glowFadeAction = cc.sequence(
	...
	fadeInHighlight:function(duration, easing) {
		var needsFade = !this.highlighted;
		...
		if (needsFade) {
			...
			var tweenAction = cc.actionTween(duration, TweenTypes.HIGHLIGHT_FADE, 0.0, 1.0);
			if (easing != null) {
				tweenAction = tweenAction.easing(easing);
			}
	...
	fadeOutHighlight:function(duration, easing) {
		var needsFade = this.highlighted;
		if (this._highlightFadeAction) {
			this.stopAction(this._highlightFadeAction);
			this._highlightFadeAction = null;
		...
		if (needsFade) {
			if (duration == null) { duration = CONFIG.FADE_MEDIUM_DURATION; }
			var tweenAction = cc.actionTween(duration, TweenTypes.HIGHLIGHT_FADE, 1.0, 0.0);
			if (easing != null) {
				tweenAction = tweenAction.easing(easing);
			}
			this._highlightFadeAction = cc.sequence(
...
var proto = GlowSprite.WebGLRenderCmd.prototype = Object.create(BaseSprite.WebGLRenderCmd.prototype);
...
proto.rebuildGlowHighlightPasses = function () {
	var node = this._node;
	var needsGlowHighlight = this.getNeedsGlowHighlight();
	...
	if (!needsGlowHighlight || width == null || width <= 0 || height == null || height <= 0) {
		...
		// calculate size
		var padding = Math.ceil(node.glowPadding * 0.5) * 2.0;
...
proto.rebuild = function () {
	...
	var needsGlowHighlight = this.getNeedsGlowHighlight();
	var isGlowingHighlighting = this.getIsGlowingHighlighting();
...
proto.drawGlow = function () {
	if (this.getIsGlowingHighlighting()) {
		...
		var glowProgram = cc.shaderCache.programForKey("Glow");
		glowProgram.use();
		...
		var glowNoiseProgram = cc.shaderCache.programForKey("GlowNoise");
...
proto.drawHighlight = function () {
	if (this.getIsGlowingHighlighting()) {
		...
		// draw highlight
		var highlightProgram = cc.shaderCache.programForKey("Highlight");
...
proto.drawPrismatic = function () {
	...
	var chromaticProgram = cc.shaderCache.programForKey("Chromatic");
