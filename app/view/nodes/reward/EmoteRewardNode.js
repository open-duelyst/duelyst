...
var EmoteRewardNode = RewardNode.extend({

	_emoteId: null,

	ctor: function (emoteId) {
		if (emoteId == null) {
	...
	getRequiredResources: function () {
		var emoteRewardResources = PKGS.getPkgForIdentifier("emote_reward");
		var emoteResources = this._emoteId != null ? SDK.CosmeticsFactory.cosmeticResourcesForIdentifier(this._emoteId) : [];
	...
	getRewardAnimationPromise: function (looping, showLabel) {
		...
		.then(function () {
			return new Promise(function (resolve) {
				// emote data
				var emoteData = SDK.CosmeticsFactory.cosmeticForIdentifier(this._emoteId);

				// emote sprite
				var emoteSprite = BaseSprite.create(emoteData.img);
				emoteSprite.setVisible(false);
				this.addChild(emoteSprite, 1);

				if (showLabel) {
					// primary label
					var labelText = _.isString(showLabel) ? showLabel : i18next.t("cosmetics.cosmetic_type_emote").toLocaleUpperCase();
					...
					// secondary label
					var rarityId = emoteData.rarityId;
					if (rarityId != null) {
