...
var CompositeHorizontalPass = require('app/view/nodes/components/CompositeHorizontalPass');

/****************************************************************************
 CoreGemSprite
 ****************************************************************************/

var CoreGemSprite = BaseSprite.extend({
	...
	ctor:function(){
		...
		this.whenRequiredResourcesReady().then(function (requestId) {
			...
			var noiseTexture = cc.textureCache.getTextureForKey(RSX.noise_128.img);
			this.setTexture(noiseTexture);

			// set custom shader
			var shaderProgram = cc.shaderCache.programForKey("CoreGem");
	...
	onBaseDrawSetAdditionalUniforms: function(shaderProgram) {
		...
		var cubemapTexture = cc.textureCache.getTextureForKey(RSX.core_gem_cubemap.name);
	 ...
	 */
	setHighlightsEdges: function (highlightsEdges) {
	...
	getHighlightsEdges: function () {
		return this.highlightsEdges;
	},
	createHighlightsEdgesComponent: function () {
	...
	setupHighlightsEdgesRender: function (shaderProgram) {
