...
var FXSprite = BaseSprite.extend({
  ...
  depthModifier: 0.0,
	...
	getImpactFX: function () {
		return this.impactFX;
	},
	getImpactFXSprites: function () {
	...
	getSourceOffsetScreenPosition: function () {
	...
	getTargetOffsetScreenPosition: function () {
	...
	getScreenPositionFrom: function (object) {
		if(object instanceof cc.Node) {
			return object.getPosition();
		} else if(object instanceof SDK.Unit) {
			return UtilsEngine.transformBoardToTileMap(object.getPosition());
		} else {
			return object;
		}
	},
	getBoardPositionFrom: function (object) {
		if(object instanceof cc.Node) {
	...
	setFadeLooping: function (fadeLooping) {
		this.fadeLooping = fadeLooping;
	},
	getFadeLooping: function () {
	...
	setRotationPerSecond: function (rotationPerSecond) {
		this.rotationPerSecond = rotationPerSecond;
	},
	setXYZRotationPerSecond: function (xyzRotationPerSecond) {
		this.xyzRotationPerSecond = xyzRotationPerSecond;
	},
	setPulseSmooth: function (pulseSmooth) {
		this.pulseSmooth = pulseSmooth;
	},
	setPulseScaleMin: function (pulseScaleMin) {
		this.pulseScaleMin = pulseScaleMin;
	},
	setPulseScaleMax: function (pulseScaleMax) {
	...
	getBaseDuration: function () {
		// base delay is max of duration and animation
		var duration = this.getDuration();
		var animAction = UtilsEngine.getAnimationAction(this.getSpriteIdentifier());
		if(animAction) {
	...
	getLifeDuration: function () {
		...
		var duration = this.getBaseDuration();
	...
	getShowDelay: function () {
		...
		var delay = this.getBaseDuration();
	...
	_calculateEmitFXTimings: function () {
		...
		var baseDuration = this.getBaseDuration();

		var fxSprites = this.getEmitFXSprites();
	...
	_calculateImpactFXTimings: function () {
		...
		var fxSprites = this.getImpactFXSprites();
		if(fxSprites && fxSprites.length > 0) {
			for (var i = 0, il = fxSprites.length; i < il; i++) {
				var fxSprite = fxSprites[i];
	...
	startAnimation: function () {
		...
		// main sequence
		var mainDuration = this.getLifeDuration();
		var mainSequenceSteps = [];
		mainSequenceSteps.push(cc.delayTime(mainDuration));

		// animation sequence
		var animAction = this.getActionForAnimationSequence();
		var animationSequenceSteps;
		...
		var maxDuration = Math.max(mainDuration, animationDuration, fadeDuration);

		// run animation
		if(animationSequenceSteps && animationSequenceSteps.length > 0) {
			if(this.looping && this.fadeLooping) {
				var animationSequenceAction = cc.sequence(animationSequenceSteps);
		...
		if(fadeSequenceSteps && fadeSequenceSteps.length > 0) {
			...
			var fadeSequenceAction = cc.sequence(fadeSequenceSteps);
		...
		if(mainSequenceSteps && mainSequenceSteps.length > 0) {
			if (this.looping) {
				var mainSequenceAction = cc.sequence(mainSequenceSteps);
	...
	startEmit: function () {
		var emitFXSprites = this.getEmitFXSprites();
		if(emitFXSprites && emitFXSprites.length > 0) {
			for(var i = 0, il = emitFXSprites.length; i < il; i++) {
				var fxSprite = emitFXSprites[i];

				// shift to center
				var fxSpritePosition = fxSprite.getPosition();
	...
	impact: function () {
		var impactFXSprites = this.getImpactFXSprites();
	...
	update: function (dt) {
		...
		if (this.pulseScaleMin !== this.pulseScaleMax) {
			var pulseTime = this.pulseSmooth ? this.getFX().getLoopingDirectionalTime() : this.getFX().getLoopingTime();
			this.setScale(this.pulseScaleMax * pulseTime + this.pulseScaleMin * (1.0 - pulseTime));

			var pulseFadeIn = this.pulseFadeIn;
			var pulseFadeOut = this.pulseFadeOut;
...
var proto = FXSprite.WebGLRenderCmd.prototype = Object.create(BaseSprite.WebGLRenderCmd.prototype);
