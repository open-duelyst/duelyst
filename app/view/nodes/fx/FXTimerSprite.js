...
var FXTimerSprite = BaseSprite.extend({
	...
	animateProgress: function (duration, progress) {
		if (this._targetProgress !== progress) {
			if (typeof duration === "number") {
				this._targetProgress = progress;
				this.stopActionByTag(CONFIG.ANIM_TAG);
				var progressAction = cc.actionTween(duration, "progress", this.progress, this._targetProgress);
	...
	setStartingAngle: function (val) {
		this.startingAngle = val;
	},

	setEdgeGradientFactor: function (val) {
...
var proto = FXTimerSprite.WebGLRenderCmd.prototype = Object.create(BaseSprite.WebGLRenderCmd.prototype);
