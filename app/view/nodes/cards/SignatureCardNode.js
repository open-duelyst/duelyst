...
var FXTimerSprite = require('./../fx/FXTimerSprite');
...
var SignatureCardNode = SdkNode.extend({
	...
	ctor: function (sdkCard) {
		...
		// cooldown label
		var cooldownLabelPosition = this.cooldownIconSprite.getPosition();
			...
			if (animResource) {
				...
				this.whenResourcesReady(this.getCardResourceRequestId()).then(function (cardResourceRequestId) {
					...
					if (this.cardSprite != null) {
						...
						this._showingCooldownAnimState = true;
						var animation = cc.animationCache.getAnimation(idleAnim);
						var frames = animation && animation.getFrames();
	...
	updateUsability: function () {
		if (this.sdkCard) {
			...
			var owner = this.sdkCard.getOwner();
		...
		if (this.highlighted !== highlighted) {
			...
			if (!this.selected) {
				if (!this.highlighted) {
					...
					var playable = this.sdkCard != null && this._usable && this.sdkCard.isOwnedByMyPlayer();
					if (playable) {
	...
	showCooldown: function (duration) {
		if (this.sdkCard) {
			if (duration == null) { duration = CONFIG.ANIMATE_MEDIUM_DURATION; }
			var owner = this.sdkCard.getOwner();
			if (!owner.getIsSignatureCardActive()) {
				// show timer sprite
				var progress = SDK.GameSession.getInstance().getProgressUntilPlayerActivatesSignatureCard(owner);
				var cooldownOpacity = CONFIG.SIGNATURE_CARD_COOLDOWN_TIMER_OPACITY * Math.max(0.5, Math.pow(progress, 0.5));
	...
	hideCooldown: function (duration) {
		if (this.sdkCard) {
			if (duration == null) { duration = CONFIG.ANIMATE_MEDIUM_DURATION; }
	...
	_animateCooldownLabelTo: function (duration) {
		var owner = this.sdkCard.getOwner();
		...
		if (this.cooldownLabel.getString() !== cooldown + "") {
			var cooldownLabelLast = this.cooldownLabel;
	 ...
	 */
	showDraw: function(sdkCard, showDelay) {
			...
			var drawAction = cc.sequence(
				...
				cc.callFunc(function () {
					...
					whenCardResourcesReadyPromise.then(function (cardResourceRequestId) {
						...
						var tintFadeDuration = 0.8;

						// fade sprite in
						this.cardSprite.setVisible(true);
						var subDrawAction = cc.sequence(
	 ...
	 * @returns {Number} show duration
	 */
	showActivate: function (showDelay) {
		var showDuration = 0.0;
	...
	updateSupportNodePositions: function () {
		if (this._instructionNode != null) {
			...
			if (this._instructionNode._carrotDirection == InstructionNode.DIRECTION_UP) {
				this._instructionNode.setPosition(position.x, position.y - this.getContentSize().height *.5);
				...
				this._instructionNode.setPosition(position.x - this.getContentSize().width *.5, position.y);
			} else {
				// assume left
				this._instructionNode.setPosition(position.x + this.getContentSize().width *.5, position.y);
			}
