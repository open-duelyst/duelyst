...
var XYZRotateTo = require('app/view/actions/XYZRotateTo');
var XYZRotateBy = require('app/view/actions/XYZRotateBy');
var SecondaryXYZRotateBy = require('app/view/actions/SecondaryXYZRotateBy');
var SecondaryXYZRotateTo = require('app/view/actions/SecondaryXYZRotateTo');
...
var CardNode = SdkNode.extend({
	...
	ctor: function (sdkCard) {
		...
		var cardBackgroundContentSize = this.getCardBackgroundContentSize();
		var contentSize = this.getCardContentSize();
	 ...
	 */
	setSdkCard: function(sdkCard, showBaseState, actionToShowStateFor, actionEventTypeToShowStateFor) {
		...
		if (this.sdkCard != sdkCard) {
			...
			if (sdkCard != null) {
				...
				.spread(function (requiredRequestId, cardResourceRequestId) {
					...
					if (sdkCard instanceof SDK.Entity) {
						...
						if (sdkCard instanceof SDK.Artifact) {
					...
					if (sdkCard instanceof SDK.Entity && sdkCard.getWasGeneral()) {
						...
						var referenceSignatureCard = sdkCard.getReferenceSignatureCard();
						if (referenceSignatureCard != null) {
							var signatureCardPosition = this._manaGemSprite.getPosition();
							...
							var signatureCardOptions = _.extend({}, referenceSignatureCard.getCardOptions());
				...
				// race and type
				var raceId = sdkCard.getRaceId();
				var race = SDK.RaceFactory.raceForIdentifier(raceId);
				var raceName = race && race.name;
				...
				// description
				var description = sdkCard.getDescription(CONFIG.FORMATTING_ENGINE)
				...
				// faction name
				var faction = SDK.FactionFactory.factionForIdentifier(sdkCard.getFactionId());
				var factionName = faction ? faction.name.toUpperCase() : "";
	...
	updateStats: function () {
		var stateAtAction = this.getStateForActionToShowStateFor();
		var manaCost = (stateAtAction != null && stateAtAction.manaCost != null ? stateAtAction.manaCost : (this.sdkCard && this.sdkCard.manaCost)) || 0;
		...
		if (this.sdkCard instanceof SDK.Entity) {
			...
			if (stateAtAction != null) {
				if (this.atkLabel.isVisible()) {
					var baseATK = stateAtAction.baseATK != null ? stateAtAction.baseATK : this.sdkCard.atk;
					if (atk < baseATK) {
	...
	getOutlineGlowBlurredSprite: function() {
	 ...
	 * @returns {Boolean}
	 */
	getNodeUnderMouse: function(screenX, screenY) {
		if (!this.whenRequiredResourcesReady().isFulfilled()) return false;

		var contentSize = this.cardBackgroundSprite._contentSize;
		var renderCmd = this._renderCmd;
		var stackMat = renderCmd._stackMatrix.mat;
		var scaleX = stackMat[0];
		var scaleY = stackMat[5];
		var x = stackMat[12] + this.centerOffset.x * 0.5 + CONFIG.CARD_PADDING * 0.5 * scaleX;
		var y = stackMat[13] + this.centerOffset.y * 0.5 + CONFIG.CARD_PADDING * 0.5 * scaleY;
	 ...
	 */
	getCardContentSizeWithBars: function () {
		var cardContentSize = this.getCardContentSize();
		var modifiersContentSize = this.getModifiersContentSize();
		var keywordsContentSize = this.getKeywordsContentSize();
	 ...
	 */
	getCardBackgroundContentSizeWithBars: function () {
		var cardContentSize = this.getCardBackgroundContentSize();
		var modifiersContentSize = this.getModifiersContentSize();
		var keywordsContentSize = this.getKeywordsContentSize();
		return cc.size(cardContentSize.width + keywordsContentSize.width, cardContentSize.height + modifiersContentSize.height)
	},
	 ...
	 */
	getModifiersContentSize: function () {
	 ...
	 */
	getKeywordsContentSize: function () {
	 ...
	 */
	getCardBackgroundContentSize: function () {
	 ...
	 */
	getCardBackgroundSpriteIdentifier: function () {
	 ...
	 */
	setActionToShowStateFor: function (val) {
	 ...
	 * @returns {Action|null}
	 */
	getActionToShowStateFor: function () {
	 ...
	 * @param {String} val
	 */
	setActionEventTypeToShowStateFor: function (val) {
	 ...
	 * @returns {Action|null}
	 */
	getActionEventTypeToShowStateFor: function () {
	 ...
	 * @returns {Action|null}
	 */
	getStateForActionToShowStateFor: function () {
	...
	getIsAnimationInProgress: function() {
	 ...
	 */
	bindCardBack: function () {
		// attempt to use this card's owner's card back
		var cardBackId;
		var ownerId = this.sdkCard && this.sdkCard.getOwnerId();
		var ownerSetupData = ownerId && SDK.GameSession.getInstance().getPlayerSetupDataForPlayerId(ownerId);
	 ...
	 */
	setCardBackId: function (cardBackId) {
		if (this._cardBackId !== cardBackId) {
			...
			if (this._cardBackId != null) {
				...
				var cardBackResourceRequestId = this._cardBackResourceRequestId = cardBackPkgId + "_" + UtilsJavascript.generateIncrementalId();

				// setup promise to wait for resources
				this.addResourceRequest(cardBackResourceRequestId, cardBackPkgId)
				.then(function (cardBackResourceRequestId) {
					if (!this.getAreResourcesValid(cardBackResourceRequestId)) return; // card back has changed

					// initialize card back sprites
					var cardBackData = SDK.CosmeticsFactory.cardBackForIdentifier(this._cardBackId);
	...
	getCardBackResourceRequestId: function () {
	...
	getModifiersShowing: function () {
		return this._modifiersNode.isVisible();
	},

	setModifiersDirty: function () {
		this._modifiersDirty = true;
	},

	getHasModifiers: function () {
		return this._hasModifers;
	},

	rebuildModifiers: function () {
		this._modifiersDirty = false;

		var modifiersBaseHeight = CONFIG.CARD_MODIFIER_PADDING_VERTICAL;
		var modifiersHeight = modifiersBaseHeight;
		var stateAtAction = this.getStateForActionToShowStateFor();
		if (stateAtAction != null) {
		...
		if (modifiersHeight > modifiersBaseHeight) {
			...
			var bgWidth = modifiersWidth + CONFIG.CARD_MODIFIER_PADDING_HORIZONTAL * 3.0;
			var bgHeight = modifiersHeight + CONFIG.CARD_MODIFIER_PADDING_VERTICAL;
			var bgBL = cc.p(0, 7);
			var bgTR = cc.p(bgBL.x + bgWidth, -(bgBL.y + bgHeight));
			var bgColor = cc.color(CONFIG.CARD_METADATA_BARS_COLOR);
	...
	_buildModifiers: function (modifierStacks, currentModifiersWidth, currentModifiersHeight) {
		var modifiersHeight = 0;

		if (modifierStacks && modifierStacks.length > 0) {
			// show modifier name and description for each stack type
			for (var i = 0; i < modifierStacks.length; i++) {
				var modifierStack = modifierStacks[i];
				var modifiersOfStackType = modifierStack.modifiers;

				// don't show stack when only contains inherent modifiers
				var numModifiersOfStackType = modifierStack.numActive - modifierStack.numInherent;
				if (numModifiersOfStackType > 0) {
					var modifier = modifiersOfStackType[0];
					...
					var modifierNameText = modifier.getAppliedName();
					if (numModifiersOfStackType > 1) {
						modifierNameText += " (x" + numModifiersOfStackType + ")";
					}
					var modifierNameLabel = new cc.LabelTTF(modifierNameText, RSX.font_bold.name, 14, cc.size(currentModifiersWidth, 0), cc.TEXT_ALIGNMENT_LEFT);
					...
					var modifierLabelContentSize = modifierNameLabel.getContentSize();
					...
					//if (parentModifier == null || parentModifier.getIsInherent() || modifier.getCardAffected() !== parentModifier.getCardAffected()) {
						var modifierDescription = modifier.getAppliedDescription();
						if (modifierDescription != null) {
							var modifierDescriptionLabel = new cc.LabelTTF(modifierDescription, RSX.font_light.name, 12, cc.size(currentModifiersWidth, 0), cc.TEXT_ALIGNMENT_LEFT);
							...
							var modifierDescriptionLabelContentSize = modifierDescriptionLabel.getContentSize();
	...
	getKeywordsShowing: function () {
		return this._keywordsNode.isVisible();
	},

	setKeywordsDirty: function () {
		this._keywordsDirty = true;
	},

	getKeywordsShowingOnLeft: function () {
		return this._keywordsShowingOnLeft;
	},

	getHasKeywords: function () {
	...
	rebuildKeywords: function () {
		...
		if (keywordClasses != null && keywordClasses.length > 0) {
			...
			var paddingLeft = CONFIG.CARD_KEYWORD_PADDING_HORIZONTAL + 5.0;
			var paddingRight = CONFIG.CARD_KEYWORD_PADDING_HORIZONTAL;
			var keywordsHeight = CONFIG.CARD_KEYWORD_PADDING_VERTICAL;
			var maxWidth = 0;
			for (var i = 0; i < keywordClasses.length; i++) {
				var keyword = keywordClasses[i];
				if (!keyword.isHiddenToUI) {
					var keywordName = keyword.getName();
					if (keywordName != null) {
						var keywordNameLabel = new cc.LabelTTF(keywordName, RSX.font_bold.name, 14, cc.size(CONFIG.CARD_KEYWORDS_WIDTH, 0), cc.TEXT_ALIGNMENT_LEFT);
						...
						var keywordLabelContentSize = keywordNameLabel.getContentSize();
						keywordsHeight += keywordLabelContentSize.height + CONFIG.CARD_KEYWORD_PADDING_VERTICAL;

						var keywordDescriptionLabel = new cc.LabelTTF(keyword.getKeywordDefinition(), RSX.font_light.name, 12, cc.size(CONFIG.CARD_KEYWORDS_WIDTH, 0), cc.TEXT_ALIGNMENT_LEFT);
						...
						var keywordDescriptionLabelContentSize = keywordDescriptionLabel.getContentSize();
			...
			// draw background rectangle for bar
			var bgWidth = maxWidth + paddingLeft + paddingRight;
			var bgHeight = keywordsHeight + CONFIG.CARD_KEYWORD_PADDING_VERTICAL;
			var bgBL = cc.p(0, CONFIG.CARD_KEYWORD_PADDING_VERTICAL);
			var bgTR = cc.p(bgBL.x + bgWidth, -(bgBL.y + bgHeight));
			var bgColor = cc.color(CONFIG.CARD_METADATA_BARS_COLOR.r, CONFIG.CARD_METADATA_BARS_COLOR.g, CONFIG.CARD_METADATA_BARS_COLOR.b, CONFIG.CARD_METADATA_BARS_COLOR.a);
	...
	showKeywords: function(keywordsOnLeft) {
		if (keywordsOnLeft) {
	...
	showActiveAnimState: function () {
		if (this.sdkCard != null) {
			var animResource = this.sdkCard.getAnimResource();
			if (animResource) {
				if (this.sdkCard instanceof SDK.Unit) {
	...
	fadeOutSnapshot: function (duration) {
	...
	showInspect: function (sdkCard, showBaseState, actionToShowStateFor, actionEventTypeToShowStateFor, keywordsOnLeft, highlighted, showInstructionals) {
		// reset
		if (sdkCard != null) {
	...
	stopShowingInspectAndClear: function () {
		this.stopShowingInspect();
		this.setSdkCard(null);
	},

	showInstructionals: function () {
	...
	showPlay: function (sdkCard, actionToShowStateFor, actionEventTypeToShowStateFor, sourceScreenPosition, targetScreenPosition, animateDuration, showDuration, noFlip, showInstructionals) {
		return new Promise(function (resolve, reject) {
			...
			if (this.sdkCard) {
				...
				var contentSize = this.getContentSize();
				var dx = targetScreenPosition.x - sourceScreenPosition.x;
				var dy = targetScreenPosition.y - sourceScreenPosition.y;
				var anchorX = dx < 0.0 ? 0.0 : 1.0;
				var offsetX = contentSize.width * (dx < 0.0 ? 0.5 : -0.5) * (noFlip ? -1.0 : 1.0);
				var anchorY = 0.0;//dy < 0.0 ? 0.0 : 1.0;
				var angRotX = 30;// * (dy < 0.0 ? -1 : 1);
				var angRotY = 60.0 * (dx < 0.0 ? -1 : 1);
				var angRotZ = 0;
				var sourceScreenPositionWithAnchor = cc.p(sourceScreenPosition.x + offsetX * 0.5, sourceScreenPosition.y + (anchorY - 0.5) * contentSize.height * 0.5);
				var targetScreenPositionWithAnchor = cc.p(targetScreenPosition.x + offsetX, targetScreenPosition.y + (anchorY - 0.5) * contentSize.height);
				...
				if (noFlip) {
					...
					.spread(function (requiredRequestId, cardResourceRequestId, cardBackResourceRequestId) {
				...
				var showDurationAction = cc.sequence(
	...
	stopShowingPlay: function (animateDuration) {
		return new Promise(function (resolve, reject) {
			if (animateDuration == null) {animateDuration = 0.0;}
			this.stopAnimations();
			...
			if (animateDuration > 0.0) {
				this.whenRequiredResourcesReady().then(function (requestId) {
					...
					var stopShowingPlayAction = cc.spawn(
		...
		if (!this._showingPrismatic && this.sdkCard != null) {
			...
			this.whenRequiredResourcesReady().then(function (requestId) {
				...
				if (this.sdkCard instanceof SDK.Entity) {
					...
					var glowWidth = Math.min(240.0, Math.max(100.0, boundingBox.width * 2.25));
					var glowHeight = Math.min(240.0, Math.max(100.0, boundingBox.height * 2.25));
	...
	stopShowingShine: function (duration) {
		if (this._showingShine) {
	 ...
	 * @returns {Promise}
	 */
	showStack: function (duration, stackCount, fanToLeft,stackLabelOffset,fanScale) {
	...
	stopShowingStack: function (duration) {
		if (this._cardStackLabel != null) {
		...
		this.whenRequiredResourcesReady().then(function (requestId) {
			...
			for (var i = 0, il = this._cardStackSprites.length; i < il; i++) {
				var cardStackSprite = this._cardStackSprites[i];
	 ...
	 */
	showFlip: function (duration, rotateInPlace) {
		if (duration == null) { duration = 0.5; }
		...
		var flipMod = this._flippingToFront ? -1.0 : 1.0;
		var xyzRotation = new cc.kmVec3(0.0, -180.0, 0.0);

		// animate rotation
		if (duration <= 0.0) {
			var targetXYZRotation = this.getXYZRotation();
			...
			// set starting properties
			var contentSize = this.getContentSize();
			var anchorX = flipMod < 0.0 ? 0.3 : 0.7;
			var anchorY = 0.25;
			var angRotX = 40.0 * (xyzRotation.y < 0.0 ? -1.0 : 1.0);
			var angRotY = 60.0 * (xyzRotation.x < 0.0 ? -1.0 : 1.0);
			var angRotZ = 0.0;
			var originalPosition = this.getPosition();
			var originalAnchor = this.getAnchorPoint();
			var startingPosition = cc.p(
				originalPosition.x + (anchorX - 0.5) * contentSize.width * flipMod,
				originalPosition.y + (anchorY - 0.5) * contentSize.height
			);
			var finalPosition = cc.p(
				originalPosition.x - (anchorX - 0.5) * contentSize.width * flipMod,
				originalPosition.y + (anchorY - 0.5) * contentSize.height
			);
			var intermediatePositionA = cc.p(
				originalPosition.x - (anchorX - 0.5) * contentSize.width * flipMod * 0.5,
				originalPosition.y + (anchorY - 0.5) * contentSize.height
			);
			var intermediatePositionB = cc.p(
				originalPosition.x + (anchorX - 0.5) * contentSize.width * flipMod,
				originalPosition.y + (anchorY - 0.5) * contentSize.height
			);
			this.setAnchorPoint(anchorX, anchorY);
	 ...
	 */
	resetFlip: function (duration, rotateInPlace) {
	 ...
	 * @returns {Promise}
	 */
	showBurn: function(sdkCard, startDelay, animateDuration, dissolveDelay, dissolveDuration, sourceScreenPosition, targetScreenPosition) {
		// reset
		if (sdkCard != null) {
		...
		if (this.sdkCard != null) {
			return new Promise(function (resolve, reject) {
				...
				.spread(function (requiredRequestId, cardResourceRequestId) {
					...
					var dy = targetScreenPosition.y - sourceScreenPosition.y;
					var anchorX;
					if (dx < 0.0) { anchorX = 0.0; }
					else if (dx > 0.0) { anchorX = 1.0; }
					else { anchorX = 0.5; }
					var anchorY;
					if (dy < 0.0) { anchorY = 0.0; }
					else if (dy > 0.0) { anchorY = 1.0; }
					else { anchorY = 0.5; }
					var angRotX = 30;// * (dy < 0.0 ? -1 : 1);
					var angRotY = 60 * (dx < 0.0 ? -1 : 1);
					var angRotZ = 0;
					var contentSize = this.getContentSize();
					var sourceScreenPositionWithAnchor = cc.p(sourceScreenPosition.x + (anchorX - 0.5) * contentSize.width, sourceScreenPosition.y + (anchorY - 0.5) * contentSize.height);
					var targetScreenPositionWithAnchor = cc.p(targetScreenPosition.x + (anchorX - 0.5) * contentSize.width, targetScreenPosition.y + (anchorY - 0.5) * contentSize.height);
					...
					var burnAction = cc.sequence(
						...
						cc.spawn(
							...
							cc.targetedAction(this._outlineGlow,
								cc.sequence(
									cc.show(),
	 ...
	 * @returns {Promise}
	 */
	showReveal: function (sdkCard, sourceScreenPosition, targetScreenPosition, moveDelay) {
		...
		if (this.sdkCard != null) {
			return new Promise(function (resolve, reject) {
				...
				.spread(function (requiredRequestId, cardResourceRequestId) {
					...
					var centerPosition = this.getCenterPosition();
					var cardBackgroundContentSize = this.getCardBackgroundContentSize();
					var moveDuration = (targetScreenPosition == null || cc.pSameAs(sourceScreenPosition, targetScreenPosition)) ? 0 : CONFIG.MOVE_MEDIUM_DURATION;
					var delayBeforeSetVisible = 0.0; // (moveDuration > 0) ? 0 : CONFIG.MOVE_MEDIUM_DURATION;
					var revealDuration = CONFIG.FADE_MEDIUM_DURATION * 2.0;
					...
					var fluidPuffAction = cc.sequence(
					...
					var fluidPuff2Action = cc.sequence(
						cc.delayTime(0.15),
						UtilsEngine.getAnimationAction(RSX.fxFluidCardShapePuff.name, false),
						cc.fadeOut(0.1),
						cc.removeSelf()
					...
					// fade out glow map
					var glowMapFadeAction = cc.fadeOut(0.4).easing(cc.easeIn(3.0))
					...
					if (this.sdkCard.rarityId > SDK.Rarity.Common) {
						...
						var rarityColor = SDK.RarityFactory.rarityForIdentifier(this.sdkCard.rarityId).color;
						...
						this.runAction(cc.targetedAction(fxFluidPuff,cc.sequence(
							cc.spawn(
								cc.callFunc(function(rarityFlare,fxFluidPuff){
							...
							cc.callFunc(function(){
								...
								// explosion particles
								var particles = cc.ParticleSystem.create(RSX.core_gem_particles.plist)
					...
					showRevealSequence.push(cc.callFunc(function () {
					...
					// fade out outline
					var outlineDelay = (this.sdkCard.rarityId > SDK.Rarity.Common) ? 0.25 : 0.0
					var outlineGlowAction = cc.sequence(
					...
					var showRevealAction = cc.sequence(showRevealSequence);
		...
		if (this.sdkCard != null) {
			return new Promise(function (resolve, reject) {
				...
				.spread(function (requiredRequestId, cardResourceRequestId) {
					...
					var cardBackAction = cc.sequence(
					...
					if (this.sdkCard instanceof SDK.Entity && this.sdkCard.getWasGeneral()) {
						if (this._signatureCardRing != null) {
							...
							var signatureCardRingAction = cc.spawn(
								cc.sequence(
						...
						if (this._signatureCardSprite != null) {
							...
							var signatureCardSpriteAction = cc.spawn(
								cc.sequence(
									cc.delayTime(baseTime * animationSpeed),
									cc.fadeIn(0.1 * animationSpeed)
								),
						...
						var manaLabelAction = cc.sequence(
					...
					if (!(this.sdkCard instanceof SDK.Tile)) {
						var unitShadowAction = cc.sequence(
					...
					var cardNameLabelAction = cc.spawn(
						cc.sequence(
							cc.delayTime((baseTime + 0.2) * animationSpeed),
							cc.fadeIn(0.1 * animationSpeed).easing(cc.easeOut(1.0))
						),
					...
					var cardTypeLabelAction = cc.spawn(
						cc.sequence(
					...
					if (this.raritySprite != null) {
						var raritySpriteAction = cc.spawn(
							cc.sequence(
...
var proto = CardNode.WebGLRenderCmd.prototype = Object.create(cc.Node.WebGLRenderCmd.prototype);
