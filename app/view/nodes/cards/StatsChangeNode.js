...
var StatsChangeNode = EntitySupportNode.extend({
	...
	getOrCreateATKBGSprite: function () {
	...
	getOrCreateHPBGSprite: function () {
	...
	getOrCreateHealBGSprite: function () {
	...
	getOrCreateDamageBGSprite: function () {
	...
	getCurrentATKBGSprite: function () {
	...
	getCurrentHPBGSprite: function () {
		if (this._hpNode.isVisible()) {
			if (this.damageBGSprite != null && this.damageBGSprite.isVisible()) {
				return this.damageBGSprite;
			} else if (this.hpBGSprite != null && this.hpBGSprite.isVisible()) {
				return this.hpBGSprite;
			} else if (this.healBGSprite != null && this.healBGSprite.isVisible()) {
				return this.healBGSprite;
			}
		}
	},
	...
	_updateLayout: function () {
		var atkBGSprite = this.getCurrentATKBGSprite();
		var hpBGSprite = this.getCurrentHPBGSprite();

		if (atkBGSprite == null || hpBGSprite == null) {
			this._atkNode.setPosition(0.0, 0.0);
			this._hpNode.setPosition(0.0, 0.0);
		} else {
			var atkBGContentSize = atkBGSprite.getContentSize();
			this._atkNode.setPosition(-atkBGContentSize.width * 0.35, 0.0);
			var hpBGContentSize = hpBGSprite.getContentSize();
	...
	showChanges: function (atkValue, hpValue, hpChangeType) {
		...
		if (atkValue == null) {
			...
			var atkLabel = this.getOrCreateATKLabel();
