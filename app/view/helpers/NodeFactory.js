...
var NodeFactory = {
	...
	createFX: function (fx, options) {
		...
		if(fx) {
			...
			if (numFX > 0) {
				...
				var sourceBoardPosition = options.absolutePosition || options.sourceBoardPosition;
				var targetBoardPosition = options.absolutePosition || options.targetBoardPosition;
	...
	_createFXSprites: function (fx, options, fxSprites, sourceBoardPosition, targetBoardPosition, sourceScreenPosition, targetScreenPosition, needsPositioning) {
		var numSpritesToCreate;
		...
		if (fx instanceof cc.Node) {
			...
			if (fx) {
				if(typeof fx === "string") {
					...
					var ext;
					var lastDotIndex = fx.lastIndexOf(".");
					if (lastDotIndex !== -1) {
			...
			if (fx.absolutePosition) {
				var absoluteScreenPosition = UtilsEngine.transformBoardToTileMap(fx.absolutePosition);
			...
			var atSource = fx.atSource;
			var targetToSource = fx.targetToSource;
			var sourceToTarget = fx.sourceToTarget;
		...
		// create sprites
		for(var i = 0; i < numSpritesToCreate; i++) {
	...
	_createFXSprite: function (fxSpriteClass, fx, options, fxSprites, startBoardPosition, endBoardPosition, startScreenPosition, endScreenPosition, startScreenOffset, endScreenOffset) {
		var fxSprite;
