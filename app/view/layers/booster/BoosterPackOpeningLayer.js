...
var BoosterPackOpeningLayer = FXCompositeLayer.extend({
		...
		this.whenRequiredResourcesReady().then(function (requestId) {
			...
			instructionalArrowSprite.runAction(cc.sequence(
				...
				cc.callFunc(function () {
					var rings = BaseParticleSystem.create(RSX.ptcl_ring_flash.plist);
	...
	onResize: function () {
		...
		this.whenRequiredResourcesReady().then(function (requestId) {
			...
			var scale = UtilsEngine.getWindowSizeRelativeNodeScale(this.bgImage)
	...
	showResetPack: function () {
		...
		this.whenRequiredResourcesReady().then(function (requestId) {
			...
			var fadeAction2 = cc.fadeTo(fadeDuration, 0);
	 ...
	 * @returns {Promise}
	 */
	showRevealPack: function(cardIds) {
		var revealPromise = new Promise(function (resolve, reject) {
			...
			this.whenRequiredResourcesReady().then(function (requestId) {
				...
				// second energy ball
				var energyBall2 = FXEnergyBall.create();
