...
var DeckLayer = BaseLayer.extend({
	...
	onPointerUp: function(event){
		...
		if (mouseOverCardNode) {
			var mouseOverSdkCard = mouseOverCardNode.sdkCard;
			if (mouseOverSdkCard != null) {
				var isGeneralCard = SDK.CardType.getIsEntityCardType(mouseOverSdkCard.getType()) && mouseOverSdkCard.getIsGeneral()
				if (!isGeneralCard) {
	...
	addCard: function(cardId) {
		...
		if (cardNode == null) {
			...
			// update card nodes layout
			this.repositionCardNodes(0.2).then(function(){
				// show card inserting into deck
				return cardNode.showInsert();
			});
		} else {
	...
	removeCard: function(cardId) {
		...
		// update count
		var count = this.cardCounts[cardId] = (this.cardCounts[cardId] - 1 || 0);
		...
		if (cardNode && count == 0) {
			...
			return cardNode.showRemove().then(function(){
				// update total height
				this._cardNodesTotalHeight -= cardNode.getContentSize().height + this.cardNodesSpacing;
				// update card nodes layout
