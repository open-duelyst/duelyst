...
var RunLayer = BaseLayer.extend({
	...
	ctor:function (arenaData,wonLastGauntletGame) {
		...
		// label
		var winLossLabel = i18next.t("gauntlet.win_loss_label",{win_count:arenaData.win_count, loss_count:arenaData.loss_count}).toUpperCase()
	...
	showKeyblade:function(winCount, wonLastGauntletGame) {
		var bladeNumber = BLADE_WIN_COUNT_MAP[winCount] || 1;
		...
		var bladeResource = RSX["key_blade_" + bladeNumber];
		...
		if (wonLastGauntletGame) {
			var bladeGlowResource = RSX["key_blade_outline_glow_" + bladeNumber];
			...
			.spread(function (keybladeRequestId, keybladeGlowRequestId) {
						...
						cc.sequence(
							...
							cc.spawn(
								...
								cc.callFunc(function () {
									...
									var levelUpParticles1 = new BaseParticleSystem({
			...
			this.keyBladeSprite.whenRequiredResourcesReady()
			.then(function (keybladeRequestId) {
	...
	showRunStatus:function(arenaData) {

		_.each(this._runGameNodes,function(node){
			node.destroy();
		})
		this._runGameNodes = [];
		...
		var lastPosition = cc.p(0,0);
		_.each(arenaData.games,function(game,i){
			var spriteIdentifier = game.won ? RSX.run_win.img : RSX.run_loss.img;
			var gameSprite = BaseSprite.create(spriteIdentifier);
			var lineSprite = BaseSprite.create(RSX.run_line.img);
			var median = Math.floor(arenaData.games.length / 2) + 1;
			var position = cc.p((-median+i)*(gameSprite.getContentSize().width + lineSprite.getContentSize().width),0);
		...
		var gameSprite = BaseSprite.create(RSX.run_current_game.img);
