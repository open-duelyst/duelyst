...
var RewardsLayer = BaseLayer.extend({
	...
	ctor:function (arenaData) {
		...
		var doneButtonSprite = new ccui.Scale9Sprite(RSX.button_confirm.img);
		var doneButtonGlowSprite = new ccui.Scale9Sprite(RSX.button_confirm_glow.img);
	...
	onClaimRewardsPressed: function () {
		...
		// claim rewards
		this.delegate.claimArenaRewards().catch(function () {
	...
	onDonePressed: function (){
		...
		this.delegate.markArenaRewardsAsSeen().catch(function () {
			// reset if there is a problem
			this.showDoneButton();
		}.bind(this));
	},
	...
	showRewards: function(arenaData) {
		// show results and claim or rewards
		if (arenaData.rewards == null) {
			var labelStr = i18next.t("gauntlet.claim_rewards_win_count_label",{count: arenaData.win_count}).toUpperCase()
			...
			var doneShowAction = cc.sequence(
				cc.delayTime(1.0),
			...
			var lootCrateNode = new GiftCrateNode();
			...
			.then(function () {
				return lootCrateNode.showIdleState(CONFIG.ANIMATE_MEDIUM_DURATION);
			}).then(function () {
				return lootCrateNode.showOpeningAndRewards();
			}).then(function () {
