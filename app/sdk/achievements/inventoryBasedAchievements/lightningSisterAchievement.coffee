...
class SisterAchievement extends Achievement
  @id: "lightningSister"
  @title: i18next.t("achievements.storm_sister_title")
  @description: i18next.t("achievements.storm_sister_desc")
  @progressRequired: 1
  @rewards:
    cards: [
  ...
  @progressForCardCollection: (cardCollection, allCards) ->
    ...
    sisterCount = cardCollection[Cards.Faction2.LightningSister]?.count || 0
    if sisterCount >= 3
      return 0

    # check if player owns 3 or more of at least 6 rare cards for this faction
    allFactionRares = _.filter(allCards,(card) ->
      return   card.getFactionId() == Factions.Faction2 and
