...
function deleteAllPaypalButtons(request) {
  return request.makeRequestAsync({
    ...
    .then(function (response) {
      ...
      const responseData = qs.parse(response.toString());
      const buttons = [];
      for (let i = 0; i < 500; i++) {
      ...
      const bar = new ProgressBar(`deleting ${buttons.length} paypal buttons [:bar] :percent :etas`, {
        complete: '='.yellow,
        incomplete: ' ',
        width: 50,
      ...
      return Promise.map(buttons, (b) => request.makeRequestAsync({
