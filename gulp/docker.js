...
export function build(cb) {
	...
	const build = exec(`docker build -t=${containerName} -f Dockerfile.main .`,
...
export function tag(cb) {
	...
	const tag = exec(`docker tag $(docker images | grep "^${containerName} " |  awk '{print $3}') ${containerRegistry}/${containerName}:${env}`,
	{maxBuffer: 10*1024*1024}, err => {
		if (err !== null) {
...
export function push(cb) {
	...
	const push = exec(`docker push ${containerRegistry}/${containerName}:${env}`,
	{maxBuffer: 10*1024*1024}, err => {
		if (err !== null) {
