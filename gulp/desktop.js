...
import {config, version, production, staging} from './shared'
...
export function build(opts, cb) {
	...
	const packagerOpts = {
		...
		derefSymlinks: opts.platform === 'darwin' ? false : true,
		arch: 'all',
	...
	return gulp.src('dist/desktop/*.zip')
		.pipe(tap(file => {
			file.contents = Buffer.concat([
				new Buffer(hasha(file.contents, {encoding: 'hex', algorithm: 'sha1'})),
				new Buffer('  '),
				new Buffer(path.basename(file.path)),
			])
		}))
		.pipe(rename(p => p.extname = ".zip.SHA1SUM"))
